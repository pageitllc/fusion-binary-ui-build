import { h as Qe, defineComponent as te, ref as y, reactive as _t, watch as fe, onMounted as he, onBeforeUnmount as _e, createElementBlock as o, openBlock as t, createCommentVNode as z, createElementVNode as a, createBlock as X, toDisplayString as S, createVNode as P, resolveDynamicComponent as ce, normalizeStyle as oe, normalizeClass as H, unref as Z, Teleport as ge, Transition as Ye, withCtx as ae, Fragment as U, renderList as ne, nextTick as ve, createTextVNode as ie, withDirectives as Te, renderSlot as J, mergeProps as tt, vModelDynamic as yt, vShow as nt, computed as N, withModifiers as ue, withKeys as De, vModelText as ot, toRaw as Ot, resolveComponent as gt, KeepAlive as Dt, createApp as Yt, watchEffect as Vt } from "vue";
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const st = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), Tt = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (v, n, s) => s ? s.toUpperCase() : n.toLowerCase()
), Ft = (e) => {
  const v = Tt(e);
  return v.charAt(0).toUpperCase() + v.slice(1);
}, Bt = (...e) => e.filter((v, n, s) => !!v && v.trim() !== "" && s.indexOf(v) === n).join(" ").trim(), it = (e) => e === "";
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
var Ae = {
  xmlns: "http://www.w3.org/2000/svg",
  width: 24,
  height: 24,
  viewBox: "0 0 24 24",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": 2,
  "stroke-linecap": "round",
  "stroke-linejoin": "round"
};
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const It = ({
  name: e,
  iconNode: v,
  absoluteStrokeWidth: n,
  "absolute-stroke-width": s,
  strokeWidth: l,
  "stroke-width": i,
  size: u = Ae.width,
  color: d = Ae.stroke,
  ..._
}, { slots: c }) => Qe(
  "svg",
  {
    ...Ae,
    ..._,
    width: u,
    height: u,
    stroke: d,
    "stroke-width": it(n) || it(s) || n === !0 || s === !0 ? Number(l || i || Ae["stroke-width"]) * 24 / Number(u) : l || i || Ae["stroke-width"],
    class: Bt(
      "lucide",
      _.class,
      ...e ? [`lucide-${st(Ft(e))}-icon`, `lucide-${st(e)}`] : ["lucide-icon"]
    )
  },
  [...v.map((w) => Qe(...w)), ...c.default ? [c.default()] : []]
);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const me = (e, v) => (n, { slots: s, attrs: l }) => Qe(
  It,
  {
    ...l,
    ...n,
    iconNode: v,
    name: e
  },
  s
);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Et = me("bell", [
  ["path", { d: "M10.268 21a2 2 0 0 0 3.464 0", key: "vwvbt9" }],
  [
    "path",
    {
      d: "M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326",
      key: "11g9vi"
    }
  ]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const bt = me("check", [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const $e = me("chevron-down", [
  ["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Pe = me("chevron-left", [
  ["path", { d: "m15 18-6-6 6-6", key: "1wnfg3" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const We = me("chevron-right", [
  ["path", { d: "m9 18 6-6-6-6", key: "mthhwq" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Lt = me("chevron-up", [
  ["path", { d: "m18 15-6-6-6 6", key: "153udz" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const At = me("circle-alert", [
  ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
  ["line", { x1: "12", x2: "12", y1: "8", y2: "12", key: "1pkeuh" }],
  ["line", { x1: "12", x2: "12.01", y1: "16", y2: "16", key: "4dfq90" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const wt = me("circle-check-big", [
  ["path", { d: "M21.801 10A10 10 0 1 1 17 3.335", key: "yps3ct" }],
  ["path", { d: "m9 11 3 3L22 4", key: "1pflzl" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const jt = me("circle-check", [
  ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
  ["path", { d: "m9 12 2 2 4-4", key: "dzmm74" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Pt = me("circle-x", [
  ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
  ["path", { d: "m15 9-6 6", key: "1uzhvr" }],
  ["path", { d: "m9 9 6 6", key: "z0biqf" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const at = me("ellipsis-vertical", [
  ["circle", { cx: "12", cy: "12", r: "1", key: "41hilf" }],
  ["circle", { cx: "12", cy: "5", r: "1", key: "gxeob9" }],
  ["circle", { cx: "12", cy: "19", r: "1", key: "lyex9k" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Wt = me("eye-off", [
  [
    "path",
    {
      d: "M10.733 5.076a10.744 10.744 0 0 1 11.205 6.575 1 1 0 0 1 0 .696 10.747 10.747 0 0 1-1.444 2.49",
      key: "ct8e1f"
    }
  ],
  ["path", { d: "M14.084 14.158a3 3 0 0 1-4.242-4.242", key: "151rxh" }],
  [
    "path",
    {
      d: "M17.479 17.499a10.75 10.75 0 0 1-15.417-5.151 1 1 0 0 1 0-.696 10.75 10.75 0 0 1 4.446-5.143",
      key: "13bj9a"
    }
  ],
  ["path", { d: "m2 2 20 20", key: "1ooewy" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Rt = me("eye", [
  [
    "path",
    {
      d: "M2.062 12.348a1 1 0 0 1 0-.696 10.75 10.75 0 0 1 19.876 0 1 1 0 0 1 0 .696 10.75 10.75 0 0 1-19.876 0",
      key: "1nclc0"
    }
  ],
  ["circle", { cx: "12", cy: "12", r: "3", key: "1v7zrd" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Ht = me("file-text", [
  ["path", { d: "M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z", key: "1rqfz7" }],
  ["path", { d: "M14 2v4a2 2 0 0 0 2 2h4", key: "tnqrlb" }],
  ["path", { d: "M10 9H8", key: "b1mrlr" }],
  ["path", { d: "M16 13H8", key: "t4e002" }],
  ["path", { d: "M16 17H8", key: "z1uh3a" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const ut = me("info", [
  ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
  ["path", { d: "M12 16v-4", key: "1dtifu" }],
  ["path", { d: "M12 8h.01", key: "e9boi3" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const qt = me("maximize-2", [
  ["path", { d: "M15 3h6v6", key: "1q9fwt" }],
  ["path", { d: "m21 3-7 7", key: "1l2asr" }],
  ["path", { d: "m3 21 7-7", key: "tjx5ai" }],
  ["path", { d: "M9 21H3v-6", key: "wtvkvv" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Nt = me("minimize-2", [
  ["path", { d: "m14 10 7-7", key: "oa77jy" }],
  ["path", { d: "M20 10h-6V4", key: "mjg0md" }],
  ["path", { d: "m3 21 7-7", key: "tjx5ai" }],
  ["path", { d: "M4 14h6v6", key: "rmj7iw" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const rt = me("monitor-smartphone", [
  ["path", { d: "M18 8V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h8", key: "10dyio" }],
  ["path", { d: "M10 19v-3.96 3.15", key: "1irgej" }],
  ["path", { d: "M7 19h5", key: "qswx4l" }],
  ["rect", { width: "6", height: "10", x: "16", y: "12", rx: "2", key: "1egngj" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const kt = me("square-pen", [
  ["path", { d: "M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7", key: "1m0v6g" }],
  [
    "path",
    {
      d: "M18.375 2.625a1 1 0 0 1 3 3l-9.013 9.014a2 2 0 0 1-.853.505l-2.873.84a.5.5 0 0 1-.62-.62l.84-2.873a2 2 0 0 1 .506-.852z",
      key: "ohrbg2"
    }
  ]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const lt = me("trash-2", [
  ["path", { d: "M10 11v6", key: "nco0om" }],
  ["path", { d: "M14 11v6", key: "outv1u" }],
  ["path", { d: "M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6", key: "miytrc" }],
  ["path", { d: "M3 6h18", key: "d0wm0j" }],
  ["path", { d: "M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2", key: "e791ji" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Ut = me("triangle-alert", [
  [
    "path",
    {
      d: "m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3",
      key: "wmoenq"
    }
  ],
  ["path", { d: "M12 9v4", key: "juzpu7" }],
  ["path", { d: "M12 17h.01", key: "p32p05" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Xt = me("video", [
  [
    "path",
    {
      d: "m16 13 5.223 3.482a.5.5 0 0 0 .777-.416V7.87a.5.5 0 0 0-.752-.432L16 10.5",
      key: "ftymec"
    }
  ],
  ["rect", { x: "2", y: "6", width: "14", height: "12", rx: "2", key: "158x01" }]
]);
/**
 * @license lucide-vue-next v0.544.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Ue = me("x", [
  ["path", { d: "M18 6 6 18", key: "1bl5f8" }],
  ["path", { d: "m6 6 12 12", key: "d8bk6v" }]
]), Kt = {
  key: 0,
  class: "fu-status-dropdown__label-text"
}, Jt = { key: 0 }, Zt = ["onClick"], Gt = { class: "fu-status-dropdown__item-label" }, Qt = /* @__PURE__ */ te({
  __name: "FusionStatusDropdown",
  props: {
    modelValue: {},
    options: {},
    align: { default: "left" },
    label: { default: "" },
    placeholder: { default: "Select Status" }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(!1), i = y(null), u = y(null), d = y(n.modelValue || null), _ = _t({
      position: "absolute",
      visibility: "hidden",
      opacity: "0",
      zIndex: "9999"
    });
    fe(
      () => n.modelValue,
      (r) => d.value = r
    );
    function c() {
      if (!i.value || !u.value) return;
      const r = i.value.getBoundingClientRect(), p = u.value.getBoundingClientRect(), j = window.innerHeight - r.bottom, D = r.top, V = j < p.height && D > j;
      let x = r.left + window.scrollX, I = "none";
      n.align === "center" && (x += r.width / 2, I = "translateX(-50%)"), n.align === "right" && (x = r.right + window.scrollX, I = "translateX(-100%)"), _.left = `${x}px`, _.transform = I, _.minWidth = `${r.width}px`, V ? _.top = `${r.top + window.scrollY - p.height - 6}px` : _.top = `${r.bottom + window.scrollY + 6}px`, _.visibility = "visible", _.opacity = "1";
    }
    const w = async () => {
      l.value = !l.value, l.value && (await ve(), c(), await ve(), c());
    }, f = (r) => {
      d.value = r, s("update:modelValue", r), l.value = !1;
    }, m = (r) => {
      const p = r.target;
      l.value && i.value && u.value && !i.value.contains(p) && !u.value.contains(p) && (l.value = !1);
    }, g = () => {
      l.value && (l.value = !1);
    }, k = (r) => {
      r.key === "Escape" && (l.value = !1);
    };
    return he(() => {
      document.addEventListener("click", m), window.addEventListener("resize", g), window.addEventListener("scroll", g, !0), document.addEventListener("keydown", k);
    }), _e(() => {
      document.removeEventListener("click", m), window.removeEventListener("resize", g), window.removeEventListener("scroll", g, !0), document.removeEventListener("keydown", k);
    }), (r, p) => (t(), o("div", {
      class: "fu-status-dropdown",
      ref_key: "dropdown",
      ref: i
    }, [
      e.label ? (t(), o("div", Kt, S(e.label), 1)) : z("", !0),
      a("button", {
        class: "fu-status-dropdown__button",
        onClick: w
      }, [
        d.value ? (t(), o("span", Jt, [
          d.value.icon ? (t(), X(ce(d.value.icon), {
            key: 0,
            class: "fu-status-dropdown__icon"
          })) : d.value.color ? (t(), o("span", {
            key: 1,
            class: "fu-status-dropdown__dot",
            style: oe({ backgroundColor: d.value.color })
          }, null, 4)) : z("", !0)
        ])) : z("", !0),
        a("span", {
          class: H(["fu-status-dropdown__label", { "fu-status-dropdown__placeholder": !d.value }])
        }, S(d.value?.label || e.placeholder), 3),
        P(Z($e), { class: "fu-status-dropdown__chevron" })
      ]),
      (t(), X(ge, { to: "body" }, [
        P(Ye, { name: "fade" }, {
          default: ae(() => [
            l.value ? (t(), o("ul", {
              key: 0,
              ref_key: "menuRef",
              ref: u,
              class: "fu-status-dropdown__menu",
              style: oe(_)
            }, [
              (t(!0), o(U, null, ne(e.options, (O) => (t(), o("li", {
                key: O.label,
                class: "fu-status-dropdown__item",
                onClick: (j) => f(O)
              }, [
                O.icon ? (t(), X(ce(O.icon), {
                  key: 0,
                  class: "fu-status-dropdown__icon"
                })) : O.color ? (t(), o("span", {
                  key: 1,
                  class: "fu-status-dropdown__dot",
                  style: oe({ backgroundColor: O.color })
                }, null, 4)) : z("", !0),
                a("span", Gt, S(O.label), 1)
              ], 8, Zt))), 128))
            ], 4)) : z("", !0)
          ]),
          _: 1
        })
      ]))
    ], 512));
  }
}), G = (e, v) => {
  const n = e.__vccOpts || e;
  for (const [s, l] of v)
    n[s] = l;
  return n;
}, $t = /* @__PURE__ */ G(Qt, [["__scopeId", "data-v-7725b140"]]), en = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: $t
}, Symbol.toStringTag, { value: "Module" })), tn = ["disabled"], nn = {
  key: 0,
  class: "fu-spinner"
}, on = {
  key: 0,
  class: "fu-tooltip"
}, an = /* @__PURE__ */ te({
  __name: "FusionActionButton",
  props: {
    disabled: { type: Boolean, default: !1 },
    size: { default: "md" },
    variant: { default: "subtle" },
    tooltip: { default: null },
    loading: { type: Boolean, default: !1 },
    icon: { default: null }
  },
  emits: ["click"],
  setup(e) {
    return (v, n) => (t(), o("div", {
      class: H(["fu-action-btn-wrapper", { "has-tooltip": e.tooltip }])
    }, [
      a("button", {
        class: H(["fu-action-btn", [`fu-action-btn--${e.size}`, `fu-action-btn--${e.variant}`, { "is-loading": e.loading }]]),
        disabled: e.disabled || e.loading,
        onClick: n[0] || (n[0] = (s) => e.loading ? null : v.$emit("click", s))
      }, [
        e.loading ? (t(), o("span", nn)) : e.icon ? (t(), X(ce(e.icon), {
          key: 1,
          class: "fu-action-btn__icon",
          size: 20
        })) : z("", !0)
      ], 10, tn),
      e.tooltip ? (t(), o("span", on, S(e.tooltip), 1)) : z("", !0)
    ], 2));
  }
}), ke = /* @__PURE__ */ G(an, [["__scopeId", "data-v-ca61cb3a"]]), ln = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: ke
}, Symbol.toStringTag, { value: "Module" })), sn = { class: "edf-container" }, un = {
  key: 0,
  class: "edf-label"
}, rn = { class: "edf-text" }, dn = /* @__PURE__ */ te({
  __name: "EditableDisplayField",
  props: {
    text: {},
    label: { default: "" },
    variant: { default: "solid" }
  },
  emits: ["edit"],
  setup(e, { emit: v }) {
    const n = v, s = y(!1);
    function l(i) {
      n("edit", i);
    }
    return (i, u) => (t(), o("div", sn, [
      e.label ? (t(), o("label", un, S(e.label), 1)) : z("", !0),
      a("div", {
        class: H(["edf-wrapper", [`edf--${e.variant}`]]),
        onMouseenter: u[0] || (u[0] = (d) => s.value = !0),
        onMouseleave: u[1] || (u[1] = (d) => s.value = !1),
        ref: "container"
      }, [
        a("span", rn, S(e.text), 1),
        s.value ? (t(), X(ke, {
          key: 0,
          class: "edf-edit-btn",
          icon: Z(kt),
          size: "sm",
          variant: "subtle",
          onClick: l
        }, null, 8, ["icon"])) : z("", !0)
      ], 34)
    ]));
  }
}), cn = /* @__PURE__ */ G(dn, [["__scopeId", "data-v-1e56c520"]]), fn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: cn
}, Symbol.toStringTag, { value: "Module" })), vn = {
  key: 0,
  class: "fu-input-label"
}, mn = {
  key: 0,
  class: "fu-input-required"
}, pn = {
  key: 0,
  class: "fu-input-icon fu-input-icon--left"
}, hn = ["type", "placeholder", "disabled", "required", "inputmode", "pattern", "min", "max", "step"], _n = {
  key: 1,
  class: "fu-input-icon fu-input-icon--right"
}, yn = {
  key: 1,
  class: "fu-input-error"
}, gn = /* @__PURE__ */ te({
  __name: "FusionTextInput",
  props: {
    modelValue: { default: "" },
    label: { default: "" },
    placeholder: { default: "" },
    type: { default: "text" },
    size: { default: "sm" },
    variant: { default: "outline" },
    disabled: { type: Boolean, default: !1 },
    error: { default: null },
    min: { default: void 0 },
    max: { default: void 0 },
    step: { default: void 0 },
    required: { type: Boolean, default: !1 },
    formWrapperWidth: { default: "fit-content" }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(n.modelValue);
    return fe(l, (i) => s("update:modelValue", i)), fe(
      () => n.modelValue,
      (i) => l.value = i
    ), (i, u) => (t(), o("div", {
      class: "fu-input-wrapper",
      style: oe({ width: e.formWrapperWidth })
    }, [
      e.label ? (t(), o("label", vn, [
        ie(S(e.label) + " ", 1),
        e.required ? (t(), o("span", mn, "*")) : z("", !0)
      ])) : z("", !0),
      a("div", {
        class: H(["fu-input-container", [`fu-input--${e.size}`, `fu-input--${e.variant}`, { "fu-input--error": e.error }]])
      }, [
        i.$slots.left ? (t(), o("div", pn, [
          J(i.$slots, "left", {}, void 0, !0)
        ])) : z("", !0),
        Te(a("input", tt(i.$attrs, {
          class: "fu-input",
          type: e.type,
          placeholder: e.placeholder,
          disabled: e.disabled,
          required: e.required,
          inputmode: e.type === "number" ? "numeric" : void 0,
          pattern: e.type === "number" ? "[0-9]*" : void 0,
          min: e.type === "number" ? e.min : void 0,
          max: e.type === "number" ? e.max : void 0,
          step: e.type === "number" ? e.step : void 0,
          "onUpdate:modelValue": u[0] || (u[0] = (d) => l.value = d)
        }), null, 16, hn), [
          [yt, l.value]
        ]),
        i.$slots.right ? (t(), o("div", _n, [
          J(i.$slots, "right", {}, void 0, !0)
        ])) : z("", !0)
      ], 2),
      e.error ? (t(), o("span", yn, S(e.error), 1)) : z("", !0)
    ], 4));
  }
}), xe = /* @__PURE__ */ G(gn, [["__scopeId", "data-v-7f197b54"]]), bn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: xe
}, Symbol.toStringTag, { value: "Module" })), wn = ["onClick", "disabled"], kn = { class: "fu-accordion__header-content" }, $n = { class: "fu-accordion__body" }, Mn = /* @__PURE__ */ te({
  __name: "FusionAccordion",
  props: {
    items: {},
    defaultOpen: {},
    type: {},
    theme: {}
  },
  setup(e) {
    const v = e, n = y(v.defaultOpen || []);
    function s(i) {
      return n.value.includes(i);
    }
    function l(i) {
      const u = s(i);
      v.type === "single" ? n.value = u ? [] : [i] : u ? n.value = n.value.filter((d) => d !== i) : n.value.push(i);
    }
    return (i, u) => (t(), o("div", {
      class: H(["fu-accordion", [`fu-accordion--${e.theme}`]])
    }, [
      (t(!0), o(U, null, ne(e.items, (d) => (t(), o("div", {
        key: d.key,
        class: "fu-accordion__item"
      }, [
        a("button", {
          class: H(["fu-accordion__header", {
            "is-open": s(d.key),
            "is-disabled": d.disabled
          }]),
          onClick: (_) => l(d.key),
          disabled: d.disabled
        }, [
          a("div", kn, [
            d.icon ? (t(), X(ce(d.icon), {
              key: 0,
              size: 16,
              class: "fu-accordion__icon"
            })) : z("", !0),
            a("span", null, S(d.title), 1)
          ]),
          (t(), o("svg", {
            class: H(["fu-accordion__chevron", { "is-open": s(d.key) }]),
            xmlns: "http://www.w3.org/2000/svg",
            width: "16",
            height: "16",
            fill: "none",
            viewBox: "0 0 24 24",
            stroke: "currentColor"
          }, [...u[0] || (u[0] = [
            a("path", {
              "stroke-linecap": "round",
              "stroke-linejoin": "round",
              "stroke-width": "2",
              d: "M6 9l6 6 6-6"
            }, null, -1)
          ])], 2))
        ], 10, wn),
        Te(a("div", $n, [
          J(i.$slots, d.key, {}, void 0, !0)
        ], 512), [
          [nt, s(d.key)]
        ])
      ]))), 128))
    ], 2));
  }
}), Cn = /* @__PURE__ */ G(Mn, [["__scopeId", "data-v-f069f986"]]), Sn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Cn
}, Symbol.toStringTag, { value: "Module" })), xn = {
  key: 0,
  class: "fu-status-dropdown__label-text"
}, zn = { key: 1 }, On = { key: 0 }, Dn = ["src"], Yn = ["onClick"], Vn = {
  key: 1,
  class: "fu-placeholder"
}, Tn = {
  key: 1,
  class: "flex flex--center flex--gap-md"
}, Fn = ["src"], Bn = {
  key: 2,
  class: "fu-status-dropdown__input-trigger"
}, In = {
  key: 0,
  class: "fu-search-wrapper"
}, En = {
  key: 1,
  class: "fu-options-scroll scrollbar__control customScrollBar"
}, Ln = ["onClick"], An = ["src"], jn = {
  key: 2,
  class: "fu-status-dropdown__empty"
}, Pn = {
  key: 3,
  class: "fu-input-error"
}, Wn = /* @__PURE__ */ te({
  __name: "FusionAutocomplete",
  props: {
    options: {},
    modelValue: { default: null },
    multiple: { type: Boolean, default: !1 },
    placeholder: { default: "Select..." },
    searchable: { type: Boolean, default: !0 },
    noResultsText: { default: "No results found" },
    searchPlaceholder: { default: "Search..." },
    label: { default: "" },
    variant: { default: "button" },
    size: { default: "md" },
    formWrapperWidth: { default: "" },
    error: { default: null },
    async: { type: Boolean, default: !1 },
    minSearchLength: { default: 2 },
    loading: { type: Boolean, default: !1 }
  },
  emits: ["update:modelValue", "search"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(!1), i = y(""), u = y(null), d = y(null), _ = y(null), c = y(null), w = y({}), f = y(null), m = y([]), g = N(() => n.async === !0);
    fe(
      () => n.modelValue,
      (D) => {
        n.multiple && Array.isArray(D) ? m.value = D : f.value = D;
      },
      { immediate: !0 }
    ), fe(i, (D) => {
      g.value && (D.length < n.minSearchLength || s("search", D));
    });
    const k = N(() => g.value || !i.value ? n.options : n.options.filter(
      (D) => D.label.toLowerCase().includes(i.value.toLowerCase())
    ));
    function r() {
      l.value = !l.value, l.value && ve(() => {
        const D = u.value?.querySelector("button, input");
        if (!D || !d.value) return;
        const V = D.getBoundingClientRect();
        w.value = {
          position: "absolute",
          top: `${V.bottom + window.scrollY}px`,
          left: `${V.left + window.scrollX}px`,
          width: `${V.width}px`,
          zIndex: "9999"
        }, n.searchable && ve(() => {
          const x = c.value?.$el?.querySelector("input") || d.value?.querySelector("input");
          x?.focus(), x?.select();
        });
      });
    }
    function p(D) {
      if (n.multiple) {
        const V = m.value.find((x) => x.value === D.value);
        m.value = V ? m.value.filter((x) => x.value !== D.value) : [...m.value, D], s("update:modelValue", m.value);
      } else
        f.value = D, s("update:modelValue", D), l.value = !1;
    }
    function O(D) {
      m.value = m.value.filter((V) => V.value !== D.value), s("update:modelValue", m.value);
    }
    function j(D) {
      u.value?.contains(D.target) || d.value?.contains(D.target) || (l.value = !1);
    }
    return he(() => document.addEventListener("click", j)), _e(() => document.removeEventListener("click", j)), (D, V) => (t(), o("div", {
      class: "fu-status-dropdown",
      ref_key: "dropdownRef",
      ref: u
    }, [
      e.label ? (t(), o("div", xn, S(e.label), 1)) : z("", !0),
      e.variant === "button" ? (t(), o("div", zn, [
        a("button", {
          class: H(["fu-status-dropdown__button", [`fu-input--${e.size}`, { "fu-input--error": e.error }]]),
          onClick: r
        }, [
          e.multiple ? (t(), o("div", On, [
            m.value.length ? (t(!0), o(U, { key: 0 }, ne(m.value, (x) => (t(), o("span", {
              key: x.value,
              class: "fu-tag"
            }, [
              x.type === "icon" ? (t(), X(ce(x.icon), {
                key: 0,
                size: "14"
              })) : x.type === "image" ? (t(), o("img", {
                key: 1,
                src: x.imageUrl,
                class: "fu-option-image",
                alt: "User Image"
              }, null, 8, Dn)) : z("", !0),
              ie(" " + S(x.label) + " ", 1),
              a("span", {
                class: "fu-tag__remove",
                onClick: ue((I) => O(x), ["stop"])
              }, "×", 8, Yn)
            ]))), 128)) : (t(), o("span", Vn, S(e.placeholder), 1))
          ])) : (t(), o("div", Tn, [
            f.value?.type === "icon" ? (t(), X(ce(f.value.icon), {
              key: 0,
              size: "16"
            })) : f.value?.type === "image" ? (t(), o("img", {
              key: 1,
              src: f.value.imageUrl,
              class: "fu-option-image",
              alt: "User Image"
            }, null, 8, Fn)) : z("", !0),
            a("span", null, S(f.value?.label || e.placeholder), 1)
          ])),
          V[2] || (V[2] = a("svg", {
            class: "fu-status-dropdown__chevron",
            viewBox: "0 0 20 20"
          }, [
            a("path", {
              d: "M6 8l4 4 4-4",
              stroke: "currentColor",
              "stroke-width": "1.5"
            })
          ], -1))
        ], 2)
      ])) : (t(), o("div", Bn, [
        P(xe, {
          size: e.size,
          readonly: "",
          variant: "outline",
          formWrapperWidth: e.formWrapperWidth,
          placeholder: f.value?.label || e.placeholder,
          onFocus: r,
          onClick: r
        }, null, 8, ["size", "formWrapperWidth", "placeholder"])
      ])),
      (t(), X(ge, { to: "body" }, [
        P(Ye, { name: "fade" }, {
          default: ae(() => [
            l.value ? (t(), o("div", {
              key: 0,
              class: "fu-status-dropdown__menu",
              style: oe(w.value),
              ref_key: "menuRef",
              ref: d
            }, [
              e.searchable ? (t(), o("div", In, [
                P(xe, {
                  ref_key: "searchInputRef",
                  ref: c,
                  modelValue: i.value,
                  "onUpdate:modelValue": V[1] || (V[1] = (x) => i.value = x),
                  type: "text",
                  placeholder: e.searchPlaceholder,
                  size: e.size,
                  formWrapperWidth: "100%"
                }, {
                  right: ae(() => [
                    a("button", {
                      class: "fu-search-clear",
                      onClick: V[0] || (V[0] = (x) => i.value = "")
                    }, "×")
                  ]),
                  _: 1
                }, 8, ["modelValue", "placeholder", "size"])
              ])) : z("", !0),
              k.value.length ? (t(), o("div", En, [
                (t(!0), o(U, null, ne(k.value, (x) => (t(), o("div", {
                  key: x.value,
                  class: "fu-status-dropdown__item",
                  onClick: (I) => p(x)
                }, [
                  x.type === "icon" ? (t(), X(ce(x.icon), {
                    key: 0,
                    size: "16"
                  })) : x.type === "image" ? (t(), o("img", {
                    key: 1,
                    src: x.imageUrl,
                    class: "fu-option-image"
                  }, null, 8, An)) : z("", !0),
                  a("span", null, S(x.label), 1)
                ], 8, Ln))), 128)),
                a("div", {
                  class: "fu-status-dropdown__slot-actions",
                  ref_key: "actionsRef",
                  ref: _
                }, [
                  J(D.$slots, "actions")
                ], 512)
              ])) : (t(), o("div", jn, S(e.noResultsText), 1))
            ], 4)) : z("", !0)
          ]),
          _: 3
        })
      ])),
      e.error ? (t(), o("span", Pn, S(e.error), 1)) : z("", !0)
    ], 512));
  }
}), Rn = /* @__PURE__ */ G(Wn, [["__scopeId", "data-v-5655257c"]]), Hn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Rn
}, Symbol.toStringTag, { value: "Module" }));
var Ee = /* @__PURE__ */ ((e) => (e[e.Offline = 0] = "Offline", e[e.Active = 1] = "Active", e[e.Away = 2] = "Away", e[e.Busy = 3] = "Busy", e[e.DoNotDisturb = 4] = "DoNotDisturb", e[e.Invisible = 5] = "Invisible", e))(Ee || {});
const qn = ["src", "alt"], Nn = {
  key: 1,
  class: "fu-avatar__placeholder"
}, Un = {
  key: 2,
  class: "fu-avatar__edit-overlay"
}, Xn = /* @__PURE__ */ te({
  __name: "FuAvatar",
  props: {
    src: {},
    alt: {},
    name: {},
    size: { default: "md" },
    status: {},
    showStatus: { type: Boolean, default: !0 },
    editable: { type: Boolean, default: !1 },
    allowRemove: { type: Boolean, default: !0 }
  },
  emits: ["update:src", "remove"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(null), i = N(
      () => n.name ? n.name.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase() : ""
    ), u = () => {
      n.editable && l.value?.click();
    }, d = (w) => {
      const f = w.target?.files?.[0];
      if (!f) return;
      const m = new FileReader();
      m.onload = () => {
        s("update:src", m.result);
      }, m.readAsDataURL(f);
    }, _ = () => {
      s("remove");
    }, c = N(() => {
      switch (n.status) {
        case Ee.Active:
        case 1:
          return "fu-status-dot--active";
        case Ee.Away:
        case 2:
          return "fu-status-dot--away";
        case Ee.Busy:
        case 3:
          return "fu-status-dot--busy";
        case Ee.DoNotDisturb:
        case 4:
          return "fu-status-dot--dnd";
        case Ee.Invisible:
        case 5:
          return "fu-status-dot--invisible";
        default:
          return "fu-status-dot--offline";
      }
    });
    return (w, f) => (t(), o("div", {
      class: H(["fu-avatar", [`fu-avatar--${e.size}`, { "fu-avatar--editable": e.editable }]])
    }, [
      a("div", {
        class: "fu-avatar__wrapper",
        onClick: u
      }, [
        e.src ? (t(), o("img", {
          key: 0,
          src: e.src,
          alt: e.alt,
          class: "fu-avatar__image"
        }, null, 8, qn)) : (t(), o("span", Nn, S(i.value), 1)),
        e.editable ? (t(), o("span", Un, " Edit ")) : z("", !0),
        e.editable && e.src && e.allowRemove ? (t(), o("button", {
          key: 3,
          class: "fu-avatar__remove",
          onClick: ue(_, ["stop"]),
          "aria-label": "Remove photo"
        }, " × ")) : z("", !0),
        e.showStatus && e.status !== void 0 ? (t(), o("span", {
          key: 4,
          class: H(["fu-status-dot", c.value])
        }, null, 2)) : z("", !0),
        a("input", {
          ref_key: "fileInput",
          ref: l,
          type: "file",
          accept: "image/*",
          class: "fu-avatar__file-input",
          onChange: d
        }, null, 544)
      ])
    ], 2));
  }
}), Le = /* @__PURE__ */ G(Xn, [["__scopeId", "data-v-51778eaa"]]), Kn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Le
}, Symbol.toStringTag, { value: "Module" })), Jn = /* @__PURE__ */ te({
  __name: "FuAvatarGroup",
  props: {
    users: {},
    max: {},
    size: {}
  },
  emits: ["click"],
  setup(e) {
    const v = e, n = v.max ?? 3, s = v.users.slice(0, n), l = v.users.length - n;
    return (i, u) => (t(), o("div", {
      class: "fu-avatar-group",
      onClick: u[0] || (u[0] = (d) => i.$emit("click"))
    }, [
      (t(!0), o(U, null, ne(Z(s), (d, _) => (t(), X(Le, {
        key: d.id || _,
        src: d.src,
        name: d.name,
        alt: d.alt,
        size: e.size
      }, null, 8, ["src", "name", "alt", "size"]))), 128)),
      l > 0 ? (t(), o("div", {
        key: 0,
        class: H(["fu-avatar fu-avatar--more", `fu-avatar--${e.size}`])
      }, " +" + S(l), 2)) : z("", !0)
    ]));
  }
}), Zn = /* @__PURE__ */ G(Jn, [["__scopeId", "data-v-d339fd2f"]]), Gn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Zn
}, Symbol.toStringTag, { value: "Module" })), Qn = /* @__PURE__ */ te({
  __name: "FusionBadge",
  props: {
    text: { default: "Badge" },
    size: { default: "md" },
    variant: { default: "solid" },
    themeClass: { default: "" }
  },
  setup(e) {
    return (v, n) => (t(), o("span", {
      class: H(["fu-badge", [
        `fu-badge--${e.variant}`,
        `fu-badge--${e.size}`,
        e.themeClass
      ]])
    }, [
      J(v.$slots, "default", {}, () => [
        ie(S(e.text), 1)
      ], !0)
    ], 2));
  }
}), eo = /* @__PURE__ */ G(Qn, [["__scopeId", "data-v-3281b175"]]), to = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: eo
}, Symbol.toStringTag, { value: "Module" })), no = {
  key: 0,
  class: "fu-spinner"
}, oo = { key: 2 }, ao = { key: 3 }, lo = {
  key: 0,
  class: "fu-tooltip"
}, so = /* @__PURE__ */ te({
  __name: "FusionButton",
  props: {
    text: { default: "Click Me" },
    disabled: { type: Boolean, default: !1 },
    size: { default: "sm" },
    variant: { default: "solid" },
    tooltip: { default: null },
    loading: { type: Boolean, default: !1 },
    loadingText: { default: null },
    link: { default: null },
    icon: { default: null },
    target: { default: "_parent" },
    buttonWidth: { default: "" }
  },
  emits: ["click"],
  setup(e) {
    return (v, n) => (t(), o("div", {
      class: H(["fu-btn-wrapper", { "has-tooltip": e.tooltip }]),
      style: oe(e.buttonWidth ? { width: e.buttonWidth } : {})
    }, [
      (t(), X(ce(e.link ? "a" : "button"), {
        class: H(["fu-btn", [
          `fu-btn--${e.variant}`,
          `fu-btn--${e.size}`,
          { "is-loading": e.loading }
        ]]),
        href: e.link || void 0,
        target: e.link ? e.target : void 0,
        rel: e.link && e.target === "_blank" ? "noopener noreferrer" : void 0,
        disabled: !e.link && (e.disabled || e.loading),
        onClick: n[0] || (n[0] = (s) => !e.link && !e.loading ? v.$emit("click", s) : null),
        style: oe(e.buttonWidth ? { width: e.buttonWidth } : {})
      }, {
        default: ae(() => [
          e.loading ? (t(), o("span", no)) : z("", !0),
          e.icon ? (t(), X(ce(e.icon), {
            key: 1,
            class: "fu-btn-icon",
            size: 16
          })) : z("", !0),
          e.loading ? (t(), o("span", oo, S(e.loadingText || "Loading..."), 1)) : (t(), o("span", ao, [
            J(v.$slots, "default", {}, () => [
              ie(S(e.text), 1)
            ], !0)
          ]))
        ]),
        _: 3
      }, 8, ["class", "href", "target", "rel", "disabled", "style"])),
      e.tooltip ? (t(), o("span", lo, S(e.tooltip), 1)) : z("", !0)
    ], 6));
  }
}), we = /* @__PURE__ */ G(so, [["__scopeId", "data-v-cf97bf26"]]), io = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: we
}, Symbol.toStringTag, { value: "Module" })), uo = {
  key: 0,
  class: "icon-box"
}, ro = { class: "content" }, co = { class: "value" }, fo = { class: "subtitle" }, vo = {
  key: 0,
  class: "caption"
}, mo = { class: "title" }, po = { class: "value" }, ho = {
  key: 0,
  class: "caption"
}, _o = /* @__PURE__ */ te({
  __name: "FusionStatCard",
  props: {
    variant: { default: "icon-left" },
    title: {},
    value: { default: "" },
    subtitle: { default: "" },
    icon: { type: [Function, Object, String, null], default: null },
    bordered: { type: Boolean, default: !0 },
    shadow: { type: Boolean, default: !1 }
  },
  setup(e) {
    return (v, n) => (t(), o("div", {
      class: H(["fu-stat-card", [`variant-${e.variant}`, e.bordered ? "bordered" : "", e.shadow ? "shadow" : ""]])
    }, [
      e.variant === "icon-left" ? (t(), o(U, { key: 0 }, [
        e.icon ? (t(), o("div", uo, [
          (t(), X(ce(e.icon), { class: "fu-icon" }))
        ])) : z("", !0),
        a("div", ro, [
          a("div", co, S(e.value), 1),
          a("div", fo, S(e.title), 1),
          e.subtitle ? (t(), o("div", vo, S(e.subtitle), 1)) : z("", !0)
        ])
      ], 64)) : (t(), o(U, { key: 1 }, [
        a("div", mo, S(e.title), 1),
        a("div", po, S(e.value), 1),
        e.subtitle ? (t(), o("div", ho, S(e.subtitle), 1)) : z("", !0)
      ], 64))
    ], 2));
  }
}), yo = /* @__PURE__ */ G(_o, [["__scopeId", "data-v-805e7e96"]]), go = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: yo
}, Symbol.toStringTag, { value: "Module" })), bo = ["for"], wo = ["id", "checked", "disabled"], ko = {
  key: 0,
  class: "fu-checkbox__label"
}, $o = /* @__PURE__ */ te({
  __name: "FusionCheckbox",
  props: {
    modelValue: { type: Boolean, default: !1 },
    label: {},
    id: {},
    size: { default: "md" },
    disabled: { type: Boolean }
  },
  emits: ["update:modelValue"],
  setup(e) {
    return (v, n) => (t(), o("label", {
      class: H(["fu-checkbox", [`fu-checkbox--${e.size}`, { "fu-checkbox--disabled": e.disabled }]]),
      for: e.id
    }, [
      a("input", {
        type: "checkbox",
        class: "fu-checkbox__input",
        id: e.id,
        checked: e.modelValue,
        disabled: e.disabled,
        onChange: n[0] || (n[0] = (s) => v.$emit("update:modelValue", s.target.checked))
      }, null, 40, wo),
      n[1] || (n[1] = a("span", { class: "fu-checkbox__box" }, null, -1)),
      e.label ? (t(), o("span", ko, S(e.label), 1)) : z("", !0)
    ], 10, bo));
  }
}), et = /* @__PURE__ */ G($o, [["__scopeId", "data-v-3ee76c20"]]), Mo = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: et
}, Symbol.toStringTag, { value: "Module" })), Co = {
  key: 0,
  class: "fu-input-label"
}, So = {
  key: 0,
  class: "fu-input-required"
}, xo = ["onUpdate:modelValue", "onInput", "onKeydown", "disabled"], zo = {
  key: 1,
  class: "fu-input-error"
}, Oo = /* @__PURE__ */ te({
  __name: "FusionCodeInput",
  props: {
    modelValue: { default: "" },
    label: { default: "" },
    length: { default: 6 },
    size: { default: "md" },
    variant: { default: "outline" },
    disabled: { type: Boolean, default: !1 },
    error: { default: null },
    required: { type: Boolean, default: !1 },
    formWrapperWidth: { default: "fit-content" }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(Array(n.length).fill("")), i = y([]);
    fe(
      () => n.modelValue,
      (c) => {
        if (!c) {
          l.value = Array(n.length).fill("");
          return;
        }
        const w = c.split("").slice(0, n.length);
        for (; w.length < n.length; ) w.push("");
        l.value = w;
      },
      { immediate: !0 }
    ), fe(
      l,
      () => {
        s("update:modelValue", l.value.join(""));
      },
      { deep: !0 }
    );
    function u(c, w) {
      const m = w.target.value.replace(/\D/g, "");
      l.value[c] = m, m && c < n.length - 1 && ve(() => i.value[c + 1]?.focus());
    }
    function d(c) {
      !l.value[c] && c > 0 && ve(() => i.value[c - 1]?.focus());
    }
    function _(c) {
      c.preventDefault();
      const f = (c.clipboardData?.getData("text") || "").replace(/\D/g, "").slice(0, n.length).split("");
      if (f.length !== 0) {
        f.forEach((m, g) => {
          l.value[g] = m;
        });
        for (let m = f.length; m < n.length; m++)
          l.value[m] = "";
        ve(() => {
          const m = Math.min(f.length - 1, n.length - 1);
          i.value[m]?.focus();
        });
      }
    }
    return (c, w) => (t(), o("div", {
      class: "fu-input-wrapper",
      style: oe({ width: e.formWrapperWidth })
    }, [
      e.label ? (t(), o("label", Co, [
        ie(S(e.label) + " ", 1),
        e.required ? (t(), o("span", So, "*")) : z("", !0)
      ])) : z("", !0),
      a("div", {
        class: H(["fu-code-container", [
          `fu-input--${e.size}`,
          `fu-input--${e.variant}`,
          { "fu-input--error": e.error }
        ]])
      }, [
        (t(!0), o(U, null, ne(l.value, (f, m) => Te((t(), o("input", {
          key: m,
          ref_for: !0,
          ref: (g) => i.value[m] = g,
          type: "text",
          maxlength: "1",
          inputmode: "numeric",
          pattern: "[0-9]*",
          class: "fu-code-box fu-input-container",
          "onUpdate:modelValue": (g) => l.value[m] = g,
          onInput: (g) => u(m, g),
          onKeydown: De((g) => d(m), ["backspace"]),
          onPaste: _,
          disabled: e.disabled
        }, null, 40, xo)), [
          [ot, l.value[m]]
        ])), 128))
      ], 2),
      e.error ? (t(), o("span", zo, S(e.error), 1)) : z("", !0)
    ], 4));
  }
}), Do = /* @__PURE__ */ G(Oo, [["__scopeId", "data-v-8582d137"]]), Yo = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Do
}, Symbol.toStringTag, { value: "Module" })), Vo = ["onKeydown"], To = { class: "fu-controls" }, Fo = { class: "fu-sliders" }, Bo = ["value"], Io = /* @__PURE__ */ te({
  __name: "FuColorPopover",
  props: {
    modelValue: {}
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y("hex"), i = y(!1), u = y(null), d = y({
      top: "0px",
      left: "0px"
    });
    function _() {
      i.value = !i.value, i.value && ve(c);
    }
    function c() {
      if (!u.value) return;
      const $ = u.value.getBoundingClientRect(), M = 260, b = 320, C = 8;
      let B = $.bottom + 6, T = $.left, A = "top left";
      T + M > window.innerWidth - C && (T = $.right - M, A = "top right"), T = Math.max(C, T), B + b > window.innerHeight - C && (B = $.top - b - 6, A = A.includes("right") ? "bottom right" : "bottom left"), B = Math.max(C, B), d.value = {
        top: `${B + window.scrollY}px`,
        left: `${T + window.scrollX}px`,
        transformOrigin: A
      };
    }
    function w($) {
      if (!i.value) return;
      const M = $.target;
      u.value?.contains(M) || M.closest(".fu-color-popover") || (i.value = !1);
    }
    he(() => {
      window.addEventListener("mousedown", w), window.addEventListener("resize", c);
    }), _e(() => {
      window.removeEventListener("mousedown", w), window.removeEventListener("resize", c);
    });
    const f = y(0), m = y(100), g = y(100), k = y(""), r = y(!1);
    function p($, M, b) {
      M /= 100, b /= 100;
      const C = b * M, B = C * (1 - Math.abs($ / 60 % 2 - 1)), T = b - C;
      let A = 0, K = 0, ee = 0;
      return $ < 60 ? [A, K, ee] = [C, B, 0] : $ < 120 ? [A, K, ee] = [B, C, 0] : $ < 180 ? [A, K, ee] = [0, C, B] : $ < 240 ? [A, K, ee] = [0, B, C] : $ < 300 ? [A, K, ee] = [B, 0, C] : [A, K, ee] = [C, 0, B], {
        r: Math.round((A + T) * 255),
        g: Math.round((K + T) * 255),
        b: Math.round((ee + T) * 255)
      };
    }
    function O($, M, b) {
      $ /= 255, M /= 255, b /= 255;
      const C = Math.max($, M, b), B = Math.min($, M, b), T = C - B;
      let A = 0;
      return T && (C === $ ? A = (M - b) / T % 6 : C === M ? A = (b - $) / T + 2 : A = ($ - M) / T + 4, A *= 60, A < 0 && (A += 360)), {
        h: Math.round(A),
        s: Math.round((C === 0 ? 0 : T / C) * 100),
        v: Math.round(C * 100)
      };
    }
    function j($, M, b) {
      return "#" + [$, M, b].map((C) => C.toString(16).padStart(2, "0")).join("").toUpperCase();
    }
    function D($, M, b) {
      return `rgb(${$}, ${M}, ${b})`;
    }
    const V = N(() => p(f.value, m.value, g.value)), x = N(
      () => j(V.value.r, V.value.g, V.value.b)
    ), I = N(() => ({
      background: `
    linear-gradient(to top, black, transparent),
    linear-gradient(to right, white, hsl(${f.value}, 100%, 50%))
  `
    }));
    function Y() {
      const { r: $, g: M, b } = V.value;
      s(
        "update:modelValue",
        l.value === "rgb" ? D($, M, b) : x.value
      );
    }
    function W($) {
      const M = $.trim().replace(/;$/, "");
      let b = null;
      /^#([0-9a-f]{6})$/i.test(M) && (l.value = "hex", b = {
        r: parseInt(M.slice(1, 3), 16),
        g: parseInt(M.slice(3, 5), 16),
        b: parseInt(M.slice(5, 7), 16)
      });
      const C = M.match(
        /^rgba?\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})/
      );
      if (C && (l.value = "rgb", b = {
        r: +C[1],
        g: +C[2],
        b: +C[3]
      }), !b) return;
      const B = O(b.r, b.g, b.b);
      f.value = B.h, m.value = B.s, g.value = B.v, Y();
    }
    function q($) {
      r.value = !0, k.value = $.target.value;
    }
    function R() {
      r.value = !1, W(k.value);
    }
    function E($) {
      $.key === "Enter" && R();
    }
    function Q($) {
      l.value = "hex";
      const b = $.currentTarget.getBoundingClientRect(), C = Math.min(Math.max(0, $.clientX - b.left), b.width), B = Math.min(Math.max(0, $.clientY - b.top), b.height);
      m.value = Math.round(C / b.width * 100), g.value = Math.round(100 - B / b.height * 100), Y();
    }
    return he(() => {
      n.modelValue && (k.value = n.modelValue, W(n.modelValue));
    }), fe(() => n.modelValue, ($) => {
      $ && (k.value = $, W($));
    }), ($, M) => (t(), o(U, null, [
      a("div", {
        ref_key: "triggerRef",
        ref: u,
        class: "fu-color-trigger",
        style: oe({ backgroundColor: x.value }),
        role: "button",
        tabindex: "0",
        onClick: _,
        onKeydown: [
          De(ue(_, ["prevent"]), ["enter"]),
          De(ue(_, ["prevent"]), ["space"])
        ]
      }, null, 44, Vo),
      (t(), X(ge, { to: "body" }, [
        i.value ? (t(), o("div", {
          key: 0,
          class: "fu-color-popover",
          style: oe(d.value)
        }, [
          a("div", {
            class: "fu-saturation",
            style: oe(I.value),
            onPointerdown: Q,
            onPointermove: M[0] || (M[0] = (b) => b.buttons === 1 && Q(b))
          }, [
            a("div", {
              class: "fu-cursor",
              style: oe({ left: m.value + "%", top: 100 - g.value + "%" })
            }, null, 4)
          ], 36),
          a("div", To, [
            a("div", {
              class: "fu-preview",
              style: oe({ backgroundColor: x.value })
            }, null, 4),
            a("div", Fo, [
              Te(a("input", {
                type: "range",
                min: "0",
                max: "360",
                "onUpdate:modelValue": M[1] || (M[1] = (b) => f.value = b),
                class: "fu-hue"
              }, null, 512), [
                [ot, f.value]
              ])
            ])
          ]),
          a("input", {
            class: "fu-output",
            value: k.value,
            onInput: q,
            onBlur: R,
            onKeydown: E,
            placeholder: "#RRGGBB or rgb(...)"
          }, null, 40, Bo)
        ], 4)) : z("", !0)
      ]))
    ], 64));
  }
}), Eo = /* @__PURE__ */ G(Io, [["__scopeId", "data-v-568b3b3c"]]), Lo = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Eo
}, Symbol.toStringTag, { value: "Module" })), Ao = { class: "fu-combobox__control" }, jo = ["value", "placeholder", "disabled"], Po = {
  key: 0,
  class: "fu-combobox__dropdown scrollbar__control customScrollBar"
}, Wo = { class: "fu-combobox__group-title" }, Ro = ["onClick"], Ho = { class: "fu-combobox__option-left" }, qo = { class: "fu-combobox__option-right" }, No = {
  key: 1,
  class: "fu-combobox__empty"
}, Uo = /* @__PURE__ */ te({
  __name: "FuCombobox",
  props: {
    options: {},
    modelValue: {},
    placeholder: {},
    disabled: { type: Boolean }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(!1), i = y(""), u = y(null), d = y(null), _ = N(() => {
      if (!i.value || u.value && i.value.toLowerCase() === u.value.label.toLowerCase())
        return c(n.options);
      const k = n.options.filter(
        (r) => r.label.toLowerCase().includes(i.value.toLowerCase())
      );
      return c(k);
    });
    function c(k) {
      const r = {};
      return k.forEach((p) => {
        const O = p.group || "Options";
        r[O] || (r[O] = []), r[O].push(p);
      }), Object.entries(r).map(([p, O]) => ({ title: p, items: O }));
    }
    fe(
      () => n.modelValue,
      (k) => {
        u.value = n.options.find((r) => r.value === k) || null, !l.value && u.value && (i.value = u.value.label);
      },
      { immediate: !0 }
    );
    function w(k) {
      i.value = k.target.value;
    }
    function f(k) {
      u.value = k, i.value = k.label, s("update:modelValue", k.value), l.value = !1;
    }
    function m() {
      l.value = !l.value;
    }
    function g(k) {
      d.value && !d.value.contains(k.target) && (l.value = !1, u.value && (i.value = u.value.label));
    }
    return he(() => {
      document.addEventListener("click", g);
    }), _e(() => {
      document.removeEventListener("click", g);
    }), (k, r) => (t(), o("div", {
      class: H(["fu-combobox", { "fu-combobox--disabled": e.disabled }]),
      ref_key: "comboboxRef",
      ref: d
    }, [
      a("div", Ao, [
        a("input", {
          type: "text",
          value: l.value ? i.value : u.value?.label || "",
          placeholder: e.placeholder,
          class: "fu-combobox__input",
          disabled: e.disabled,
          onInput: w,
          onFocus: r[0] || (r[0] = (p) => !e.disabled && (l.value = !0))
        }, null, 40, jo),
        a("span", {
          class: "fu-combobox__icon",
          onClick: ue(m, ["stop"])
        }, [
          P(Z($e), {
            size: 18,
            "stroke-width": 1
          })
        ])
      ]),
      l.value && !e.disabled ? (t(), o("div", Po, [
        _.value.length > 0 ? (t(!0), o(U, { key: 0 }, ne(_.value, (p) => (t(), o("div", {
          key: p.title,
          class: "fu-combobox__group"
        }, [
          a("div", Wo, S(p.title), 1),
          (t(!0), o(U, null, ne(p.items, (O) => (t(), o("div", {
            key: O.value,
            class: H(["fu-combobox__option", {
              "fu-combobox__option--selected": O.value === u.value?.value
            }]),
            onClick: (j) => f(O)
          }, [
            a("div", Ho, [
              J(k.$slots, "option", { option: O }, () => [
                O.icon ? (t(), X(ce(O.icon), {
                  key: 0,
                  class: "fu-combobox__option-icon"
                })) : z("", !0),
                a("span", null, S(O.label), 1)
              ], !0)
            ]),
            a("div", qo, [
              O.value === u.value?.value ? (t(), X(Z(bt), {
                key: 0,
                class: "fu-combobox__check"
              })) : z("", !0)
            ])
          ], 10, Ro))), 128))
        ]))), 128)) : (t(), o("div", No, "No results found"))
      ])) : z("", !0)
    ], 2));
  }
}), Xo = /* @__PURE__ */ G(Uo, [["__scopeId", "data-v-8ca05e69"]]), Ko = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Xo
}, Symbol.toStringTag, { value: "Module" }));
function Mt(e) {
  return e && e.__esModule && Object.prototype.hasOwnProperty.call(e, "default") ? e.default : e;
}
var qe = { exports: {} }, Jo = qe.exports, dt;
function Zo() {
  return dt || (dt = 1, (function(e, v) {
    (function(n, s) {
      e.exports = s();
    })(Jo, (function() {
      var n = 1e3, s = 6e4, l = 36e5, i = "millisecond", u = "second", d = "minute", _ = "hour", c = "day", w = "week", f = "month", m = "quarter", g = "year", k = "date", r = "Invalid Date", p = /^(\d{4})[-/]?(\d{1,2})?[-/]?(\d{0,2})[Tt\s]*(\d{1,2})?:?(\d{1,2})?:?(\d{1,2})?[.:]?(\d+)?$/, O = /\[([^\]]+)]|Y{1,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a|A|m{1,2}|s{1,2}|Z{1,2}|SSS/g, j = { name: "en", weekdays: "Sunday_Monday_Tuesday_Wednesday_Thursday_Friday_Saturday".split("_"), months: "January_February_March_April_May_June_July_August_September_October_November_December".split("_"), ordinal: function(M) {
        var b = ["th", "st", "nd", "rd"], C = M % 100;
        return "[" + M + (b[(C - 20) % 10] || b[C] || b[0]) + "]";
      } }, D = function(M, b, C) {
        var B = String(M);
        return !B || B.length >= b ? M : "" + Array(b + 1 - B.length).join(C) + M;
      }, V = { s: D, z: function(M) {
        var b = -M.utcOffset(), C = Math.abs(b), B = Math.floor(C / 60), T = C % 60;
        return (b <= 0 ? "+" : "-") + D(B, 2, "0") + ":" + D(T, 2, "0");
      }, m: function M(b, C) {
        if (b.date() < C.date()) return -M(C, b);
        var B = 12 * (C.year() - b.year()) + (C.month() - b.month()), T = b.clone().add(B, f), A = C - T < 0, K = b.clone().add(B + (A ? -1 : 1), f);
        return +(-(B + (C - T) / (A ? T - K : K - T)) || 0);
      }, a: function(M) {
        return M < 0 ? Math.ceil(M) || 0 : Math.floor(M);
      }, p: function(M) {
        return { M: f, y: g, w, d: c, D: k, h: _, m: d, s: u, ms: i, Q: m }[M] || String(M || "").toLowerCase().replace(/s$/, "");
      }, u: function(M) {
        return M === void 0;
      } }, x = "en", I = {};
      I[x] = j;
      var Y = "$isDayjsObject", W = function(M) {
        return M instanceof Q || !(!M || !M[Y]);
      }, q = function M(b, C, B) {
        var T;
        if (!b) return x;
        if (typeof b == "string") {
          var A = b.toLowerCase();
          I[A] && (T = A), C && (I[A] = C, T = A);
          var K = b.split("-");
          if (!T && K.length > 1) return M(K[0]);
        } else {
          var ee = b.name;
          I[ee] = b, T = ee;
        }
        return !B && T && (x = T), T || !B && x;
      }, R = function(M, b) {
        if (W(M)) return M.clone();
        var C = typeof b == "object" ? b : {};
        return C.date = M, C.args = arguments, new Q(C);
      }, E = V;
      E.l = q, E.i = W, E.w = function(M, b) {
        return R(M, { locale: b.$L, utc: b.$u, x: b.$x, $offset: b.$offset });
      };
      var Q = (function() {
        function M(C) {
          this.$L = q(C.locale, null, !0), this.parse(C), this.$x = this.$x || C.x || {}, this[Y] = !0;
        }
        var b = M.prototype;
        return b.parse = function(C) {
          this.$d = (function(B) {
            var T = B.date, A = B.utc;
            if (T === null) return /* @__PURE__ */ new Date(NaN);
            if (E.u(T)) return /* @__PURE__ */ new Date();
            if (T instanceof Date) return new Date(T);
            if (typeof T == "string" && !/Z$/i.test(T)) {
              var K = T.match(p);
              if (K) {
                var ee = K[2] - 1 || 0, re = (K[7] || "0").substring(0, 3);
                return A ? new Date(Date.UTC(K[1], ee, K[3] || 1, K[4] || 0, K[5] || 0, K[6] || 0, re)) : new Date(K[1], ee, K[3] || 1, K[4] || 0, K[5] || 0, K[6] || 0, re);
              }
            }
            return new Date(T);
          })(C), this.init();
        }, b.init = function() {
          var C = this.$d;
          this.$y = C.getFullYear(), this.$M = C.getMonth(), this.$D = C.getDate(), this.$W = C.getDay(), this.$H = C.getHours(), this.$m = C.getMinutes(), this.$s = C.getSeconds(), this.$ms = C.getMilliseconds();
        }, b.$utils = function() {
          return E;
        }, b.isValid = function() {
          return this.$d.toString() !== r;
        }, b.isSame = function(C, B) {
          var T = R(C);
          return this.startOf(B) <= T && T <= this.endOf(B);
        }, b.isAfter = function(C, B) {
          return R(C) < this.startOf(B);
        }, b.isBefore = function(C, B) {
          return this.endOf(B) < R(C);
        }, b.$g = function(C, B, T) {
          return E.u(C) ? this[B] : this.set(T, C);
        }, b.unix = function() {
          return Math.floor(this.valueOf() / 1e3);
        }, b.valueOf = function() {
          return this.$d.getTime();
        }, b.startOf = function(C, B) {
          var T = this, A = !!E.u(B) || B, K = E.p(C), ee = function(Me, h) {
            var F = E.w(T.$u ? Date.UTC(T.$y, h, Me) : new Date(T.$y, h, Me), T);
            return A ? F : F.endOf(c);
          }, re = function(Me, h) {
            return E.w(T.toDate()[Me].apply(T.toDate("s"), (A ? [0, 0, 0, 0] : [23, 59, 59, 999]).slice(h)), T);
          }, de = this.$W, pe = this.$M, ye = this.$D, Ce = "set" + (this.$u ? "UTC" : "");
          switch (K) {
            case g:
              return A ? ee(1, 0) : ee(31, 11);
            case f:
              return A ? ee(1, pe) : ee(0, pe + 1);
            case w:
              var be = this.$locale().weekStart || 0, ze = (de < be ? de + 7 : de) - be;
              return ee(A ? ye - ze : ye + (6 - ze), pe);
            case c:
            case k:
              return re(Ce + "Hours", 0);
            case _:
              return re(Ce + "Minutes", 1);
            case d:
              return re(Ce + "Seconds", 2);
            case u:
              return re(Ce + "Milliseconds", 3);
            default:
              return this.clone();
          }
        }, b.endOf = function(C) {
          return this.startOf(C, !1);
        }, b.$set = function(C, B) {
          var T, A = E.p(C), K = "set" + (this.$u ? "UTC" : ""), ee = (T = {}, T[c] = K + "Date", T[k] = K + "Date", T[f] = K + "Month", T[g] = K + "FullYear", T[_] = K + "Hours", T[d] = K + "Minutes", T[u] = K + "Seconds", T[i] = K + "Milliseconds", T)[A], re = A === c ? this.$D + (B - this.$W) : B;
          if (A === f || A === g) {
            var de = this.clone().set(k, 1);
            de.$d[ee](re), de.init(), this.$d = de.set(k, Math.min(this.$D, de.daysInMonth())).$d;
          } else ee && this.$d[ee](re);
          return this.init(), this;
        }, b.set = function(C, B) {
          return this.clone().$set(C, B);
        }, b.get = function(C) {
          return this[E.p(C)]();
        }, b.add = function(C, B) {
          var T, A = this;
          C = Number(C);
          var K = E.p(B), ee = function(pe) {
            var ye = R(A);
            return E.w(ye.date(ye.date() + Math.round(pe * C)), A);
          };
          if (K === f) return this.set(f, this.$M + C);
          if (K === g) return this.set(g, this.$y + C);
          if (K === c) return ee(1);
          if (K === w) return ee(7);
          var re = (T = {}, T[d] = s, T[_] = l, T[u] = n, T)[K] || 1, de = this.$d.getTime() + C * re;
          return E.w(de, this);
        }, b.subtract = function(C, B) {
          return this.add(-1 * C, B);
        }, b.format = function(C) {
          var B = this, T = this.$locale();
          if (!this.isValid()) return T.invalidDate || r;
          var A = C || "YYYY-MM-DDTHH:mm:ssZ", K = E.z(this), ee = this.$H, re = this.$m, de = this.$M, pe = T.weekdays, ye = T.months, Ce = T.meridiem, be = function(h, F, L, le) {
            return h && (h[F] || h(B, A)) || L[F].slice(0, le);
          }, ze = function(h) {
            return E.s(ee % 12 || 12, h, "0");
          }, Me = Ce || function(h, F, L) {
            var le = h < 12 ? "AM" : "PM";
            return L ? le.toLowerCase() : le;
          };
          return A.replace(O, (function(h, F) {
            return F || (function(L) {
              switch (L) {
                case "YY":
                  return String(B.$y).slice(-2);
                case "YYYY":
                  return E.s(B.$y, 4, "0");
                case "M":
                  return de + 1;
                case "MM":
                  return E.s(de + 1, 2, "0");
                case "MMM":
                  return be(T.monthsShort, de, ye, 3);
                case "MMMM":
                  return be(ye, de);
                case "D":
                  return B.$D;
                case "DD":
                  return E.s(B.$D, 2, "0");
                case "d":
                  return String(B.$W);
                case "dd":
                  return be(T.weekdaysMin, B.$W, pe, 2);
                case "ddd":
                  return be(T.weekdaysShort, B.$W, pe, 3);
                case "dddd":
                  return pe[B.$W];
                case "H":
                  return String(ee);
                case "HH":
                  return E.s(ee, 2, "0");
                case "h":
                  return ze(1);
                case "hh":
                  return ze(2);
                case "a":
                  return Me(ee, re, !0);
                case "A":
                  return Me(ee, re, !1);
                case "m":
                  return String(re);
                case "mm":
                  return E.s(re, 2, "0");
                case "s":
                  return String(B.$s);
                case "ss":
                  return E.s(B.$s, 2, "0");
                case "SSS":
                  return E.s(B.$ms, 3, "0");
                case "Z":
                  return K;
              }
              return null;
            })(h) || K.replace(":", "");
          }));
        }, b.utcOffset = function() {
          return 15 * -Math.round(this.$d.getTimezoneOffset() / 15);
        }, b.diff = function(C, B, T) {
          var A, K = this, ee = E.p(B), re = R(C), de = (re.utcOffset() - this.utcOffset()) * s, pe = this - re, ye = function() {
            return E.m(K, re);
          };
          switch (ee) {
            case g:
              A = ye() / 12;
              break;
            case f:
              A = ye();
              break;
            case m:
              A = ye() / 3;
              break;
            case w:
              A = (pe - de) / 6048e5;
              break;
            case c:
              A = (pe - de) / 864e5;
              break;
            case _:
              A = pe / l;
              break;
            case d:
              A = pe / s;
              break;
            case u:
              A = pe / n;
              break;
            default:
              A = pe;
          }
          return T ? A : E.a(A);
        }, b.daysInMonth = function() {
          return this.endOf(f).$D;
        }, b.$locale = function() {
          return I[this.$L];
        }, b.locale = function(C, B) {
          if (!C) return this.$L;
          var T = this.clone(), A = q(C, B, !0);
          return A && (T.$L = A), T;
        }, b.clone = function() {
          return E.w(this.$d, this);
        }, b.toDate = function() {
          return new Date(this.valueOf());
        }, b.toJSON = function() {
          return this.isValid() ? this.toISOString() : null;
        }, b.toISOString = function() {
          return this.$d.toISOString();
        }, b.toString = function() {
          return this.$d.toUTCString();
        }, M;
      })(), $ = Q.prototype;
      return R.prototype = $, [["$ms", i], ["$s", u], ["$m", d], ["$H", _], ["$W", c], ["$M", f], ["$y", g], ["$D", k]].forEach((function(M) {
        $[M[1]] = function(b) {
          return this.$g(b, M[0], M[1]);
        };
      })), R.extend = function(M, b) {
        return M.$i || (M(b, Q, R), M.$i = !0), R;
      }, R.locale = q, R.isDayjs = W, R.unix = function(M) {
        return R(1e3 * M);
      }, R.en = I[x], R.Ls = I, R.p = {}, R;
    }));
  })(qe)), qe.exports;
}
var Go = Zo();
const se = /* @__PURE__ */ Mt(Go);
var Ne = { exports: {} }, Qo = Ne.exports, ct;
function ea() {
  return ct || (ct = 1, (function(e, v) {
    (function(n, s) {
      e.exports = s();
    })(Qo, (function() {
      var n = { LTS: "h:mm:ss A", LT: "h:mm A", L: "MM/DD/YYYY", LL: "MMMM D, YYYY", LLL: "MMMM D, YYYY h:mm A", LLLL: "dddd, MMMM D, YYYY h:mm A" }, s = /(\[[^[]*\])|([-_:/.,()\s]+)|(A|a|Q|YYYY|YY?|ww?|MM?M?M?|Do|DD?|hh?|HH?|mm?|ss?|S{1,3}|z|ZZ?)/g, l = /\d/, i = /\d\d/, u = /\d\d?/, d = /\d*[^-_:/,()\s\d]+/, _ = {}, c = function(p) {
        return (p = +p) + (p > 68 ? 1900 : 2e3);
      }, w = function(p) {
        return function(O) {
          this[p] = +O;
        };
      }, f = [/[+-]\d\d:?(\d\d)?|Z/, function(p) {
        (this.zone || (this.zone = {})).offset = (function(O) {
          if (!O || O === "Z") return 0;
          var j = O.match(/([+-]|\d\d)/g), D = 60 * j[1] + (+j[2] || 0);
          return D === 0 ? 0 : j[0] === "+" ? -D : D;
        })(p);
      }], m = function(p) {
        var O = _[p];
        return O && (O.indexOf ? O : O.s.concat(O.f));
      }, g = function(p, O) {
        var j, D = _.meridiem;
        if (D) {
          for (var V = 1; V <= 24; V += 1) if (p.indexOf(D(V, 0, O)) > -1) {
            j = V > 12;
            break;
          }
        } else j = p === (O ? "pm" : "PM");
        return j;
      }, k = { A: [d, function(p) {
        this.afternoon = g(p, !1);
      }], a: [d, function(p) {
        this.afternoon = g(p, !0);
      }], Q: [l, function(p) {
        this.month = 3 * (p - 1) + 1;
      }], S: [l, function(p) {
        this.milliseconds = 100 * +p;
      }], SS: [i, function(p) {
        this.milliseconds = 10 * +p;
      }], SSS: [/\d{3}/, function(p) {
        this.milliseconds = +p;
      }], s: [u, w("seconds")], ss: [u, w("seconds")], m: [u, w("minutes")], mm: [u, w("minutes")], H: [u, w("hours")], h: [u, w("hours")], HH: [u, w("hours")], hh: [u, w("hours")], D: [u, w("day")], DD: [i, w("day")], Do: [d, function(p) {
        var O = _.ordinal, j = p.match(/\d+/);
        if (this.day = j[0], O) for (var D = 1; D <= 31; D += 1) O(D).replace(/\[|\]/g, "") === p && (this.day = D);
      }], w: [u, w("week")], ww: [i, w("week")], M: [u, w("month")], MM: [i, w("month")], MMM: [d, function(p) {
        var O = m("months"), j = (m("monthsShort") || O.map((function(D) {
          return D.slice(0, 3);
        }))).indexOf(p) + 1;
        if (j < 1) throw new Error();
        this.month = j % 12 || j;
      }], MMMM: [d, function(p) {
        var O = m("months").indexOf(p) + 1;
        if (O < 1) throw new Error();
        this.month = O % 12 || O;
      }], Y: [/[+-]?\d+/, w("year")], YY: [i, function(p) {
        this.year = c(p);
      }], YYYY: [/\d{4}/, w("year")], Z: f, ZZ: f };
      function r(p) {
        var O, j;
        O = p, j = _ && _.formats;
        for (var D = (p = O.replace(/(\[[^\]]+])|(LTS?|l{1,4}|L{1,4})/g, (function(R, E, Q) {
          var $ = Q && Q.toUpperCase();
          return E || j[Q] || n[Q] || j[$].replace(/(\[[^\]]+])|(MMMM|MM|DD|dddd)/g, (function(M, b, C) {
            return b || C.slice(1);
          }));
        }))).match(s), V = D.length, x = 0; x < V; x += 1) {
          var I = D[x], Y = k[I], W = Y && Y[0], q = Y && Y[1];
          D[x] = q ? { regex: W, parser: q } : I.replace(/^\[|\]$/g, "");
        }
        return function(R) {
          for (var E = {}, Q = 0, $ = 0; Q < V; Q += 1) {
            var M = D[Q];
            if (typeof M == "string") $ += M.length;
            else {
              var b = M.regex, C = M.parser, B = R.slice($), T = b.exec(B)[0];
              C.call(E, T), R = R.replace(T, "");
            }
          }
          return (function(A) {
            var K = A.afternoon;
            if (K !== void 0) {
              var ee = A.hours;
              K ? ee < 12 && (A.hours += 12) : ee === 12 && (A.hours = 0), delete A.afternoon;
            }
          })(E), E;
        };
      }
      return function(p, O, j) {
        j.p.customParseFormat = !0, p && p.parseTwoDigitYear && (c = p.parseTwoDigitYear);
        var D = O.prototype, V = D.parse;
        D.parse = function(x) {
          var I = x.date, Y = x.utc, W = x.args;
          this.$u = Y;
          var q = W[1];
          if (typeof q == "string") {
            var R = W[2] === !0, E = W[3] === !0, Q = R || E, $ = W[2];
            E && ($ = W[2]), _ = this.$locale(), !R && $ && (_ = j.Ls[$]), this.$d = (function(B, T, A, K) {
              try {
                if (["x", "X"].indexOf(T) > -1) return new Date((T === "X" ? 1e3 : 1) * B);
                var ee = r(T)(B), re = ee.year, de = ee.month, pe = ee.day, ye = ee.hours, Ce = ee.minutes, be = ee.seconds, ze = ee.milliseconds, Me = ee.zone, h = ee.week, F = /* @__PURE__ */ new Date(), L = pe || (re || de ? 1 : F.getDate()), le = re || F.getFullYear(), Se = 0;
                re && !de || (Se = de > 0 ? de - 1 : F.getMonth());
                var Ve, Oe = ye || 0, Ke = Ce || 0, Je = be || 0, Ze = ze || 0;
                return Me ? new Date(Date.UTC(le, Se, L, Oe, Ke, Je, Ze + 60 * Me.offset * 1e3)) : A ? new Date(Date.UTC(le, Se, L, Oe, Ke, Je, Ze)) : (Ve = new Date(le, Se, L, Oe, Ke, Je, Ze), h && (Ve = K(Ve).week(h).toDate()), Ve);
              } catch {
                return /* @__PURE__ */ new Date("");
              }
            })(I, q, Y, j), this.init(), $ && $ !== !0 && (this.$L = this.locale($).$L), Q && I != this.format(q) && (this.$d = /* @__PURE__ */ new Date("")), _ = {};
          } else if (q instanceof Array) for (var M = q.length, b = 1; b <= M; b += 1) {
            W[1] = q[b - 1];
            var C = j.apply(this, W);
            if (C.isValid()) {
              this.$d = C.$d, this.$L = C.$L, this.init();
              break;
            }
            b === M && (this.$d = /* @__PURE__ */ new Date(""));
          }
          else V.call(this, x);
        };
      };
    }));
  })(Ne)), Ne.exports;
}
var ta = ea();
const Ct = /* @__PURE__ */ Mt(ta), na = { class: "calendar-header" }, oa = { class: "flex flex--gap-sm" }, aa = { key: 0 }, la = { class: "calendar-weekdays" }, sa = { class: "calendar-days" }, ia = ["onClick"], ua = {
  key: 1,
  class: "calendar-months"
}, ra = ["onClick"], da = {
  key: 2,
  class: "calendar-years"
}, ca = ["onClick"], fa = { class: "flex flex--space flex--gap-md px-2 pb-2" }, va = {
  key: 0,
  class: "flex flex--gap-sm"
}, ma = { key: 1 }, pa = {
  key: 3,
  class: "calendar-time"
}, ha = { class: "fu-time-input-wrapper" }, _a = {
  key: 0,
  class: "fu-time-dropdown customScrollBar"
}, ya = ["onMousedown"], Be = 12, ga = {
  __name: "FusionDatePicker",
  props: {
    modelValue: [String, Object],
    variant: {
      type: String,
      default: "date",
      validator: (e) => ["date", "date-time", "date-range"].includes(e)
    },
    min: String,
    max: String,
    // New props
    formWrapperWidth: String,
    size: {
      type: String,
      default: "md",
      validator: (e) => ["sm", "md", "lg"].includes(e)
    },
    error: {
      type: [String, null],
      default: null
    },
    label: {
      type: String,
      default: null
    },
    required: {
      type: Boolean,
      default: !1
    },
    disabled: {
      type: Boolean,
      default: !1
    }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    se.extend(Ct);
    const n = e, s = y(!1), l = v, i = y(!1), u = y(null), d = y(null), _ = y(null), c = y(se().startOf("month")), w = y(null), f = y({ start: null, end: null }), m = y("00:00"), g = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"], k = [
      "Jan",
      "Feb",
      "Mar",
      "Apr",
      "May",
      "Jun",
      "Jul",
      "Aug",
      "Sep",
      "Oct",
      "Nov",
      "Dec"
    ], r = y("days"), p = N(
      () => Math.floor(c.value.year() / Be) * Be
    ), O = N(() => p.value + Be - 1), j = N(
      () => Array.from({ length: Be }, (h, F) => p.value + F)
    ), D = N(() => {
      const h = c.value.startOf("month").startOf("week"), F = c.value.endOf("month").endOf("week"), L = [];
      let le = h.clone();
      for (; le.isBefore(F) || le.isSame(F, "day"); )
        L.push({
          date: le.clone(),
          isCurrentMonth: le.month() === c.value.month()
        }), le = le.add(1, "day");
      return L;
    });
    function V(h) {
      return !!(n.min && h.isBefore(se(n.min), "day") || n.max && h.isAfter(se(n.max), "day"));
    }
    function x(h) {
      return n.variant === "date-range" ? f.value.start && h.isSame(f.value.start, "day") || f.value.end && h.isSame(f.value.end, "day") : w.value && h.isSame(w.value, "day");
    }
    function I(h) {
      return n.variant === "date-range" && f.value.start && f.value.end && h.isAfter(f.value.start, "day") && h.isBefore(f.value.end, "day");
    }
    function Y(h) {
      if (!V(h)) {
        if (n.variant === "date-range") {
          !f.value.start || u.value === "start" ? (f.value.start = h.clone(), f.value.end = null, u.value = "end") : h.isBefore(f.value.start, "day") ? (f.value.end = f.value.start.clone(), f.value.start = h.clone()) : f.value.end = h.clone();
          return;
        }
        w.value = h.clone(), l(
          "update:modelValue",
          n.variant === "date-time" ? w.value.format("YYYY-MM-DDTHH:mm") : w.value.format("YYYY-MM-DD")
        ), R();
      }
    }
    function W() {
      f.value.start && f.value.end && (l("update:modelValue", {
        start: f.value.start.format("YYYY-MM-DD"),
        end: f.value.end.format("YYYY-MM-DD")
      }), R());
    }
    function q(h = null) {
      u.value = h, i.value = !0, n.variant === "date-range" ? f.value.start ? c.value = f.value.start.startOf("month") : c.value = se().startOf("month") : w.value ? c.value = w.value.startOf("month") : c.value = se().startOf("month"), ve(() => {
        $(), window.addEventListener("resize", $), window.addEventListener("click", E);
      });
    }
    function R() {
      i.value = !1, window.removeEventListener("resize", $), window.removeEventListener("click", E);
    }
    function E(h) {
      !d.value?.contains(h.target) && !_.value?.contains(h.target) && R();
    }
    const Q = y({
      position: "absolute",
      top: "0px",
      left: "0px",
      zIndex: 9999
    });
    function $() {
      if (!d.value || !_.value) return;
      const h = d.value.getBoundingClientRect(), F = _.value.getBoundingClientRect(), L = window.innerHeight - h.bottom, le = h.top, Se = window.scrollY || window.pageYOffset, Ve = window.scrollX || window.pageXOffset;
      let Oe;
      L < F.height && le > F.height ? Oe = h.top + Se - F.height - 6 : Oe = h.bottom + Se + 6, Q.value = {
        position: "absolute",
        top: `${Oe}px`,
        left: `${h.left + Ve}px`,
        zIndex: 9999
      };
    }
    const M = N(() => w.value ? n.variant === "date-time" ? w.value.format("YYYY-MM-DD HH:mm") : w.value.format("YYYY-MM-DD") : ""), b = N(() => n.variant !== "date-range" ? "" : f.value.start && f.value.end ? `${f.value.start.format(
      "YYYY-MM-DD"
    )} to ${f.value.end.format("YYYY-MM-DD")}` : f.value.start ? `${f.value.start.format("YYYY-MM-DD")} to ...` : ""), C = N(() => `fu-date-picker--${n.variant}`);
    fe(
      () => n.modelValue,
      (h) => {
        if (n.variant !== "date-range") {
          if (typeof h == "string" && h) {
            const F = se(h);
            if (F.isValid()) {
              w.value = F, c.value = F.startOf("month"), n.variant === "date-time" ? m.value = F.format("h:mm A") : m.value = "00:00";
              return;
            }
          }
          (h === null || h === "") && (w.value = null, f.value = { start: null, end: null }, m.value = "00:00");
        }
      },
      { immediate: !0 }
    );
    function B() {
      r.value === "days" ? r.value = "months" : r.value === "months" ? r.value = "years" : r.value = "days";
    }
    function T() {
      r.value === "days" ? c.value = c.value.subtract(1, "month") : r.value === "months" ? c.value = c.value.subtract(1, "year") : c.value = c.value.subtract(Be, "year");
    }
    function A() {
      r.value === "days" ? c.value = c.value.add(1, "month") : r.value === "months" ? c.value = c.value.add(1, "year") : c.value = c.value.add(Be, "year");
    }
    function K(h) {
      c.value = c.value.month(h), r.value = "days";
    }
    function ee(h) {
      c.value = c.value.year(h), r.value = "months";
    }
    function re() {
      const h = se();
      n.variant === "date-range" ? f.value = { start: h.clone(), end: h.clone() } : (w.value = h.clone(), l(
        "update:modelValue",
        n.variant === "date-time" ? h.format("YYYY-MM-DDTHH:mm") : h.format("YYYY-MM-DD")
      ), R());
    }
    function de() {
      const h = se().add(1, "day");
      n.variant === "date-range" ? f.value = { start: h.clone(), end: h.clone() } : (w.value = h.clone(), l(
        "update:modelValue",
        n.variant === "date-time" ? h.format("YYYY-MM-DDTHH:mm") : h.format("YYYY-MM-DD")
      ), R());
    }
    function pe() {
      w.value = null, f.value = { start: null, end: null }, l(
        "update:modelValue",
        n.variant === "date-range" ? { start: null, end: null } : null
      ), R();
    }
    const ye = N(() => {
      const h = [];
      for (let F = 0; F < 24; F++)
        for (let L = 0; L < 60; L += 15)
          h.push(se().hour(F).minute(L).format("h:mm A"));
      return h;
    }), Ce = N(() => {
      if (!m.value) return ye.value;
      const h = m.value.toLowerCase().replace(/\s+/g, "");
      return ye.value.filter(
        (F) => F.toLowerCase().replace(/\s+/g, "").startsWith(h)
      );
    });
    function be() {
      if (!w.value || !m.value) return;
      const h = String(m.value).trim().toLowerCase(), F = se(
        h,
        ["h:mm a", "h:mma", "ha", "h a", "hh:mm a", "H:mm", "HH:mm", "H"],
        !0
      );
      if (!F.isValid()) {
        s.value = !1;
        return;
      }
      w.value = w.value.hour(F.hour()).minute(F.minute()), m.value = w.value.format("h:mm A"), l("update:modelValue", w.value.format("YYYY-MM-DDTHH:mm")), s.value = !1;
    }
    function ze(h) {
      m.value = h, be();
    }
    function Me() {
      setTimeout(() => {
        be(), s.value = !1;
      }, 120);
    }
    return _e(() => {
      window.removeEventListener("resize", $), window.removeEventListener("click", E);
    }), (h, F) => (t(), o("div", {
      class: H(["fu-date-picker", C.value]),
      ref_key: "pickerRef",
      ref: d,
      style: oe({ width: e.formWrapperWidth })
    }, [
      e.variant !== "date-range" ? (t(), X(xe, {
        key: 0,
        type: "text",
        modelValue: M.value,
        placeholder: "Select date",
        readonly: "",
        onClick: q,
        formWrapperWidth: e.formWrapperWidth,
        size: e.size,
        error: e.error,
        required: e.required,
        label: e.label,
        disabled: e.disabled
      }, {
        right: ae(() => [
          P(Z($e))
        ]),
        _: 1
      }, 8, ["modelValue", "formWrapperWidth", "size", "error", "required", "label", "disabled"])) : (t(), X(xe, {
        key: 1,
        type: "text",
        modelValue: b.value,
        placeholder: "Select date range",
        onClick: q,
        readonly: "",
        formWrapperWidth: e.formWrapperWidth,
        size: e.size,
        error: e.error,
        required: e.required,
        disabled: e.disabled
      }, {
        right: ae(() => [
          P(Z($e))
        ]),
        _: 1
      }, 8, ["modelValue", "formWrapperWidth", "size", "error", "required", "disabled"])),
      (t(), X(ge, { to: "body" }, [
        i.value ? (t(), o("div", {
          key: 0,
          class: "fu-date-picker__calendar-overlay",
          onClick: ue(R, ["self"])
        }, [
          a("div", {
            class: "fu-date-picker__calendar",
            style: oe(Q.value),
            ref_key: "calendarRef",
            ref: _,
            onClick: F[2] || (F[2] = ue(() => {
            }, ["stop"]))
          }, [
            a("div", na, [
              P(we, {
                variant: "ghost",
                size: "sm",
                icon: Z($e),
                onClick: B
              }, {
                default: ae(() => [
                  r.value === "days" ? (t(), o(U, { key: 0 }, [
                    ie(S(c.value.format("MMMM YYYY")), 1)
                  ], 64)) : r.value === "months" ? (t(), o(U, { key: 1 }, [
                    ie(S(c.value.year()), 1)
                  ], 64)) : (t(), o(U, { key: 2 }, [
                    ie(S(p.value) + " - " + S(O.value), 1)
                  ], 64))
                ]),
                _: 1
              }, 8, ["icon"]),
              a("div", oa, [
                P(ke, {
                  icon: Z(Pe),
                  size: "sm",
                  onClick: T
                }, null, 8, ["icon"]),
                P(ke, {
                  icon: Z(We),
                  size: "sm",
                  onClick: A
                }, null, 8, ["icon"])
              ])
            ]),
            r.value === "days" ? (t(), o("div", aa, [
              a("div", la, [
                (t(), o(U, null, ne(g, (L) => a("div", {
                  key: L,
                  class: "calendar-weekday"
                }, S(L), 1)), 64))
              ]),
              a("div", sa, [
                (t(!0), o(U, null, ne(D.value, (L) => (t(), o("div", {
                  key: L.date.toString(),
                  class: H(["calendar-day", {
                    "calendar-day--other-month": !L.isCurrentMonth,
                    "calendar-day--selected": x(L.date),
                    "calendar-day--in-range": I(L.date),
                    "calendar-day--disabled": V(L.date)
                  }]),
                  onClick: (le) => Y(L.date)
                }, S(L.date.date()), 11, ia))), 128))
              ])
            ])) : r.value === "months" ? (t(), o("div", ua, [
              (t(), o(U, null, ne(k, (L, le) => a("div", {
                key: L,
                class: H(["calendar-month", { "calendar-month--selected": le === c.value.month() }]),
                onClick: (Se) => K(le)
              }, S(L), 11, ra)), 64))
            ])) : (t(), o("div", da, [
              (t(!0), o(U, null, ne(j.value, (L) => (t(), o("div", {
                key: L,
                class: H(["calendar-year", { "calendar-year--selected": L === c.value.year() }]),
                onClick: (le) => ee(L)
              }, S(L), 11, ca))), 128))
            ])),
            F[7] || (F[7] = a("hr", null, null, -1)),
            a("div", fa, [
              e.variant !== "date-range" ? (t(), o("div", va, [
                P(we, {
                  variant: "outline",
                  onClick: re
                }, {
                  default: ae(() => [...F[3] || (F[3] = [
                    ie("Today", -1)
                  ])]),
                  _: 1
                }),
                P(we, {
                  variant: "outline",
                  onClick: de
                }, {
                  default: ae(() => [...F[4] || (F[4] = [
                    ie("Tomorrow", -1)
                  ])]),
                  _: 1
                })
              ])) : z("", !0),
              e.variant === "date-range" ? (t(), o("div", ma, [
                P(we, {
                  variant: "outline",
                  onClick: W
                }, {
                  default: ae(() => [...F[5] || (F[5] = [
                    ie("Apply", -1)
                  ])]),
                  _: 1
                })
              ])) : z("", !0),
              P(we, {
                variant: "outline",
                onClick: pe
              }, {
                default: ae(() => [...F[6] || (F[6] = [
                  ie("Clear", -1)
                ])]),
                _: 1
              })
            ]),
            e.variant === "date-time" ? (t(), o("div", pa, [
              a("div", ha, [
                P(xe, {
                  type: "text",
                  modelValue: m.value,
                  "onUpdate:modelValue": F[0] || (F[0] = (L) => m.value = L),
                  placeholder: "HH:mm or 4:30pm",
                  onFocus: F[1] || (F[1] = (L) => s.value = !0),
                  onKeydown: De(ue(be, ["prevent"]), ["enter"]),
                  onBlur: Me,
                  formWrapperWidth: "100%"
                }, {
                  right: ae(() => [
                    P(Z($e))
                  ]),
                  _: 1
                }, 8, ["modelValue", "onKeydown"]),
                s.value ? (t(), o("div", _a, [
                  (t(!0), o(U, null, ne(Ce.value, (L) => (t(), o("div", {
                    key: L,
                    class: "fu-time-option",
                    onMousedown: ue((le) => ze(L), ["prevent"])
                  }, S(L), 41, ya))), 128))
                ])) : z("", !0)
              ])
            ])) : z("", !0)
          ], 4)
        ])) : z("", !0)
      ]))
    ], 6));
  }
}, ba = /* @__PURE__ */ G(ga, [["__scopeId", "data-v-41bea5ec"]]), wa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: ba
}, Symbol.toStringTag, { value: "Module" })), ka = { class: "calendar-header" }, $a = { class: "flex flex--gap-sm" }, Ma = { key: 0 }, Ca = { class: "calendar-weekdays" }, Sa = { class: "calendar-days" }, xa = ["onClick"], za = {
  key: 1,
  class: "calendar-months"
}, Oa = ["onClick"], Da = {
  key: 2,
  class: "calendar-years"
}, Ya = ["onClick"], Va = { class: "flex flex--space flex--gap-md px-2 pb-2" }, Ta = {
  key: 0,
  class: "flex flex--gap-sm"
}, Fa = { key: 1 }, Ba = {
  key: 3,
  class: "calendar-time"
}, Ia = { class: "fu-time-input-wrapper" }, Ea = {
  key: 0,
  class: "fu-time-dropdown customScrollBar"
}, La = ["onMousedown"], Ie = 12, Aa = {
  __name: "datePickerBackup",
  props: {
    modelValue: [String, Object],
    variant: {
      type: String,
      default: "date",
      validator: (e) => ["date", "date-time", "date-range"].includes(e)
    },
    min: String,
    max: String,
    // New props
    formWrapperWidth: String,
    size: {
      type: String,
      default: "md",
      validator: (e) => ["sm", "md", "lg"].includes(e)
    },
    error: {
      type: [String, null],
      default: null
    },
    label: {
      type: String,
      default: null
    },
    required: {
      type: Boolean,
      default: !1
    },
    disabled: {
      type: Boolean,
      default: !1
    }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    se.extend(Ct);
    const n = e, s = y(!1), l = v, i = y(!1), u = y(null), d = y(null), _ = y(null), c = y(se().startOf("month")), w = y(null), f = y({ start: null, end: null }), m = y("00:00"), g = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"], k = [
      "Jan",
      "Feb",
      "Mar",
      "Apr",
      "May",
      "Jun",
      "Jul",
      "Aug",
      "Sep",
      "Oct",
      "Nov",
      "Dec"
    ], r = y("days"), p = N(
      () => Math.floor(c.value.year() / Ie) * Ie
    ), O = N(() => p.value + Ie - 1), j = N(
      () => Array.from({ length: Ie }, (h, F) => p.value + F)
    ), D = N(() => {
      const h = c.value.startOf("month").startOf("week"), F = c.value.endOf("month").endOf("week"), L = [];
      let le = h.clone();
      for (; le.isBefore(F) || le.isSame(F, "day"); )
        L.push({
          date: le.clone(),
          isCurrentMonth: le.month() === c.value.month()
        }), le = le.add(1, "day");
      return L;
    });
    function V(h) {
      return !!(n.min && h.isBefore(se(n.min), "day") || n.max && h.isAfter(se(n.max), "day"));
    }
    function x(h) {
      return n.variant === "date-range" ? f.value.start && h.isSame(f.value.start, "day") || f.value.end && h.isSame(f.value.end, "day") : w.value && h.isSame(w.value, "day");
    }
    function I(h) {
      return n.variant === "date-range" && f.value.start && f.value.end && h.isAfter(f.value.start, "day") && h.isBefore(f.value.end, "day");
    }
    function Y(h) {
      if (!V(h)) {
        if (n.variant === "date-range") {
          !f.value.start || u.value === "start" ? (f.value.start = h.clone(), f.value.end = null, u.value = "end") : h.isBefore(f.value.start, "day") ? (f.value.end = f.value.start.clone(), f.value.start = h.clone()) : f.value.end = h.clone();
          return;
        }
        w.value = h.clone(), l(
          "update:modelValue",
          n.variant === "date-time" ? w.value.format("YYYY-MM-DDTHH:mm") : w.value.format("YYYY-MM-DD")
        ), R();
      }
    }
    function W() {
      f.value.start && f.value.end && (l("update:modelValue", {
        start: f.value.start.format("YYYY-MM-DD"),
        end: f.value.end.format("YYYY-MM-DD")
      }), R());
    }
    function q(h = null) {
      u.value = h, i.value = !0, n.variant === "date-range" ? f.value.start ? c.value = f.value.start.startOf("month") : c.value = se().startOf("month") : w.value ? c.value = w.value.startOf("month") : c.value = se().startOf("month"), ve(() => {
        $(), window.addEventListener("resize", $), window.addEventListener("click", E);
      });
    }
    function R() {
      i.value = !1, window.removeEventListener("resize", $), window.removeEventListener("click", E);
    }
    function E(h) {
      !d.value?.contains(h.target) && !_.value?.contains(h.target) && R();
    }
    const Q = y({
      position: "absolute",
      top: "0px",
      left: "0px",
      zIndex: 9999
    });
    function $() {
      if (!d.value || !_.value) return;
      const h = d.value.getBoundingClientRect(), F = _.value.getBoundingClientRect(), L = window.innerHeight - h.bottom, le = h.top, Se = window.scrollY || window.pageYOffset, Ve = window.scrollX || window.pageXOffset;
      let Oe;
      L < F.height && le > F.height ? Oe = h.top + Se - F.height - 6 : Oe = h.bottom + Se + 6, Q.value = {
        position: "absolute",
        top: `${Oe}px`,
        left: `${h.left + Ve}px`,
        zIndex: 9999
      };
    }
    const M = N(() => w.value ? n.variant === "date-time" ? w.value.format("YYYY-MM-DD HH:mm") : w.value.format("YYYY-MM-DD") : ""), b = N(() => n.variant !== "date-range" ? "" : f.value.start && f.value.end ? `${f.value.start.format(
      "YYYY-MM-DD"
    )} to ${f.value.end.format("YYYY-MM-DD")}` : f.value.start ? `${f.value.start.format("YYYY-MM-DD")} to ...` : ""), C = N(() => `fu-date-picker--${n.variant}`);
    fe(
      () => n.modelValue,
      (h) => {
        if (n.variant !== "date-range") {
          if (typeof h == "string" && h) {
            const F = se(h);
            if (F.isValid()) {
              w.value = F, c.value = F.startOf("month"), n.variant === "date-time" ? m.value = F.format("h:mm A") : m.value = "00:00";
              return;
            }
          }
          (h === null || h === "") && (w.value = null, f.value = { start: null, end: null }, m.value = "00:00");
        }
      },
      { immediate: !0 }
    );
    function B() {
      r.value === "days" ? r.value = "months" : r.value === "months" ? r.value = "years" : r.value = "days";
    }
    function T() {
      r.value === "days" ? c.value = c.value.subtract(1, "month") : r.value === "months" ? c.value = c.value.subtract(1, "year") : c.value = c.value.subtract(Ie, "year");
    }
    function A() {
      r.value === "days" ? c.value = c.value.add(1, "month") : r.value === "months" ? c.value = c.value.add(1, "year") : c.value = c.value.add(Ie, "year");
    }
    function K(h) {
      c.value = c.value.month(h), r.value = "days";
    }
    function ee(h) {
      c.value = c.value.year(h), r.value = "months";
    }
    function re() {
      const h = se();
      n.variant === "date-range" ? f.value = { start: h.clone(), end: h.clone() } : (w.value = h.clone(), l(
        "update:modelValue",
        n.variant === "date-time" ? h.format("YYYY-MM-DDTHH:mm") : h.format("YYYY-MM-DD")
      ), R());
    }
    function de() {
      const h = se().add(1, "day");
      n.variant === "date-range" ? f.value = { start: h.clone(), end: h.clone() } : (w.value = h.clone(), l(
        "update:modelValue",
        n.variant === "date-time" ? h.format("YYYY-MM-DDTHH:mm") : h.format("YYYY-MM-DD")
      ), R());
    }
    function pe() {
      w.value = null, f.value = { start: null, end: null }, l(
        "update:modelValue",
        n.variant === "date-range" ? { start: null, end: null } : null
      ), R();
    }
    const ye = N(() => {
      const h = [];
      for (let F = 0; F < 24; F++)
        for (let L = 0; L < 60; L += 15)
          h.push(se().hour(F).minute(L).format("h:mm A"));
      return h;
    }), Ce = N(() => {
      if (!m.value) return ye.value;
      const h = m.value.toLowerCase().replace(/\s+/g, "");
      return ye.value.filter(
        (F) => F.toLowerCase().replace(/\s+/g, "").startsWith(h)
      );
    });
    function be() {
      if (!w.value || !m.value) return;
      const h = String(m.value).trim().toLowerCase(), F = se(
        h,
        ["h:mm a", "h:mma", "ha", "h a", "hh:mm a", "H:mm", "HH:mm", "H"],
        !0
      );
      if (!F.isValid()) {
        s.value = !1;
        return;
      }
      w.value = w.value.hour(F.hour()).minute(F.minute()), m.value = w.value.format("h:mm A"), l("update:modelValue", w.value.format("YYYY-MM-DDTHH:mm")), s.value = !1;
    }
    function ze(h) {
      m.value = h, be();
    }
    function Me() {
      setTimeout(() => {
        be(), s.value = !1;
      }, 120);
    }
    return _e(() => {
      window.removeEventListener("resize", $), window.removeEventListener("click", E);
    }), (h, F) => (t(), o("div", {
      class: H(["fu-date-picker", C.value]),
      ref_key: "pickerRef",
      ref: d,
      style: oe({ width: e.formWrapperWidth })
    }, [
      e.variant !== "date-range" ? (t(), X(xe, {
        key: 0,
        type: "text",
        modelValue: M.value,
        placeholder: "Select date",
        readonly: "",
        onClick: q,
        formWrapperWidth: e.formWrapperWidth,
        size: e.size,
        error: e.error,
        required: e.required,
        label: e.label,
        disabled: e.disabled
      }, {
        right: ae(() => [
          P(Z($e))
        ]),
        _: 1
      }, 8, ["modelValue", "formWrapperWidth", "size", "error", "required", "label", "disabled"])) : (t(), X(xe, {
        key: 1,
        type: "text",
        modelValue: b.value,
        placeholder: "Select date range",
        onClick: q,
        readonly: "",
        formWrapperWidth: e.formWrapperWidth,
        size: e.size,
        error: e.error,
        required: e.required,
        disabled: e.disabled
      }, {
        right: ae(() => [
          P(Z($e))
        ]),
        _: 1
      }, 8, ["modelValue", "formWrapperWidth", "size", "error", "required", "disabled"])),
      (t(), X(ge, { to: "body" }, [
        i.value ? (t(), o("div", {
          key: 0,
          class: "fu-date-picker__calendar-overlay",
          onClick: ue(R, ["self"])
        }, [
          a("div", {
            class: "fu-date-picker__calendar",
            style: oe(Q.value),
            ref_key: "calendarRef",
            ref: _,
            onClick: F[2] || (F[2] = ue(() => {
            }, ["stop"]))
          }, [
            a("div", ka, [
              P(we, {
                variant: "ghost",
                size: "sm",
                onClick: B
              }, {
                default: ae(() => [
                  r.value === "days" ? (t(), o(U, { key: 0 }, [
                    ie(S(c.value.format("MMMM YYYY")), 1)
                  ], 64)) : r.value === "months" ? (t(), o(U, { key: 1 }, [
                    ie(S(c.value.year()), 1)
                  ], 64)) : (t(), o(U, { key: 2 }, [
                    ie(S(p.value) + " - " + S(O.value), 1)
                  ], 64))
                ]),
                _: 1
              }),
              a("div", $a, [
                P(ke, {
                  icon: Z(Pe),
                  size: "sm",
                  onClick: T
                }, null, 8, ["icon"]),
                P(ke, {
                  icon: Z(We),
                  size: "sm",
                  onClick: A
                }, null, 8, ["icon"])
              ])
            ]),
            r.value === "days" ? (t(), o("div", Ma, [
              a("div", Ca, [
                (t(), o(U, null, ne(g, (L) => a("div", {
                  key: L,
                  class: "calendar-weekday"
                }, S(L), 1)), 64))
              ]),
              a("div", Sa, [
                (t(!0), o(U, null, ne(D.value, (L) => (t(), o("div", {
                  key: L.date.toString(),
                  class: H(["calendar-day", {
                    "calendar-day--other-month": !L.isCurrentMonth,
                    "calendar-day--selected": x(L.date),
                    "calendar-day--in-range": I(L.date),
                    "calendar-day--disabled": V(L.date)
                  }]),
                  onClick: (le) => Y(L.date)
                }, S(L.date.date()), 11, xa))), 128))
              ])
            ])) : r.value === "months" ? (t(), o("div", za, [
              (t(), o(U, null, ne(k, (L, le) => a("div", {
                key: L,
                class: H(["calendar-month", { "calendar-month--selected": le === c.value.month() }]),
                onClick: (Se) => K(le)
              }, S(L), 11, Oa)), 64))
            ])) : (t(), o("div", Da, [
              (t(!0), o(U, null, ne(j.value, (L) => (t(), o("div", {
                key: L,
                class: H(["calendar-year", { "calendar-year--selected": L === c.value.year() }]),
                onClick: (le) => ee(L)
              }, S(L), 11, Ya))), 128))
            ])),
            F[7] || (F[7] = a("hr", null, null, -1)),
            a("div", Va, [
              e.variant !== "date-range" ? (t(), o("div", Ta, [
                P(we, {
                  variant: "outline",
                  onClick: re
                }, {
                  default: ae(() => [...F[3] || (F[3] = [
                    ie("Today", -1)
                  ])]),
                  _: 1
                }),
                P(we, {
                  variant: "outline",
                  onClick: de
                }, {
                  default: ae(() => [...F[4] || (F[4] = [
                    ie("Tomorrow", -1)
                  ])]),
                  _: 1
                })
              ])) : z("", !0),
              e.variant === "date-range" ? (t(), o("div", Fa, [
                P(we, {
                  variant: "outline",
                  onClick: W
                }, {
                  default: ae(() => [...F[5] || (F[5] = [
                    ie("Apply", -1)
                  ])]),
                  _: 1
                })
              ])) : z("", !0),
              P(we, {
                variant: "outline",
                onClick: pe
              }, {
                default: ae(() => [...F[6] || (F[6] = [
                  ie("Clear", -1)
                ])]),
                _: 1
              })
            ]),
            e.variant === "date-time" ? (t(), o("div", Ba, [
              a("div", Ia, [
                P(xe, {
                  type: "text",
                  modelValue: m.value,
                  "onUpdate:modelValue": F[0] || (F[0] = (L) => m.value = L),
                  placeholder: "HH:mm or 4:30pm",
                  onFocus: F[1] || (F[1] = (L) => s.value = !0),
                  onKeydown: De(ue(be, ["prevent"]), ["enter"]),
                  onBlur: Me,
                  formWrapperWidth: "100%"
                }, {
                  right: ae(() => [
                    P(Z($e))
                  ]),
                  _: 1
                }, 8, ["modelValue", "onKeydown"]),
                s.value ? (t(), o("div", Ea, [
                  (t(!0), o(U, null, ne(Ce.value, (L) => (t(), o("div", {
                    key: L,
                    class: "fu-time-option",
                    onMousedown: ue((le) => ze(L), ["prevent"])
                  }, S(L), 41, La))), 128))
                ])) : z("", !0)
              ])
            ])) : z("", !0)
          ], 4)
        ])) : z("", !0)
      ]))
    ], 6));
  }
}, ja = /* @__PURE__ */ G(Aa, [["__scopeId", "data-v-7bb6d1f9"]]), Pa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: ja
}, Symbol.toStringTag, { value: "Module" })), Wa = { class: "calendar-header" }, Ra = { class: "flex flex--gap-sm" }, Ha = {
  key: 0,
  class: "calendar-months"
}, qa = ["onClick"], Na = {
  key: 1,
  class: "calendar-years"
}, Ua = ["onClick"], Xa = { class: "flex flex--space flex--gap-md px-2 pb-2" }, Re = 12, Ka = {
  __name: "FusionMonthPicker",
  props: {
    modelValue: String,
    // "YYYY-MM"
    min: String,
    // "YYYY-MM"
    max: String,
    // "YYYY-MM"
    formWrapperWidth: String,
    size: {
      type: String,
      default: "md",
      validator: (e) => ["sm", "md", "lg"].includes(e)
    },
    error: {
      type: [String, null],
      default: null
    },
    required: {
      type: Boolean,
      default: !1
    },
    disabled: {
      type: Boolean,
      default: !1
    },
    label: String
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(!1), i = y(null), u = y(null), d = y("months"), _ = y(se().year()), c = [
      "Jan",
      "Feb",
      "Mar",
      "Apr",
      "May",
      "Jun",
      "Jul",
      "Aug",
      "Sep",
      "Oct",
      "Nov",
      "Dec"
    ], w = N(
      () => _.value - _.value % Re
    ), f = N(() => w.value + Re - 1), m = N(() => {
      const $ = w.value;
      return Array.from({ length: Re }, (M, b) => $ + b);
    }), g = N(() => {
      if (!n.modelValue) return null;
      const $ = se(n.modelValue, "YYYY-MM", !0);
      return $.isValid() ? $ : null;
    });
    function k($) {
      return g.value ? g.value.month() === $ && g.value.year() === _.value : !1;
    }
    function r($) {
      const M = se(
        `${_.value}-${($ + 1).toString().padStart(2, "0")}`,
        "YYYY-MM"
      );
      return !!(n.min && M.isBefore(se(n.min, "YYYY-MM"), "month") || n.max && M.isAfter(se(n.max, "YYYY-MM"), "month"));
    }
    function p($) {
      _.value = $, d.value = "months";
    }
    function O($) {
      if (r($)) return;
      const M = se(
        `${_.value}-${($ + 1).toString().padStart(2, "0")}`,
        "YYYY-MM"
      );
      s("update:modelValue", M.format("MMM, YYYY")), l.value = !1;
    }
    function j() {
      const $ = se();
      _.value = $.year(), s("update:modelValue", $.format("MMM, YYYY")), l.value = !1;
    }
    function D() {
      n.disabled || (l.value = !l.value, l.value ? (g.value && (_.value = g.value.year()), ve(() => {
        Y(), window.addEventListener("resize", Y), window.addEventListener("click", x);
      })) : (window.removeEventListener("resize", Y), window.removeEventListener("click", x)));
    }
    function V() {
      l.value = !1, window.removeEventListener("resize", Y), window.removeEventListener("click", x);
    }
    function x($) {
      !i.value?.contains($.target) && !u.value?.contains($.target) && V();
    }
    const I = y({
      position: "absolute",
      top: "0px",
      left: "0px",
      zIndex: 9999
    });
    function Y() {
      if (!i.value || !u.value) return;
      const $ = i.value.getBoundingClientRect(), M = u.value.getBoundingClientRect(), b = window.innerHeight - $.bottom, C = $.top, B = window.scrollY || window.pageYOffset, T = window.scrollX || window.pageXOffset;
      let A;
      b < M.height && C > M.height ? A = $.top + B - M.height - 6 : A = $.bottom + B + 6, I.value = {
        position: "absolute",
        top: `${A}px`,
        left: `${$.left + T}px`,
        zIndex: 9999
      };
    }
    function W() {
      d.value = d.value === "months" ? "years" : "months";
    }
    function q() {
      d.value === "months" ? _.value-- : _.value = Math.max(w.value - Re, 0);
    }
    function R() {
      d.value === "months" ? _.value++ : _.value = f.value + 1;
    }
    function E() {
      s("update:modelValue", null), l.value = !1;
    }
    const Q = N(() => n.modelValue || "");
    return _e(() => {
      window.removeEventListener("resize", Y), window.removeEventListener("click", x);
    }), ($, M) => (t(), o("div", {
      class: "fu-month-picker",
      ref_key: "pickerRef",
      ref: i,
      style: oe({ width: e.formWrapperWidth })
    }, [
      P(xe, {
        type: "text",
        modelValue: Q.value,
        placeholder: "Select month",
        readonly: "",
        onClick: [
          D,
          ue(V, ["self"])
        ],
        formWrapperWidth: e.formWrapperWidth,
        size: e.size,
        error: e.error,
        required: e.required,
        label: e.label,
        disabled: e.disabled
      }, {
        right: ae(() => [
          P(Z($e))
        ]),
        _: 1
      }, 8, ["modelValue", "formWrapperWidth", "size", "error", "required", "label", "disabled"]),
      (t(), X(ge, { to: "body" }, [
        l.value ? (t(), o("div", {
          key: 0,
          class: "fu-month-picker__calendar-overlay",
          onClick: ue(V, ["self"])
        }, [
          a("div", {
            class: "fu-month-picker__calendar",
            style: oe(I.value),
            ref_key: "calendarRef",
            ref: u,
            onClick: M[0] || (M[0] = ue(() => {
            }, ["stop"]))
          }, [
            a("div", Wa, [
              P(we, {
                variant: "ghost",
                size: "sm",
                onClick: W,
                icon: Z($e)
              }, {
                default: ae(() => [
                  d.value === "months" ? (t(), o(U, { key: 0 }, [
                    ie(S(_.value), 1)
                  ], 64)) : (t(), o(U, { key: 1 }, [
                    ie(S(w.value) + " - " + S(f.value), 1)
                  ], 64))
                ]),
                _: 1
              }, 8, ["icon"]),
              a("div", Ra, [
                P(ke, {
                  icon: Z(Pe),
                  size: "sm",
                  onClick: q
                }, null, 8, ["icon"]),
                P(ke, {
                  icon: Z(We),
                  size: "sm",
                  onClick: R
                }, null, 8, ["icon"])
              ])
            ]),
            d.value === "months" ? (t(), o("div", Ha, [
              (t(), o(U, null, ne(c, (b, C) => a("div", {
                key: b,
                class: H(["calendar-month", {
                  "calendar-month--selected": k(C),
                  "calendar-month--disabled": r(C)
                }]),
                onClick: (B) => O(C)
              }, S(b), 11, qa)), 64))
            ])) : (t(), o("div", Na, [
              (t(!0), o(U, null, ne(m.value, (b) => (t(), o("div", {
                key: b,
                class: H(["calendar-year", { "calendar-year--selected": b === _.value }]),
                onClick: (C) => p(b)
              }, S(b), 11, Ua))), 128))
            ])),
            M[3] || (M[3] = a("hr", null, null, -1)),
            a("div", Xa, [
              P(we, {
                variant: "outline",
                onClick: j
              }, {
                default: ae(() => [...M[1] || (M[1] = [
                  ie(" This Month ", -1)
                ])]),
                _: 1
              }),
              P(we, {
                variant: "outline",
                onClick: E
              }, {
                default: ae(() => [...M[2] || (M[2] = [
                  ie(" Clear ", -1)
                ])]),
                _: 1
              })
            ])
          ], 4)
        ])) : z("", !0)
      ]))
    ], 4));
  }
}, Ja = /* @__PURE__ */ G(Ka, [["__scopeId", "data-v-7377986b"]]), Za = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Ja
}, Symbol.toStringTag, { value: "Module" })), Ga = { class: "calendar-header" }, Qa = { class: "flex flex--gap-sm" }, el = { key: 0 }, tl = { class: "calendar-weekdays" }, nl = { class: "calendar-days" }, ol = ["onClick"], al = {
  key: 1,
  class: "calendar-months"
}, ll = ["onClick"], sl = {
  key: 2,
  class: "calendar-years"
}, il = ["onClick"], ul = {
  key: 3,
  class: "calendar-time"
}, rl = {
  key: 0,
  class: "fu-time-dropdown customScrollBar"
}, dl = ["onMousedown"], He = 12, cl = {
  __name: "FusionPlainDatePicker",
  props: {
    modelValue: String,
    variant: { type: String, default: "date" },
    // "date" or "date-time"
    formWrapperWidth: String
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(se().startOf("month")), i = y(null), u = y("00:00"), d = y("days"), _ = y(!1), c = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"], w = [
      "Jan",
      "Feb",
      "Mar",
      "Apr",
      "May",
      "Jun",
      "Jul",
      "Aug",
      "Sep",
      "Oct",
      "Nov",
      "Dec"
    ], f = N(
      () => Math.floor(l.value.year() / He) * He
    ), m = N(() => f.value + He - 1), g = N(
      () => Array.from({ length: He }, (E, Q) => f.value + Q)
    ), k = N(() => {
      const E = l.value.startOf("month").startOf("week"), Q = l.value.endOf("month").endOf("week"), $ = [];
      let M = E.clone();
      for (; M.isBefore(Q) || M.isSame(Q, "day"); )
        $.push({
          date: M.clone(),
          isCurrentMonth: M.month() === l.value.month()
        }), M = M.add(1, "day");
      return $;
    }), r = N(() => {
      const E = [];
      for (let Q = 0; Q < 24; Q++)
        for (let $ = 0; $ < 60; $ += 15)
          E.push(se().hour(Q).minute($).format("h:mm A"));
      return E;
    }), p = N(() => r.value), O = () => !1, j = (E) => i.value && E.isSame(i.value, "day");
    function D(E) {
      if (i.value = E.clone(), n.variant === "date-time") {
        const Q = se(
          `${i.value.format("YYYY-MM-DD")} ${u.value}`,
          "YYYY-MM-DD HH:mm"
        );
        s("update:modelValue", Q.format("YYYY-MM-DDTHH:mm"));
      } else
        s("update:modelValue", i.value.format("YYYY-MM-DD"));
    }
    const V = () => l.value = l.value.subtract(1, "month"), x = () => l.value = l.value.add(1, "month"), I = () => d.value = d.value === "days" ? "months" : d.value === "months" ? "years" : "days", Y = (E) => {
      l.value = l.value.month(E), d.value = "days";
    }, W = (E) => {
      l.value = l.value.year(E), d.value = "months";
    };
    function q() {
      if (!i.value) return;
      const E = se(`${i.value.format("YYYY-MM-DD")} ${u.value}`, [
        "YYYY-MM-DD HH:mm",
        "YYYY-MM-DD h:mm A"
      ]);
      E.isValid() && (u.value = E.format("HH:mm"), s("update:modelValue", E.format("YYYY-MM-DDTHH:mm")), _.value = !1);
    }
    function R(E) {
      if (!i.value) return;
      u.value = se(E, "h:mm A").format("HH:mm");
      const Q = se(
        `${i.value.format("YYYY-MM-DD")} ${u.value}`,
        "YYYY-MM-DD HH:mm"
      );
      s("update:modelValue", Q.format("YYYY-MM-DDTHH:mm")), _.value = !1;
    }
    return fe(
      () => n.modelValue,
      (E) => {
        if (!E) {
          i.value = null;
          return;
        }
        const Q = se(E);
        Q.isValid() && (i.value = Q, l.value = Q.startOf("month"), n.variant === "date-time" && (u.value = Q.format("HH:mm")));
      },
      { immediate: !0 }
    ), (E, Q) => (t(), o("div", {
      class: "fu-date-picker fu-date-picker--plain",
      style: oe({ width: e.formWrapperWidth })
    }, [
      a("div", {
        class: "fu-date-picker",
        style: oe({ width: e.formWrapperWidth })
      }, [
        a("div", Ga, [
          a("button", { onClick: I }, [
            d.value === "days" ? (t(), o(U, { key: 0 }, [
              ie(S(l.value.format("MMMM YYYY")), 1)
            ], 64)) : d.value === "months" ? (t(), o(U, { key: 1 }, [
              ie(S(l.value.year()), 1)
            ], 64)) : (t(), o(U, { key: 2 }, [
              ie(S(f.value) + " - " + S(m.value), 1)
            ], 64))
          ]),
          a("div", Qa, [
            a("button", { onClick: V }, [
              P(Z(Pe), {
                size: 16,
                color: "var(--fu-color-text)"
              })
            ]),
            a("button", { onClick: x }, [
              P(Z(We), { size: 16 })
            ])
          ])
        ]),
        d.value === "days" ? (t(), o("div", el, [
          a("div", tl, [
            (t(), o(U, null, ne(c, ($) => a("div", {
              key: $,
              class: "calendar-weekday"
            }, S($), 1)), 64))
          ]),
          a("div", nl, [
            (t(!0), o(U, null, ne(k.value, ($) => (t(), o("div", {
              key: $.date.toString(),
              class: H(["calendar-day", {
                "calendar-day--other-month": !$.isCurrentMonth,
                "calendar-day--selected": j($.date),
                "calendar-day--disabled": O($.date)
              }]),
              onClick: (M) => D($.date)
            }, S($.date.date()), 11, ol))), 128))
          ])
        ])) : d.value === "months" ? (t(), o("div", al, [
          (t(), o(U, null, ne(w, ($, M) => a("div", {
            key: $,
            class: H(["calendar-month", { "calendar-month--selected": M === l.value.month() }]),
            onClick: (b) => Y(M)
          }, S($), 11, ll)), 64))
        ])) : (t(), o("div", sl, [
          (t(!0), o(U, null, ne(g.value, ($) => (t(), o("div", {
            key: $,
            class: H(["calendar-year", { "calendar-year--selected": $ === l.value.year() }]),
            onClick: (M) => W($)
          }, S($), 11, il))), 128))
        ])),
        e.variant === "date-time" ? (t(), o("div", ul, [
          P(xe, {
            type: "text",
            modelValue: u.value,
            "onUpdate:modelValue": Q[0] || (Q[0] = ($) => u.value = $),
            placeholder: "HH:mm or 4:30pm",
            onFocus: Q[1] || (Q[1] = ($) => _.value = !0),
            onKeydown: De(ue(q, ["prevent"]), ["enter"]),
            formWrapperWidth: "100%"
          }, {
            right: ae(() => [
              P(Z($e))
            ]),
            _: 1
          }, 8, ["modelValue", "onKeydown"]),
          _.value ? (t(), o("div", rl, [
            (t(!0), o(U, null, ne(p.value, ($) => (t(), o("div", {
              key: $,
              class: "fu-time-option",
              onMousedown: ue((M) => R($), ["prevent"])
            }, S($), 41, dl))), 128))
          ])) : z("", !0)
        ])) : z("", !0)
      ], 4)
    ], 4));
  }
}, fl = /* @__PURE__ */ G(cl, [["__scopeId", "data-v-4161642e"]]), vl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: fl
}, Symbol.toStringTag, { value: "Module" })), ml = ["onMousedown"], ft = 240, pl = {
  __name: "FusionTimePicker",
  props: {
    modelValue: String,
    interval: { type: Number, default: 30 },
    error: String,
    label: String,
    required: Boolean,
    disabled: Boolean,
    size: String,
    formWrapperWidth: String
  },
  emits: ["update:modelValue", "change"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(n.modelValue || ""), i = y(!1), u = y(!1), d = y(null), _ = y(null), c = y({ left: 0, top: 0, bottom: 0, width: 0 }), w = y(null), f = N(() => {
      const I = [];
      for (let Y = 0; Y < 24; Y++)
        for (let W = 0; W < 60; W += n.interval) {
          const q = /* @__PURE__ */ new Date();
          q.setHours(Y, W, 0), I.push({
            label: q.toLocaleTimeString([], {
              hour: "numeric",
              minute: "2-digit",
              hour12: !0
            }),
            value: `${String(Y).padStart(2, "0")}:${String(W).padStart(2, "0")}`
          });
        }
      return I;
    }), m = N(
      () => l.value ? f.value.filter(
        (I) => I.label.toLowerCase().includes(l.value.toLowerCase())
      ) : f.value
    ), g = N(
      () => m.value.length ? m.value : f.value
    ), k = (I) => {
      let Y = I.target.value.toLowerCase();
      Y = Y.replace(/[^0-9:apm]/g, "");
      const W = I.target;
      if (/^\d{2}$/.test(Y)) {
        Y += ":", l.value = Y, ve(() => W.setSelectionRange(3, 3));
        return;
      }
      l.value = Y;
    }, r = (I) => {
      if (!I) return null;
      const Y = I.trim().toLowerCase().match(/^(\d{1,2})(?::(\d{2}))?\s?(am|pm)?$/);
      if (!Y) return null;
      let W = parseInt(Y[1], 10);
      const q = parseInt(Y[2] || "00", 10), R = Y[3];
      if (R === "pm" && W < 12 && (W += 12), R === "am" && W === 12 && (W = 0), W > 23 || q > 59) return null;
      const E = W % 12 || 12, Q = W < 12 ? "am" : "pm";
      return `${String(E).padStart(2, "0")}:${String(q).padStart(
        2,
        "0"
      )}${Q}`;
    }, p = () => {
      if (!d.value) return;
      const I = d.value.getBoundingClientRect();
      c.value = {
        left: I.left,
        top: I.top,
        bottom: I.bottom,
        width: I.width
      };
      const Y = window.innerHeight - I.bottom, W = I.top;
      u.value = Y < ft && W > Y;
    }, O = (I, Y) => {
      Y === 0 && I && (w.value = I);
    }, j = (I) => {
      l.value = I, s("update:modelValue", I), s("change", I), i.value = !1;
    }, D = () => {
      const I = r(l.value);
      I && (l.value = I, s("update:modelValue", I), s("change", I), i.value = !1);
    }, V = () => setTimeout(() => {
      D(), i.value = !1;
    }, 120);
    fe(
      () => n.modelValue,
      (I) => l.value = I || ""
    ), fe(l, async () => {
      await ve(), w.value && w.value.scrollIntoView({ behavior: "smooth", block: "nearest" });
    });
    const x = () => p();
    return he(() => {
      window.addEventListener("resize", x), ve(p);
    }), _e(() => window.removeEventListener("resize", x)), (I, Y) => (t(), o("div", {
      class: "fu-time-picker",
      ref_key: "inputRef",
      ref: d
    }, [
      P(xe, {
        type: "text",
        modelValue: l.value,
        "onUpdate:modelValue": Y[0] || (Y[0] = (W) => l.value = W),
        placeholder: "HH:mm or 4:30pm",
        onFocus: Y[1] || (Y[1] = (W) => {
          i.value = !0, p();
        }),
        onInput: k,
        onKeydown: De(ue(D, ["prevent"]), ["enter"]),
        onBlur: V,
        label: e.label,
        error: e.error,
        required: e.required,
        disabled: e.disabled,
        size: e.size,
        formWrapperWidth: e.formWrapperWidth
      }, {
        right: ae(() => [
          P(Z($e))
        ]),
        _: 1
      }, 8, ["modelValue", "onKeydown", "label", "error", "required", "disabled", "size", "formWrapperWidth"]),
      (t(), X(ge, { to: "body" }, [
        i.value ? (t(), o("div", {
          key: 0,
          ref_key: "dropdownRef",
          ref: _,
          class: H(["fu-time-dropdown customScrollBar", { "open-up": u.value }]),
          style: oe({
            left: c.value.left + "px",
            width: c.value.width + "px",
            top: u.value ? c.value.top - ft + "px" : c.value.bottom + "px"
          })
        }, [
          (t(!0), o(U, null, ne(g.value, (W, q) => (t(), o("div", {
            key: W.value,
            class: "fu-time-option",
            ref_for: !0,
            ref: (R) => O(R, q),
            onMousedown: ue((R) => j(W.value), ["prevent"])
          }, S(W.label), 41, ml))), 128))
        ], 6)) : z("", !0)
      ]))
    ], 512));
  }
}, hl = /* @__PURE__ */ G(pl, [["__scopeId", "data-v-de0d9c0d"]]), _l = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: hl
}, Symbol.toStringTag, { value: "Module" })), yl = {
  key: 0,
  class: "fu-drawer"
}, gl = { class: "fu-drawer__header-content" }, bl = {
  key: 0,
  class: "fu-drawer__header-actions"
}, wl = { class: "fu-drawer__body" }, kl = { class: "fu-drawer__footer" }, $l = /* @__PURE__ */ te({
  __name: "FusionDrawer",
  props: {
    open: { type: Boolean },
    title: {},
    position: { default: "right" },
    size: { default: "half" },
    showControls: { type: Boolean, default: !0 },
    hideHeaderBorder: { type: Boolean, default: !1 },
    canNext: { type: Boolean },
    canPrev: { type: Boolean }
  },
  emits: ["close", "next", "prev"],
  setup(e, { emit: v }) {
    const n = e, s = v;
    function l(i) {
      i.key === "Escape" && n.open && s("close");
    }
    return he(() => window.addEventListener("keydown", l)), _e(() => window.removeEventListener("keydown", l)), (i, u) => (t(), X(Ye, { name: "drawer-fade" }, {
      default: ae(() => [
        e.open ? (t(), o("div", yl, [
          a("div", {
            class: "fu-drawer__backdrop",
            onClick: u[0] || (u[0] = (d) => i.$emit("close"))
          }),
          a("div", {
            class: H(["fu-drawer__panel", [
              `fu-drawer__panel--${e.position}`,
              `fu-drawer__panel--${e.size}`,
              { "fu-drawer__panel--with-controls": e.showControls }
            ]])
          }, [
            i.$slots.header || e.showControls ? (t(), o("div", {
              key: 0,
              class: H(["fu-drawer__header", { "border-bottom-0": e.hideHeaderBorder }])
            }, [
              a("div", gl, [
                J(i.$slots, "header", {}, void 0, !0)
              ]),
              e.showControls ? (t(), o("div", bl, [
                P(ke, {
                  size: "sm",
                  variant: "subtle",
                  icon: Z(Ue),
                  onClick: u[1] || (u[1] = (d) => i.$emit("close"))
                }, null, 8, ["icon"])
              ])) : z("", !0)
            ], 2)) : z("", !0),
            a("div", wl, [
              J(i.$slots, "default", { class: "slot-body" }, void 0, !0)
            ]),
            a("div", {
              class: H(["fu-drawer__nav", [
                e.position === "right" ? "fu-drawer__nav--left" : "",
                e.position === "left" ? "fu-drawer__nav--right" : "",
                e.position === "bottom" ? "fu-drawer__nav--center" : ""
              ]])
            }, [
              e.canPrev ? (t(), X(ke, {
                key: 0,
                size: "sm",
                variant: "subtle",
                icon: Z(Lt),
                onClick: u[2] || (u[2] = (d) => i.$emit("prev"))
              }, null, 8, ["icon"])) : z("", !0),
              e.canNext ? (t(), X(ke, {
                key: 1,
                size: "sm",
                variant: "subtle",
                icon: Z($e),
                onClick: u[3] || (u[3] = (d) => i.$emit("next"))
              }, null, 8, ["icon"])) : z("", !0)
            ], 2),
            a("div", kl, [
              J(i.$slots, "footer", {}, void 0, !0)
            ])
          ], 2)
        ])) : z("", !0)
      ]),
      _: 3
    }));
  }
}), Ml = /* @__PURE__ */ G($l, [["__scopeId", "data-v-37927fd4"]]), Cl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Ml
}, Symbol.toStringTag, { value: "Module" })), Sl = ["onClick"], xl = /* @__PURE__ */ te({
  __name: "FusionDropdownButton",
  props: {
    buttonText: {},
    buttonIcon: {},
    actions: {},
    align: { default: "right" }
  },
  setup(e) {
    const v = e, n = y(!1), s = y(null), l = y(null), i = y({});
    let u = null;
    function d() {
      if (n.value = !n.value, n.value) {
        if (u && u !== s.value) {
          const m = new CustomEvent("close-other-dropdowns");
          document.dispatchEvent(m);
        }
        u = s.value, ve(_);
      }
    }
    function _() {
      const m = s.value?.querySelector("button");
      if (!m || !l.value) return;
      const g = m.getBoundingClientRect(), k = l.value.offsetWidth, r = {
        left: `${g.left}px`,
        top: `${g.bottom + 4}px`
      };
      v.align === "right" ? r.left = `${g.right - k}px` : v.align === "center" && (r.left = `${g.left + g.width / 2 - k / 2}px`), i.value = {
        position: "absolute",
        ...r,
        zIndex: "1000"
      };
    }
    function c(m) {
      m.onClick?.(), n.value = !1;
    }
    function w(m) {
      s.value && !s.value.contains(m.target) && l.value && !l.value.contains(m.target) && (n.value = !1);
    }
    function f() {
      n.value = !1;
    }
    return he(() => {
      document.addEventListener("click", w), document.addEventListener("close-other-dropdowns", f);
    }), _e(() => {
      document.removeEventListener("click", w), document.removeEventListener("close-other-dropdowns", f);
    }), (m, g) => (t(), o("div", {
      class: "fu-dropdown",
      ref_key: "dropdown",
      ref: s
    }, [
      a("button", {
        class: "fu-dropdown__button",
        onClick: d
      }, [
        ie(S(e.buttonText) + " ", 1),
        e.buttonIcon ? (t(), X(ce(e.buttonIcon), {
          key: 0,
          class: "fu-dropdown__icon"
        })) : z("", !0)
      ]),
      (t(), X(ge, { to: "body" }, [
        n.value ? (t(), o("ul", {
          key: 0,
          class: H(["fu-dropdown__menu", [`fu-dropdown__menu--${e.align}`, { show: n.value }]]),
          style: oe(i.value),
          ref_key: "menuRef",
          ref: l
        }, [
          (t(!0), o(U, null, ne(e.actions, (k) => (t(), o("li", {
            key: k.label
          }, [
            a("a", {
              class: "fu-dropdown__item",
              onClick: (r) => c(k)
            }, [
              k.icon ? (t(), X(ce(k.icon), {
                key: 0,
                class: "fu-dropdown__icon"
              })) : z("", !0),
              ie(" " + S(k.label), 1)
            ], 8, Sl)
          ]))), 128))
        ], 6)) : z("", !0)
      ]))
    ], 512));
  }
}), zl = /* @__PURE__ */ G(xl, [["__scopeId", "data-v-478aec9e"]]), Ol = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: zl
}, Symbol.toStringTag, { value: "Module" })), Dl = { class: "fu-dropdown-inline__wrapper" }, Yl = ["value", "placeholder"], Vl = ["onMousedown"], Tl = /* @__PURE__ */ te({
  __name: "FusionDropdownInline",
  props: {
    modelValue: {},
    options: {},
    placeholder: {},
    isOpen: { type: Boolean }
  },
  emits: ["update:modelValue", "open", "close"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(null), i = y(null), u = y(n.modelValue || null), d = y(!1), _ = y({});
    fe(
      () => n.modelValue,
      (g) => u.value = g
    ), fe(
      () => n.isOpen,
      (g) => {
        d.value = !!g, g && m();
      }
    );
    function c() {
      d.value || s("open"), d.value = !0, m();
    }
    function w(g) {
      u.value = g, s("update:modelValue", g), s("close"), d.value = !1, i.value?.blur();
    }
    function f(g) {
      l.value && !l.value.contains(g.target) && (d.value && s("close"), d.value = !1);
    }
    function m() {
      const g = l.value;
      if (!g) return;
      const k = g.getBoundingClientRect();
      _.value = {
        position: "absolute",
        top: `${k.bottom + 4}px`,
        left: `${k.left}px`,
        width: `${k.width}px`,
        zIndex: "9999"
      };
    }
    return he(() => {
      window.addEventListener("click", f), window.addEventListener("resize", m);
    }), _e(() => {
      window.removeEventListener("click", f), window.removeEventListener("resize", m);
    }), (g, k) => (t(), o("div", {
      class: "fu-dropdown-inline",
      ref_key: "inlineRef",
      ref: l
    }, [
      a("div", Dl, [
        u.value ? (t(), o("span", {
          key: 0,
          class: "fu-dropdown-inline__dot",
          style: oe({ backgroundColor: u.value.color })
        }, null, 4)) : z("", !0),
        a("input", {
          ref_key: "inputRef",
          ref: i,
          type: "text",
          class: "fu-dropdown-inline__input",
          value: u.value?.label || "",
          placeholder: e.placeholder,
          readonly: "",
          onFocus: c
        }, null, 40, Yl)
      ]),
      (t(), X(ge, { to: "body" }, [
        d.value ? (t(), o("ul", {
          key: 0,
          class: "fu-dropdown-inline__menu",
          style: oe(_.value)
        }, [
          (t(!0), o(U, null, ne(e.options, (r) => (t(), o("li", {
            key: r.label,
            class: "fu-dropdown-inline__item",
            onMousedown: ue((p) => w(r), ["prevent"])
          }, [
            a("span", {
              class: "fu-dropdown-inline__dot",
              style: oe({ backgroundColor: r.color })
            }, null, 4),
            ie(" " + S(r.label), 1)
          ], 40, Vl))), 128))
        ], 4)) : z("", !0)
      ]))
    ], 512));
  }
}), Fl = /* @__PURE__ */ G(Tl, [["__scopeId", "data-v-2181a04e"]]), Bl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Fl
}, Symbol.toStringTag, { value: "Module" })), Il = {
  key: 0,
  class: "content"
}, El = ["onClick"], Ll = /* @__PURE__ */ te({
  __name: "DropdownMenu",
  props: {
    actions: {},
    align: { default: "right" },
    content: { type: Boolean, default: !1 },
    isOpen: { type: Boolean, default: void 0 },
    closeOnSelect: { type: Boolean, default: !1 }
  },
  emits: ["open", "close", "update:isOpen"],
  setup(e, { emit: v }) {
    const n = v, s = e, l = y(!1), i = y(null), u = y(null), d = y({
      top: "0px",
      left: "0px"
    });
    fe(
      () => s.isOpen,
      (r) => {
        typeof r == "boolean" && (l.value = r);
      }
    );
    function _(r) {
      r?.stopPropagation();
      const p = !l.value;
      p && document.dispatchEvent(new CustomEvent("close-all-dropdowns")), l.value = p, n(p ? "open" : "close"), n("update:isOpen", p), p && ve(c);
    }
    function c() {
      if (!i.value || !u.value) return;
      const r = i.value.getBoundingClientRect(), p = r.bottom + window.scrollY + 6, O = u.value.offsetWidth;
      let j = r.left + window.scrollX;
      s.align === "center" ? j += r.width / 2 - O / 2 : s.align === "right" && (j = r.right - O + window.scrollX), d.value = {
        position: "absolute",
        top: `${p}px`,
        left: `${j}px`,
        zIndex: "2000"
      };
    }
    function w(r, p) {
      r.onClick?.(), g();
    }
    function f(r) {
      l.value && i.value && !i.value.contains(r.target) && u.value && !u.value.contains(r.target) && g();
    }
    function m() {
      l.value && g();
    }
    function g() {
      l.value = !1, n("close"), n("update:isOpen", !1);
    }
    function k() {
      g();
    }
    return he(() => {
      document.addEventListener("click", f), window.addEventListener("resize", m), document.addEventListener("close-all-dropdowns", k);
    }), _e(() => {
      document.removeEventListener("click", f), window.removeEventListener("resize", m), document.removeEventListener("close-all-dropdowns", k);
    }), (r, p) => (t(), o("div", {
      class: "fu-dropdown",
      ref_key: "dropdown",
      ref: i
    }, [
      a("div", {
        class: "fu-dropdown__trigger",
        onClick: _
      }, [
        J(r.$slots, "trigger", {}, void 0, !0)
      ]),
      (t(), X(ge, { to: "body" }, [
        P(Ye, { name: "fade" }, {
          default: ae(() => [
            l.value ? (t(), o("div", {
              key: 0,
              class: H(["fu-dropdown__menu", [`fu-dropdown__menu--${e.align}`]]),
              style: oe(d.value),
              ref_key: "menuRef",
              ref: u
            }, [
              e.content ? (t(), o("div", Il, [
                J(r.$slots, "content", {}, void 0, !0)
              ])) : z("", !0),
              (t(!0), o(U, null, ne(e.actions, (O) => (t(), o("div", {
                class: "flex w-100",
                key: O.label
              }, [
                a("a", {
                  class: "fu-dropdown__item",
                  onClick: (j) => w(O)
                }, [
                  O.icon ? (t(), X(ce(O.icon), {
                    key: 0,
                    class: "fu-dropdown__icon"
                  })) : z("", !0),
                  ie(" " + S(O.label), 1)
                ], 8, El)
              ]))), 128))
            ], 6)) : z("", !0)
          ]),
          _: 3
        })
      ]))
    ], 512));
  }
}), Xe = /* @__PURE__ */ G(Ll, [["__scopeId", "data-v-d9e510d8"]]), Al = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Xe
}, Symbol.toStringTag, { value: "Module" })), jl = {
  key: 0,
  class: "efw-read"
}, Pl = {
  key: 1,
  class: "efw-edit"
}, Wl = { class: "efw-footer" }, Rl = { class: "efw-read" }, Hl = { class: "efw-footer" }, ql = /* @__PURE__ */ te({
  __name: "EditableFieldWrapper",
  props: {
    modelValue: { default: () => ({}) },
    mode: { default: "inline" },
    teleportTo: { default: "body" },
    align: { default: "right" },
    disableOutsideClose: { type: Boolean, default: !1 }
  },
  setup(e, { expose: v }) {
    const n = e, s = y(!1), l = y(null), i = _t({ top: 0, left: 0 }), u = y(null), d = y(null);
    function _(k) {
      if (k === null || typeof k != "object") return k;
      const r = Ot(k);
      return Array.isArray(r) ? [...r] : r.constructor === Object ? { ...r } : r;
    }
    function c() {
      document.dispatchEvent(new CustomEvent("close-all-editors")), l.value = _(n.modelValue), s.value = !0;
    }
    function w(k) {
      if (s.value) {
        f();
        return;
      }
      document.dispatchEvent(new CustomEvent("close-all-editors")), l.value = _(n.modelValue), ve(() => {
        s.value = !0, ve(() => {
          const r = k?.currentTarget;
          if (!r || !d.value) return;
          const p = r.getBoundingClientRect(), O = d.value.offsetWidth;
          i.top = p.bottom + 6 + window.scrollY, n.align === "left" ? i.left = p.left + window.scrollX : n.align === "center" ? i.left = p.left + p.width / 2 - O / 2 + window.scrollX : i.left = p.right - O + window.scrollX;
        });
      });
    }
    function f() {
      s.value = !1;
    }
    function m() {
      s.value && f();
    }
    function g(k) {
      if (!s.value || n.disableOutsideClose) return;
      const r = k.target;
      r.closest(
        ".fu-status-dropdown, .fu-status-dropdown__menu, .fu-autocomplete-dropdown, .fu-select-dropdown, .fu-datepicker-dropdown"
      ) || (n.mode === "inline" ? u.value && !u.value.contains(r) && f() : d.value && !d.value.contains(r) && f());
    }
    return he(() => {
      document.addEventListener("close-all-editors", m), document.addEventListener("ew-close", f), document.addEventListener("pointerdown", g), window.addEventListener("resize", f);
    }), _e(() => {
      document.removeEventListener("close-all-editors", m), document.removeEventListener("ew-close", f), document.removeEventListener("pointerdown", g), window.removeEventListener("resize", f);
    }), v({
      startEditing: c,
      openTeleport: w,
      closeEditor: f
    }), (k, r) => e.mode === "inline" ? (t(), o("div", {
      key: 0,
      class: "efw-wrapper",
      ref_key: "inlineRef",
      ref: u
    }, [
      s.value ? (t(), o("div", Pl, [
        J(k.$slots, "edit", { model: l.value }, void 0, !0),
        a("div", Wl, [
          J(k.$slots, "actions", {}, void 0, !0)
        ])
      ])) : (t(), o("div", jl, [
        J(k.$slots, "read", {}, void 0, !0)
      ]))
    ], 512)) : (t(), o(U, { key: 1 }, [
      a("div", Rl, [
        J(k.$slots, "read", {}, void 0, !0)
      ]),
      (t(), X(ge, { to: e.teleportTo }, [
        s.value ? (t(), o("div", {
          key: 0,
          class: "efw-teleport-card",
          ref_key: "teleportRef",
          ref: d,
          style: oe({ top: i.top + "px", left: i.left + "px" })
        }, [
          J(k.$slots, "edit", { model: l.value }, void 0, !0),
          a("div", Hl, [
            J(k.$slots, "actions", {}, void 0, !0)
          ])
        ], 4)) : z("", !0)
      ], 8, ["to"]))
    ], 64));
  }
}), Nl = /* @__PURE__ */ G(ql, [["__scopeId", "data-v-90094e16"]]), Ul = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Nl
}, Symbol.toStringTag, { value: "Module" })), Xl = { class: "fu-upload__content" }, Kl = {
  key: 0,
  class: "fu-upload__previews"
}, Jl = ["onClick"], Zl = ["src"], Gl = {
  key: 1,
  class: "fu-upload__file-fallback"
}, Ql = {
  key: 1,
  class: "fu-upload__prompt"
}, es = ["multiple", "accept"], ts = /* @__PURE__ */ te({
  __name: "FusionUpload",
  props: {
    multiple: { type: Boolean, default: !1 },
    accept: { type: String, default: "*" },
    maxFiles: { type: Number, default: 1 / 0 },
    maxFileSizeMB: { type: Number, default: 1 / 0 }
  },
  emits: ["filesSelected", "uploadError"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(!1), i = y(null), u = y([]), d = y([]);
    function _() {
      return typeof crypto < "u" && crypto.randomUUID ? crypto.randomUUID() : `file_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
    }
    function c() {
      i.value?.click();
    }
    function w(k) {
      const r = k.target;
      r.files?.length && (m(r.files), r.value = "");
    }
    function f(k) {
      l.value = !1;
      const r = k.dataTransfer?.files;
      r?.length && m(r);
    }
    function m(k) {
      const r = Array.from(k);
      n.multiple || (u.value = [], d.value = []);
      for (const p of r) {
        const O = n.maxFileSizeMB * 1024 * 1024;
        if (p.size > O) {
          const V = `File "${p.name}" exceeds max size of ${n.maxFileSizeMB} MB.`;
          s("uploadError", V);
          continue;
        }
        if (u.value.length >= n.maxFiles) {
          const V = `Maximum of ${n.maxFiles} files allowed.`;
          s("uploadError", V);
          break;
        }
        u.value.push(p);
        const j = _();
        if (p.type.startsWith("image/")) {
          const V = new FileReader();
          V.onload = (x) => {
            d.value.push({
              id: j,
              src: x.target?.result,
              file: p,
              isImage: !0
            });
          }, V.readAsDataURL(p);
        } else
          d.value.push({
            id: j,
            src: "",
            file: p,
            isImage: !1
          });
      }
      s("filesSelected", u.value);
    }
    function g(k) {
      u.value.splice(k, 1), d.value.splice(k, 1), s("filesSelected", u.value);
    }
    return (k, r) => (t(), o("div", {
      class: H(["fu-upload", { dragging: l.value }]),
      onClick: c,
      onDragover: r[0] || (r[0] = ue((p) => l.value = !0, ["prevent"])),
      onDragleave: r[1] || (r[1] = ue((p) => l.value = !1, ["prevent"])),
      onDrop: ue(f, ["prevent"])
    }, [
      a("div", Xl, [
        d.value.length ? (t(), o("div", Kl, [
          (t(!0), o(U, null, ne(d.value, (p, O) => (t(), o("div", {
            key: p.id,
            class: "fu-upload__preview-item"
          }, [
            a("button", {
              class: "fu-upload__remove",
              onClick: ue((j) => g(O), ["stop"])
            }, " ✕ ", 8, Jl),
            p.isImage ? (t(), o("img", {
              key: 0,
              src: p.src,
              class: "fu-upload__preview-img",
              alt: "Preview"
            }, null, 8, Zl)) : (t(), o("div", Gl, [
              P(Z(rt), { size: 20 }),
              a("span", null, S(p.file.name), 1)
            ]))
          ]))), 128))
        ])) : (t(), o("div", Ql, [
          P(Z(rt), {
            class: "fu-upload__icon",
            size: 22
          }),
          J(k.$slots, "description", {}, () => [
            r[2] || (r[2] = a("p", { class: "fu-upload__text" }, " Drag & drop files or click to browse ", -1))
          ], !0)
        ])),
        a("input", {
          ref_key: "fileInput",
          ref: i,
          type: "file",
          class: "fu-upload__input",
          multiple: e.multiple,
          accept: e.accept,
          onChange: w
        }, null, 40, es)
      ])
    ], 34));
  }
}), ns = /* @__PURE__ */ G(ts, [["__scopeId", "data-v-1b1249ff"]]), os = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: ns
}, Symbol.toStringTag, { value: "Module" })), as = {
  key: 0,
  class: "fu-filter-dropdown__header"
}, ls = { class: "fu-filter-dropdown__title" }, ss = { class: "fu-filter-dropdown__body" }, is = { class: "fu-filter-dropdown__footer" }, us = /* @__PURE__ */ te({
  __name: "FusionFilterDropdown",
  props: {
    align: { default: "left" },
    isOpen: { type: Boolean, default: void 0 },
    payload: {},
    title: { default: "" },
    width: { default: 280 },
    maxWidth: { default: 360 }
  },
  emits: ["apply", "cancel", "open", "close", "update:isOpen"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(!1), i = y(null), u = y(null), d = y({
      top: "0px",
      left: "0px"
    }), _ = N(() => ({
      width: typeof n.width == "number" ? `${n.width}px` : n.width,
      maxWidth: typeof n.maxWidth == "number" ? `${n.maxWidth}px` : n.maxWidth
    }));
    fe(
      () => n.isOpen,
      (p) => {
        typeof p == "boolean" && (l.value = p, p ? (s("open"), ve(g)) : s("close"));
      }
    );
    function c(p) {
      p?.stopPropagation();
      const O = !l.value;
      O && document.dispatchEvent(new CustomEvent("close-all-dropdowns")), l.value = O, s("update:isOpen", O), O ? (s("open"), ve(g)) : s("close");
    }
    function w() {
      s("apply", n.payload), m();
    }
    function f() {
      s("cancel"), m();
    }
    function m() {
      l.value && (l.value = !1, s("update:isOpen", !1), s("close"));
    }
    function g() {
      if (!i.value || !u.value) return;
      const p = i.value.getBoundingClientRect(), O = p.bottom + window.scrollY + 8;
      let j = p.left + window.scrollX;
      n.align === "center" && (j += p.width / 2 - u.value.offsetWidth / 2), n.align === "right" && (j = p.right - u.value.offsetWidth + window.scrollX), d.value = {
        position: "absolute",
        top: `${O}px`,
        left: `${j}px`,
        zIndex: 2e3
      };
    }
    function k(p) {
      l.value && i.value && !i.value.contains(p.target) && u.value && !u.value.contains(p.target) && m();
    }
    function r() {
      m();
    }
    return he(() => {
      document.addEventListener("click", k), document.addEventListener("close-all-dropdowns", r);
    }), _e(() => {
      document.removeEventListener("click", k), document.removeEventListener("close-all-dropdowns", r);
    }), (p, O) => (t(), o("div", {
      class: "fu-filter-dropdown",
      ref_key: "dropdown",
      ref: i
    }, [
      a("div", {
        class: "fu-filter-dropdown__trigger",
        onClick: c
      }, [
        J(p.$slots, "trigger", {}, void 0, !0)
      ]),
      (t(), X(ge, { to: "body" }, [
        P(Ye, { name: "fade" }, {
          default: ae(() => [
            l.value ? (t(), o("div", {
              key: 0,
              ref_key: "menuRef",
              ref: u,
              class: H(["fu-filter-dropdown__menu", [`fu-filter-dropdown__menu--${e.align}`]]),
              style: oe([d.value, _.value])
            }, [
              e.title ? (t(), o("div", as, [
                a("span", ls, S(e.title), 1),
                a("button", {
                  type: "button",
                  class: "fu-filter-dropdown__close",
                  onClick: m,
                  "aria-label": "Close"
                }, " × ")
              ])) : z("", !0),
              a("div", ss, [
                J(p.$slots, "content", {}, void 0, !0)
              ]),
              a("div", is, [
                J(p.$slots, "footer", {}, () => [
                  a("button", {
                    type: "button",
                    class: "fu-btn fu-btn--outline",
                    onClick: f
                  }, " Cancel "),
                  a("button", {
                    type: "button",
                    class: "fu-btn fu-btn--primary",
                    onClick: w
                  }, " Apply ")
                ], !0)
              ])
            ], 6)) : z("", !0)
          ]),
          _: 3
        })
      ]))
    ], 512));
  }
}), rs = /* @__PURE__ */ G(us, [["__scopeId", "data-v-338720a3"]]), ds = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: rs
}, Symbol.toStringTag, { value: "Module" })), cs = {}, fs = { class: "fu-smart-header" };
function vs(e, v) {
  return t(), o("header", fs, [
    J(e.$slots, "default", {}, void 0, !0)
  ]);
}
const ms = /* @__PURE__ */ G(cs, [["render", vs], ["__scopeId", "data-v-317a0cd5"]]), ps = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: ms
}, Symbol.toStringTag, { value: "Module" })), hs = { class: "fu-trash-icon" }, _s = /* @__PURE__ */ te({
  __name: "FusionTrashIcon",
  setup(e) {
    return (v, n) => (t(), o("div", hs, [
      P(Z(lt), { size: 18 })
    ]));
  }
}), ys = /* @__PURE__ */ G(_s, [["__scopeId", "data-v-7cafc569"]]), gs = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: ys
}, Symbol.toStringTag, { value: "Module" })), bs = { class: "fu-activity-item" }, ws = { class: "fu-activity-icon" }, ks = { class: "fu-activity-content" }, $s = { class: "fu-activity-header" }, Ms = { class: "fu-activity-title" }, Cs = { class: "fu-activity-status" }, Ss = { class: "fu-activity-text" }, xs = { class: "fu-activity-subtitle" }, zs = { class: "fu-activity-footer" }, Os = { class: "fu-activity-timestamp" }, Ds = { class: "fu-activity-user" }, Ys = /* @__PURE__ */ te({
  __name: "FusionActivityItem",
  props: {
    icon: {},
    title: {},
    fileName: {},
    timestamp: {},
    userName: {},
    userAvatar: { default: null },
    clickable: { type: Boolean, default: !0 }
  },
  emits: ["click"],
  setup(e, { emit: v }) {
    const n = e, s = v;
    function l() {
      n.clickable && s("click");
    }
    return (i, u) => (t(), o("div", bs, [
      a("div", ws, [
        (t(), X(ce(e.icon), {
          class: "fu-activity-icon__svg",
          size: 18
        }))
      ]),
      u[0] || (u[0] = a("div", { class: "fu-activity-line" }, null, -1)),
      a("div", ks, [
        a("div", {
          class: H(["fu-activity-card", { "is-clickable": e.clickable }]),
          onClick: l
        }, [
          a("div", $s, [
            a("div", Ms, [
              a("span", Cs, [
                P(Z(wt), {
                  class: "fu-activity-status__icon",
                  size: 16
                })
              ]),
              a("p", Ss, S(e.title), 1)
            ])
          ]),
          a("p", xs, S(e.fileName), 1),
          a("div", zs, [
            a("span", Os, S(e.timestamp), 1),
            a("div", Ds, [
              P(Le, {
                src: e.userAvatar || void 0,
                name: e.userName,
                alt: e.userName,
                size: "xs",
                "show-status": !1
              }, null, 8, ["src", "name", "alt"])
            ])
          ])
        ], 2)
      ])
    ]));
  }
}), Vs = /* @__PURE__ */ G(Ys, [["__scopeId", "data-v-3de71024"]]), Ts = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Vs
}, Symbol.toStringTag, { value: "Module" })), Fs = { class: "fu-attachment-left flex flex--gap-md flex--align-center" }, Bs = { class: "fu-attachment-preview" }, Is = ["src", "alt"], Es = { class: "fu-attachment-info flex flex--column" }, Ls = { class: "fu-attachment-title" }, As = { class: "fu-attachment-meta" }, js = { class: "fu-attachment-right flex flex--align-center flex--gap-md" }, Ps = { class: "flex flex--column flex--align-center" }, Ws = { class: "fu-attachment-time" }, Rs = /* @__PURE__ */ te({
  __name: "FusionAttachment",
  props: {
    id: {},
    fileName: {},
    fileUrl: {},
    fileSize: { default: "" },
    timestamp: {},
    userName: {},
    userAvatar: { default: null },
    actions: { default: () => [] }
  },
  emits: ["click"],
  setup(e, { emit: v }) {
    const n = e, s = v;
    function l() {
      s("click", n.id);
    }
    const i = N(() => /\.(png|jpe?g|gif|webp|svg)$/i.test(n.fileName)), u = N(() => /\.(mp4|mov|avi|webm)$/i.test(n.fileName));
    return (d, _) => (t(), o("div", {
      class: "fu-attachment-item",
      role: "button",
      tabindex: "0",
      onClick: l,
      onKeypress: De(l, ["enter"])
    }, [
      a("div", Fs, [
        a("div", Bs, [
          i.value ? (t(), o("img", {
            key: 0,
            src: e.fileUrl,
            alt: e.fileName,
            class: "fu-attachment-thumbnail"
          }, null, 8, Is)) : u.value ? (t(), X(Z(Xt), {
            key: 1,
            class: "fu-attachment-icon",
            size: 20
          })) : (t(), X(Z(Ht), {
            key: 2,
            class: "fu-attachment-icon",
            size: 20
          }))
        ]),
        a("div", Es, [
          a("span", Ls, S(e.fileName), 1),
          a("span", As, S(e.fileSize), 1)
        ])
      ]),
      a("div", js, [
        a("div", Ps, [
          P(Le, {
            src: e.userAvatar || void 0,
            name: e.userName,
            alt: e.userName,
            size: "xs",
            "show-status": !1
          }, null, 8, ["src", "name", "alt"]),
          a("span", Ws, S(e.timestamp), 1)
        ]),
        e.actions?.length ? (t(), o("div", {
          key: 0,
          class: "fu-attachment-actions",
          onClick: _[0] || (_[0] = ue(() => {
          }, ["stop"]))
        }, [
          P(Xe, {
            actions: e.actions,
            align: "right"
          }, {
            trigger: ae(() => [
              P(ke, {
                icon: Z(at),
                variant: "ghost",
                size: "sm"
              }, null, 8, ["icon"])
            ]),
            _: 1
          }, 8, ["actions"])
        ])) : z("", !0)
      ])
    ], 32));
  }
}), Hs = /* @__PURE__ */ G(Rs, [["__scopeId", "data-v-c5d821de"]]), qs = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Hs
}, Symbol.toStringTag, { value: "Module" })), Ns = { class: "fu-note-header" }, Us = { class: "fu-note-title" }, Xs = ["innerHTML"], Ks = { class: "fu-note-actions" }, Js = { class: "fu-note-footer" }, Zs = { class: "fu-note-owner" }, Gs = { class: "fu-note-date" }, Qs = /* @__PURE__ */ te({
  __name: "FusionNoteCard",
  props: {
    id: {},
    title: {},
    content: { default: "" },
    ownerName: {},
    ownerAvatar: { default: null },
    date: {},
    actions: { default: () => [] }
  },
  emits: ["open"],
  setup(e) {
    const v = y(!1);
    return (n, s) => (t(), o("div", {
      class: "fu-note-card",
      onMouseenter: s[0] || (s[0] = (l) => v.value = !0),
      onMouseleave: s[1] || (s[1] = (l) => v.value = !1),
      onClick: s[2] || (s[2] = (l) => n.$emit("open", e.id))
    }, [
      a("div", Ns, [
        a("div", Us, [
          a("h4", null, S(e.title), 1),
          e.content ? (t(), o("div", {
            key: 0,
            class: "fu-note-content",
            innerHTML: e.content
          }, null, 8, Xs)) : z("", !0)
        ]),
        a("div", Ks, [
          P(Xe, {
            actions: e.actions,
            align: "right"
          }, {
            trigger: ae(() => [
              P(ke, {
                icon: Z(at),
                variant: "ghost",
                size: "sm",
                class: "fu-action-trigger"
              }, null, 8, ["icon"])
            ]),
            _: 1
          }, 8, ["actions"])
        ])
      ]),
      a("div", Js, [
        a("div", Zs, [
          P(Le, {
            src: e.ownerAvatar || void 0,
            name: e.ownerName,
            alt: e.ownerName,
            size: "xs",
            "show-status": !1
          }, null, 8, ["src", "name", "alt"])
        ]),
        a("span", Gs, S(e.date), 1)
      ])
    ], 32));
  }
}), ei = /* @__PURE__ */ G(Qs, [["__scopeId", "data-v-2819a085"]]), ti = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: ei
}, Symbol.toStringTag, { value: "Module" })), ni = { class: "fu-task-list" }, oi = { class: "fu-task-left flex flex--gap-xl flex--align-center" }, ai = { class: "flex flex--column flex--gap-lg w-100" }, li = { class: "fu-task-title" }, si = { class: "fu-task-meta" }, ii = { class: "fu-task-priority" }, ui = { class: "fu-priority-label" }, ri = { class: "fu-task-owner" }, di = { class: "fu-task-actions" }, ci = /* @__PURE__ */ te({
  __name: "FusionTaskItem",
  props: {
    id: {},
    title: {},
    ownerName: {},
    ownerAvatar: { default: null },
    priorityLabel: {},
    priorityColor: { default: "#ccc" }
  },
  emits: ["edit", "delete"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = N(() => [
      {
        label: "Edit Task",
        icon: kt,
        onClick: () => s("edit", n.id)
      },
      {
        label: "Delete Task",
        icon: lt,
        onClick: () => s("delete", n.id)
      }
    ]);
    return (i, u) => (t(), o("div", ni, [
      a("div", oi, [
        a("div", ai, [
          a("div", li, S(e.title), 1),
          a("div", si, [
            a("div", ii, [
              a("span", {
                class: "fu-priority-dot",
                style: oe({ backgroundColor: e.priorityColor || "#999" })
              }, null, 4),
              a("span", ui, S(e.priorityLabel), 1)
            ]),
            u[0] || (u[0] = a("span", { class: "fu-dot" }, null, -1)),
            a("div", ri, [
              P(Le, {
                src: e.ownerAvatar || void 0,
                name: e.ownerName,
                alt: e.ownerName,
                size: "xs",
                "show-status": !1
              }, null, 8, ["src", "name", "alt"])
            ])
          ])
        ])
      ]),
      a("div", di, [
        P(Xe, {
          actions: l.value,
          content: !1,
          align: "right"
        }, {
          trigger: ae(() => [
            P(ke, {
              icon: Z(at),
              variant: "subtle",
              size: "sm"
            }, null, 8, ["icon"])
          ]),
          _: 1
        }, 8, ["actions"])
      ])
    ]));
  }
}), fi = /* @__PURE__ */ G(ci, [["__scopeId", "data-v-36cc95a2"]]), vi = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: fi
}, Symbol.toStringTag, { value: "Module" })), mi = { class: "fu-kanban scrollbar__control customScrollBar" }, pi = ["draggable", "onDragstart", "onDrop"], hi = { class: "fu-kanban__column-header" }, _i = { class: "flex flex--center flex--space" }, yi = { class: "fu-kanban__column-title" }, gi = ["title"], bi = { class: "fu-kanban__header-right" }, wi = ["title", "onClick"], ki = { class: "flex flex--center flex--gap-sm" }, $i = { class: "fu-kanban__count" }, Mi = {
  key: 0,
  class: "fu-kanban__edit-body"
}, Ci = ["onDragover", "onDrop"], Si = ["onDragstart", "onDrop", "onClick"], xi = { class: "fu-kanban__card-header" }, zi = { class: "fu-kanban__card-body" }, Oi = {
  key: 0,
  class: "fu-kanban__empty"
}, Di = ["onClick"], Yi = /* @__PURE__ */ te({
  __name: "Kanban",
  props: {
    columns: {},
    editMode: { type: Boolean },
    addItemButtonText: {},
    addColumnButtonText: {},
    noItemtext: {}
  },
  emits: ["update:columns", "update:items", "card-click", "add-item"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = n.addItemButtonText || "+ Add Item", i = n.addColumnButtonText || "+ Add Stage", u = y(JSON.parse(JSON.stringify(n.columns || []))), d = y(null), _ = y(null), c = y(null);
    fe(
      () => n.columns,
      (D) => {
        u.value = JSON.parse(JSON.stringify(D));
      },
      { deep: !0 }
    );
    function w(D, V) {
      d.value = { fromColumnId: D, item: V };
    }
    function f(D) {
      c.value = D;
    }
    function m() {
      c.value = null;
    }
    function g(D, V) {
      const x = d.value;
      if (!x) return;
      const I = u.value.find((W) => W.id === x.fromColumnId), Y = u.value.find((W) => W.id === D);
      !I || !Y || (I.items = I.items.filter((W) => W.id !== x.item.id), V === null ? Y.items.push(x.item) : Y.items.splice(V, 0, x.item), s("update:items", u.value), d.value = null, c.value = null);
    }
    function k(D) {
      n.editMode && (_.value = D);
    }
    function r(D) {
      if (!n.editMode) return;
      const V = _.value;
      if (V === null || V === D) return;
      const x = [...u.value], [I] = x.splice(V, 1);
      x.splice(D, 0, I), u.value = x.map((Y, W) => ({ ...Y, position: W + 1 })), s("update:columns", u.value), _.value = null;
    }
    function p() {
      const D = {
        id: `col-${u.value.length + 1}`,
        title: `Stage ${u.value.length + 1}`,
        color: "#8B5CF6",
        position: u.value.length + 1,
        items: []
      };
      u.value.push(D), s("update:columns", u.value);
    }
    function O(D, V) {
      s("add-item", { column: D, index: V });
    }
    function j(D, V) {
      s("card-click", { id: D.id, item: D, column: V });
    }
    return (D, V) => (t(), o("div", mi, [
      a("div", {
        class: "fu-kanban__columns",
        onDragover: V[1] || (V[1] = ue(() => {
        }, ["prevent"]))
      }, [
        (t(!0), o(U, null, ne(u.value, (x, I) => (t(), o("div", {
          key: x.id,
          class: H(["fu-kanban__column", { "fu-kanban__column--drag": e.editMode }]),
          draggable: e.editMode,
          onDragstart: (Y) => k(I),
          onDrop: (Y) => r(I),
          onDragover: V[0] || (V[0] = ue(() => {
          }, ["prevent"]))
        }, [
          a("header", hi, [
            a("div", _i, [
              a("div", yi, [
                a("span", {
                  class: "fu-kanban__dot",
                  style: oe({ background: x.color || "#9ca3af" })
                }, null, 4),
                a("span", {
                  class: "fu-kanban__column-name",
                  title: x.title
                }, S(x.title), 9, gi)
              ]),
              a("div", bi, [
                a("button", {
                  class: "fu-kanban__add-item-btn",
                  title: Z(l),
                  onClick: ue((Y) => O(x, I), ["stop"])
                }, " + ", 8, wi)
              ])
            ]),
            a("div", ki, [
              J(D.$slots, "column-header", {}, void 0, !0),
              a("span", $i, S(x.items.length), 1)
            ])
          ]),
          e.editMode ? (t(), o("div", Mi, [
            J(D.$slots, "edit-column", {
              column: x,
              index: I
            }, void 0, !0)
          ])) : (t(), o(U, { key: 1 }, [
            a("div", {
              class: H(["fu-kanban__cards scrollbar__control customScrollBar", { "fu-kanban__cards--hover": c.value === x.id }]),
              onDragover: ue((Y) => f(x.id), ["prevent"]),
              onDragleave: m,
              onDrop: (Y) => g(x.id, null)
            }, [
              (t(!0), o(U, null, ne(x.items, (Y, W) => (t(), o("div", {
                key: Y.id,
                class: "fu-kanban__card",
                draggable: "true",
                onDragstart: (q) => w(x.id, Y),
                onDrop: (q) => g(x.id, W),
                onClick: (q) => j(Y, x)
              }, [
                a("header", xi, [
                  a("strong", null, S(Y.title), 1)
                ]),
                a("div", zi, [
                  J(D.$slots, "card-body", {
                    item: Y,
                    column: x
                  }, void 0, !0)
                ])
              ], 40, Si))), 128)),
              x.items.length ? z("", !0) : (t(), o("div", Oi, S(e.noItemtext), 1))
            ], 42, Ci),
            a("button", {
              class: "fu-kanban__add-card",
              onClick: (Y) => O(x, I)
            }, S(Z(l)), 9, Di)
          ], 64))
        ], 42, pi))), 128)),
        e.editMode ? (t(), o("div", {
          key: 0,
          class: "fu-kanban__add-column",
          onClick: p
        }, S(Z(i)), 1)) : z("", !0)
      ], 32)
    ]));
  }
}), Vi = /* @__PURE__ */ G(Yi, [["__scopeId", "data-v-11abb07b"]]), Ti = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Vi
}, Symbol.toStringTag, { value: "Module" })), Fi = { class: "fu-modal__header" }, Bi = { class: "fu-modal__title" }, Ii = { class: "fu-modal__body" }, Ei = {
  key: 0,
  class: "fu-modal__footer"
}, Li = /* @__PURE__ */ te({
  __name: "FusionModal",
  props: {
    isVisible: { type: Boolean },
    title: {},
    size: { default: "md" },
    showFooter: { type: Boolean, default: !0 }
  },
  emits: ["close", "cancel", "confirm"],
  setup(e, { emit: v }) {
    const n = v, s = () => n("close");
    function l(i) {
      (i.key === "Escape" || i.key === "Esc") && s();
    }
    return he(() => {
      window.addEventListener("keydown", l);
    }), _e(() => {
      window.removeEventListener("keydown", l);
    }), (i, u) => (t(), X(ge, { to: "body" }, [
      e.isVisible ? (t(), o("div", {
        key: 0,
        class: "fu-modal__backdrop",
        onClick: ue(s, ["self"])
      }, [
        a("div", {
          class: H(["fu-modal", `fu-modal--${e.size}`])
        }, [
          a("div", Fi, [
            a("h3", Bi, S(e.title), 1),
            P(ke, {
              text: " ",
              icon: Z(Ue),
              class: "fu-modal__close",
              onClick: s,
              variant: "ghost",
              size: "sm"
            }, null, 8, ["icon"])
          ]),
          a("div", Ii, [
            J(i.$slots, "default", {}, () => [
              u[0] || (u[0] = ie(" Default modal content. ", -1))
            ], !0)
          ]),
          e.showFooter ? (t(), o("div", Ei, [
            J(i.$slots, "footer", {}, void 0, !0)
          ])) : z("", !0)
        ], 2)
      ])) : z("", !0)
    ]));
  }
}), St = /* @__PURE__ */ G(Li, [["__scopeId", "data-v-4d75747a"]]), Ai = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: St
}, Symbol.toStringTag, { value: "Module" })), ji = { class: "app-container" }, Pi = { class: "app-shell" }, Wi = { class: "fu-topbar" }, Ri = { class: "fu-topbar-l" }, Hi = { class: "fu-topbar-c" }, qi = { class: "fu-topbar-r" }, Ni = { class: "fu-main-area" }, Ui = { class: "fu-sidebar" }, Xi = { class: "fu-menu" }, Ki = { class: "ai-panel-body" }, Ji = { class: "ai-header-actions" }, Zi = { class: "ai-content" }, Ge = 991, vt = "fu-app-ai-panel", mt = 340, Gi = 500, Qi = {
  __name: "AppShell",
  props: {
    listenToToggleEvent: { type: Boolean, default: !0 }
  },
  setup(e) {
    const v = y(!1), n = y(!0), s = y(!1), l = y(340), i = y(!1), u = y(typeof window < "u" ? window.innerWidth : 1200);
    let d = !1;
    const _ = y(!1), c = e;
    function w() {
      u.value <= Ge ? (v.value = !v.value, n.value = !0) : n.value = !n.value;
    }
    function f() {
      if (u.value <= Ge) {
        _.value = !0, s.value = !1, j();
        return;
      }
      s.value = !s.value, j(), s.value && window.dispatchEvent(new Event("open-ai"));
    }
    function m() {
      l.value = i.value ? mt : Gi, i.value = !i.value, j();
    }
    function g(V) {
      d = !0, document.body.style.cursor = "col-resize", window.addEventListener("mousemove", k), window.addEventListener("mouseup", r);
    }
    function k(V) {
      if (!d) return;
      const x = window.innerWidth - V.clientX;
      x > 280 && x < 600 && (l.value = x, j());
    }
    function r() {
      d = !1, document.body.style.cursor = "default", window.removeEventListener("mousemove", k), window.removeEventListener("mouseup", r);
    }
    function p() {
      u.value = window.innerWidth;
    }
    function O() {
      u.value <= Ge && v.value && (v.value = !1);
    }
    function j() {
      const V = {
        open: s.value,
        width: l.value,
        maximized: i.value
      };
      localStorage.setItem(vt, JSON.stringify(V));
    }
    function D() {
      const V = localStorage.getItem(vt);
      if (V)
        try {
          const { open: x, width: I, maximized: Y } = JSON.parse(V);
          s.value = !!x, l.value = I || mt, i.value = !!Y;
        } catch (x) {
          console.warn("Failed to restore AI panel state:", x);
        }
    }
    return he(() => {
      if (D(), window.addEventListener("resize", p), c.listenToToggleEvent && window.addEventListener("toggle-ai", f), s.value) {
        const V = () => {
          window.removeEventListener("tabs-ready", V), ve(() => {
            s.value = !0, window.dispatchEvent(new Event("open-ai"));
          });
        };
        window.addEventListener("tabs-ready", V);
      }
    }), (V, x) => {
      const I = gt("FusionActionButton");
      return t(), o("div", ji, [
        a("div", Pi, [
          a("div", Wi, [
            a("div", Ri, [
              a("button", {
                class: "fu-menu-toggle",
                onClick: w
              }, [...x[2] || (x[2] = [
                a("svg", {
                  class: "fu-icon fu-icon--grid",
                  width: "16",
                  height: "17",
                  viewBox: "0 0 16 17",
                  fill: "none",
                  xmlns: "http://www.w3.org/2000/svg"
                }, [
                  a("path", {
                    d: "M1 13H3C3.27614 13 3.5 13.2238 3.5 13.5V15.5C3.5 15.7762 3.27614 16 3 16H1C0.723856 16 0.5 15.7762 0.5 15.5V13.5C0.5 13.2238 0.723856 13 1 13ZM7 13H9C9.27616 13 9.5 13.2238 9.5 13.5V15.5C9.5 15.7762 9.27616 16 9 16H7C6.72384 16 6.5 15.7762 6.5 15.5V13.5C6.5 13.2238 6.72384 13 7 13ZM13 13H15C15.2762 13 15.5 13.2238 15.5 13.5V15.5C15.5 15.7762 15.2762 16 15 16H13C12.7238 16 12.5 15.7762 12.5 15.5V13.5C12.5 13.2238 12.7238 13 13 13ZM1 7H3C3.27614 7 3.5 7.22385 3.5 7.5V9.5C3.5 9.77615 3.27614 10 3 10H1C0.723856 10 0.5 9.77615 0.5 9.5V7.5C0.5 7.22385 0.723856 7 1 7ZM7 7H9C9.27616 7 9.5 7.22384 9.5 7.5V9.5C9.5 9.77616 9.27616 10 9 10H7C6.72384 10 6.5 9.77616 6.5 9.5V7.5C6.5 7.22384 6.72384 7 7 7ZM13 7H15C15.2762 7 15.5 7.22384 15.5 7.5V9.5C15.5 9.77616 15.2762 10 15 10H13C12.7238 10 12.5 9.77616 12.5 9.5V7.5C12.5 7.22384 12.7238 7 13 7ZM1 1H3C3.27614 1 3.5 1.22386 3.5 1.5V3.5C3.5 3.77614 3.27614 4 3 4H1C0.723862 4 0.5 3.77614 0.5 3.5V1.5C0.5 1.22386 0.723862 1 1 1ZM7 1H9C9.27615 1 9.5 1.22386 9.5 1.5V3.5C9.5 3.77614 9.27615 4 9 4H7C6.72385 4 6.5 3.77614 6.5 3.5V1.5C6.5 1.22386 6.72385 1 7 1ZM13 1H15C15.2762 1 15.5 1.22386 15.5 1.5V3.5C15.5 3.77614 15.2762 4 15 4H13C12.7238 4 12.5 3.77614 12.5 3.5V1.5C12.5 1.22386 12.7238 1 13 1Z",
                    stroke: "currentColor"
                  })
                ], -1)
              ])]),
              J(V.$slots, "brand-logo", {}, () => [
                x[3] || (x[3] = a("img", {
                  src: "https://storage.googleapis.com/skkido/logo-assets/skkido-logo-white.svg",
                  class: "fu-brand-logo",
                  alt: "Logo"
                }, null, -1))
              ])
            ]),
            a("div", Hi, [
              J(V.$slots, "header")
            ]),
            a("div", qi, [
              J(V.$slots, "header-right")
            ])
          ]),
          a("div", Ni, [
            a("div", {
              class: H(["fu-nav-panel", { open: v.value }])
            }, [
              a("section", Ui, [
                J(V.$slots, "modules-sidebar")
              ]),
              Te(a("section", Xi, [
                J(V.$slots, "module-menu")
              ], 512), [
                [nt, n.value]
              ])
            ], 2),
            a("div", {
              class: H(["fu-body-area", { "ai-open": s.value }])
            }, [
              a("div", {
                onClick: O,
                class: "fu-body-slot"
              }, [
                J(V.$slots, "default")
              ]),
              P(Ye, { name: "slide-left" }, {
                default: ae(() => [
                  s.value ? (t(), o("div", {
                    key: 0,
                    class: "ai-panel",
                    style: oe({ width: l.value + "px" })
                  }, [
                    a("div", {
                      class: "ai-resize-handle",
                      onMousedown: g
                    }, null, 32),
                    a("div", Ki, [
                      a("header", null, [
                        x[4] || (x[4] = a("h3", null, "Àdisa", -1)),
                        a("div", Ji, [
                          P(I, {
                            icon: i.value ? Z(Nt) : Z(qt),
                            variant: "ghost",
                            size: "sm",
                            onClick: m
                          }, null, 8, ["icon"]),
                          P(I, {
                            icon: Z(Ue),
                            variant: "ghost",
                            size: "sm",
                            onClick: f
                          }, null, 8, ["icon"])
                        ])
                      ]),
                      a("div", Zi, [
                        J(V.$slots, "ai-content")
                      ])
                    ])
                  ], 4)) : z("", !0)
                ]),
                _: 3
              })
            ], 2)
          ])
        ]),
        _.value ? (t(), X(St, {
          key: 0,
          isVisible: _.value,
          title: "Adisa",
          size: "sm",
          onClose: x[0] || (x[0] = (Y) => _.value = !1),
          onCancel: x[1] || (x[1] = (Y) => _.value = !1)
        }, {
          default: ae(() => [...x[5] || (x[5] = [
            ie(" Downlaod Skkido to use Adisa on Mobile ", -1)
          ])]),
          _: 1
        }, 8, ["isVisible"])) : z("", !0)
      ]);
    };
  }
}, eu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Qi
}, Symbol.toStringTag, { value: "Module" })), tu = { key: 0 }, nu = {
  key: 0,
  class: "fu-listview__th fu-listview__th--checkbox"
}, ou = ["role", "tabindex", "onClick", "onKeydown"], au = { class: "fu-listview__th-label" }, lu = {
  key: 1,
  class: "fu-listview__sort-indicator"
}, su = ["onMousedown"], iu = ["onClick"], uu = { class: "fu-listview__cell" }, ru = /* @__PURE__ */ te({
  __name: "FusionListView",
  props: {
    columns: {},
    rows: {},
    rowKey: {},
    showHeader: { type: Boolean },
    options: {}
  },
  emits: ["row-selected", "row-clicked", "sort-change"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(
      n.columns.map((Y) => ({
        ...Y,
        width: Y.width || "150px",
        textAlign: Y.textAlign || "justify",
        sortable: !!Y.sortable
      }))
    ), i = y(!1), u = y(null), d = y(null), _ = y(null), c = y("asc"), w = n.showHeader ?? !0, f = () => n.options?.sortable !== !1;
    function m(Y) {
      return n.options?.selectable ? n.options?.isRowSelectable ? n.options.isRowSelectable(Y) : !0 : !1;
    }
    function g() {
      const Y = n.rows.filter((W) => W.__selected && m(W));
      s("row-selected", Y);
    }
    fe(i, (Y) => {
      n.options?.selectable && (n.rows.forEach((W) => {
        m(W) && (W.__selected = Y);
      }), g());
    });
    function k(Y) {
      f() && Y.sortable && (_.value !== Y.key ? (_.value = Y.key, c.value = "asc") : c.value = c.value === "asc" ? "desc" : "asc", s("sort-change", {
        key: _.value,
        direction: c.value
      }));
    }
    function r(Y) {
      s("row-clicked", Y), n.options?.onRowClick?.(Y);
    }
    let p = null, O = 0, j = 0;
    function D(Y, W) {
      n.options?.resizeColumn && (p = W, O = Y.clientX, j = parseInt(l.value[W].width, 10), document.addEventListener("mousemove", V), document.addEventListener("mouseup", x));
    }
    function V(Y) {
      if (p === null) return;
      const W = Y.clientX - O;
      l.value[p].width = `${Math.max(60, j + W)}px`;
    }
    function x() {
      p = null, document.removeEventListener("mousemove", V), document.removeEventListener("mouseup", x);
    }
    function I() {
      const Y = d.value;
      Y && (Y.style.overflowY = "hidden", requestAnimationFrame(() => {
        Y.style.overflowY = "auto";
      }));
    }
    return he(() => {
      I(), window.addEventListener("resize", I);
    }), _e(() => {
      window.removeEventListener("resize", I), document.removeEventListener("mousemove", V), document.removeEventListener("mouseup", x);
    }), (Y, W) => (t(), o("div", {
      class: "fu-listview",
      ref_key: "listviewRef",
      ref: u
    }, [
      a("div", {
        class: "fu-listview__table-wrapper customScrollBar",
        ref_key: "tableWrapper",
        ref: d
      }, [
        a("table", null, [
          Z(w) ? (t(), o("thead", tu, [
            a("tr", null, [
              e.options?.selectable ? (t(), o("th", nu, [
                P(et, {
                  modelValue: i.value,
                  "onUpdate:modelValue": W[0] || (W[0] = (q) => i.value = q),
                  size: "sm"
                }, null, 8, ["modelValue"])
              ])) : z("", !0),
              (t(!0), o(U, null, ne(l.value, (q, R) => (t(), o("th", {
                key: q.key,
                style: oe({ width: q.width || "auto" }),
                class: H(["fu-listview__th", `align-${q.textAlign || "justify"}`])
              }, [
                a("div", {
                  class: H(["fu-listview__th-content", `align-${q.textAlign || "justify"}`])
                }, [
                  a("div", {
                    class: H(["fu-listview__th-sortable", {
                      "is-sortable": !!q.sortable,
                      "is-active": _.value === q.key
                    }]),
                    role: q.sortable ? "button" : void 0,
                    tabindex: q.sortable ? 0 : -1,
                    onClick: (E) => k(q),
                    onKeydown: [
                      De(ue((E) => k(q), ["prevent"]), ["enter"]),
                      De(ue((E) => k(q), ["prevent"]), ["space"])
                    ]
                  }, [
                    q.icon ? (t(), X(ce(q.icon), {
                      key: 0,
                      class: "fu-listview__th-icon"
                    })) : z("", !0),
                    a("span", au, S(q.label), 1),
                    q.sortable && _.value === q.key ? (t(), o("span", lu, S(c.value === "asc" ? "▲" : "▼"), 1)) : z("", !0)
                  ], 42, ou),
                  e.options?.resizeColumn ? (t(), o("span", {
                    key: 0,
                    class: "fu-listview__resize-handle",
                    onMousedown: ue((E) => D(E, R), ["stop"])
                  }, null, 40, su)) : z("", !0)
                ], 2)
              ], 6))), 128))
            ])
          ])) : z("", !0),
          a("tbody", null, [
            (t(!0), o(U, null, ne(e.rows, (q) => (t(), o("tr", {
              key: q[e.rowKey],
              class: "fu-listview__row",
              onClick: (R) => r(q)
            }, [
              e.options?.selectable ? (t(), o("td", {
                key: 0,
                class: "fu-listview__td fu-listview__td--checkbox",
                onClick: W[1] || (W[1] = ue(() => {
                }, ["stop"]))
              }, [
                P(et, {
                  modelValue: q.__selected,
                  "onUpdate:modelValue": (R) => q.__selected = R,
                  onChange: g,
                  size: "sm",
                  disabled: !m(q)
                }, null, 8, ["modelValue", "onUpdate:modelValue", "disabled"])
              ])) : z("", !0),
              J(Y.$slots, "tableRow", { row: q }, () => [
                (t(!0), o(U, null, ne(l.value, (R) => (t(), o("td", {
                  key: R.key,
                  class: H(["fu-listview__td", `align-${R.textAlign || "justify"}`]),
                  style: oe({ width: R.width })
                }, [
                  J(Y.$slots, `cell-${R.key}`, {
                    row: q,
                    col: R
                  }, () => [
                    a("span", uu, S(q[R.key]), 1)
                  ], !0)
                ], 6))), 128))
              ], !0)
            ], 8, iu))), 128))
          ])
        ])
      ], 512)
    ], 512));
  }
}), du = /* @__PURE__ */ G(ru, [["__scopeId", "data-v-51d62c5a"]]), cu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: du
}, Symbol.toStringTag, { value: "Module" })), fu = { class: "fu-confirm__body" }, vu = { class: "fu-confirm__icon" }, mu = { class: "fu-confirm__title" }, pu = { class: "fu-confirm__message" }, hu = { class: "fu-confirm__footer" }, _u = /* @__PURE__ */ te({
  __name: "FusionConfirmDialog",
  props: {
    isVisible: { type: Boolean },
    title: {},
    message: {},
    variant: { default: "confirm" },
    confirmText: { default: "Confirm" },
    cancelText: { default: "Cancel" }
  },
  emits: ["cancel", "confirm"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = () => s("cancel"), i = () => s("confirm"), u = N(() => n.variant === "delete" ? lt : n.variant === "warning" ? Ut : wt), d = N(() => n.variant === "delete" || n.variant === "warning" ? "danger" : "solid"), _ = (c) => {
      n.isVisible && (c.key === "Enter" && (c.preventDefault(), i()), c.key === "Escape" && l());
    };
    return he(() => {
      window.addEventListener("keydown", _);
    }), _e(() => {
      window.removeEventListener("keydown", _);
    }), (c, w) => (t(), X(ge, { to: "body" }, [
      e.isVisible ? (t(), o("div", {
        key: 0,
        class: "fu-modal__backdrop",
        onClick: ue(l, ["self"])
      }, [
        a("div", {
          class: H(["fu-confirm", `fu-confirm--${e.variant}`])
        }, [
          a("div", fu, [
            a("div", vu, [
              (t(), X(ce(u.value)))
            ]),
            a("h3", mu, S(e.title), 1),
            a("p", pu, S(e.message), 1)
          ]),
          a("div", hu, [
            P(we, {
              variant: "outline",
              buttonWidth: "100%",
              onClick: l
            }, {
              default: ae(() => [
                ie(S(e.cancelText), 1)
              ]),
              _: 1
            }),
            P(we, {
              variant: d.value,
              buttonWidth: "100%",
              onClick: i
            }, {
              default: ae(() => [
                ie(S(e.confirmText), 1)
              ]),
              _: 1
            }, 8, ["variant"])
          ])
        ], 2)
      ])) : z("", !0)
    ]));
  }
}), yu = /* @__PURE__ */ G(_u, [["__scopeId", "data-v-ea4f0f2e"]]), gu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: yu
}, Symbol.toStringTag, { value: "Module" })), bu = {
  key: 0,
  class: "fu-preview-backdrop"
}, wu = { class: "fu-preview-modal" }, ku = { class: "fu-preview-header" }, $u = { class: "fu-preview-header__left" }, Mu = { class: "fu-preview-header__right" }, Cu = { class: "fu-preview-subheader" }, Su = { class: "fu-preview-body" }, xu = /* @__PURE__ */ te({
  __name: "FusionPreviewModal",
  props: {
    isVisible: { type: Boolean }
  },
  emits: ["close"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = () => s("close");
    fe(
      () => n.isVisible,
      (u) => {
        document.body.style.overflow = u ? "hidden" : "";
      },
      { immediate: !0 }
    );
    function i(u) {
      u.key === "Escape" && l();
    }
    return he(() => {
      window.addEventListener("keydown", i);
    }), _e(() => {
      window.removeEventListener("keydown", i), document.body.style.overflow = "";
    }), (u, d) => (t(), X(ge, { to: "body" }, [
      e.isVisible ? (t(), o("div", bu, [
        a("div", wu, [
          a("header", ku, [
            a("div", $u, [
              J(u.$slots, "header-left", {}, void 0, !0)
            ]),
            a("div", Mu, [
              J(u.$slots, "header-right", {}, () => [
                P(we, {
                  variant: "subtle",
                  size: "sm",
                  text: "Close preview",
                  onClick: l
                })
              ], !0)
            ])
          ]),
          a("header", Cu, [
            J(u.$slots, "subheader", {}, void 0, !0)
          ]),
          a("main", Su, [
            J(u.$slots, "default", {}, void 0, !0)
          ])
        ])
      ])) : z("", !0)
    ]));
  }
}), zu = /* @__PURE__ */ G(xu, [["__scopeId", "data-v-fa20d496"]]), Ou = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: zu
}, Symbol.toStringTag, { value: "Module" })), Du = { class: "fu-module-menu-wrapper scrollbar__control customScrollBar" }, Yu = { class: "fu-module-menu-wrapper__list" }, Vu = {
  key: 0,
  class: "fu-module-menu-empty"
}, Tu = /* @__PURE__ */ te({
  __name: "FusionModuleMenu",
  props: {
    items: {},
    activePath: {}
  },
  setup(e) {
    return (v, n) => {
      const s = gt("router-link");
      return t(), o("div", Du, [
        a("ul", Yu, [
          (t(!0), o(U, null, ne(e.items, (l) => (t(), o("li", {
            key: l.path,
            class: H({ active: e.activePath && e.activePath.startsWith(l.path) })
          }, [
            P(s, {
              class: "fu-module-menu-link",
              to: l.path
            }, {
              default: ae(() => [
                l.icon ? (t(), X(ce(l.icon), {
                  key: 0,
                  size: 15,
                  class: "fu-module-menu-icon"
                })) : z("", !0),
                a("span", null, S(l.label), 1)
              ]),
              _: 2
            }, 1032, ["to"])
          ], 2))), 128)),
          !e.items || !e.items.length ? (t(), o("li", Vu, " No menu items ")) : z("", !0)
        ])
      ]);
    };
  }
}), Fu = /* @__PURE__ */ G(Tu, [["__scopeId", "data-v-7631f4b6"]]), Bu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Fu
}, Symbol.toStringTag, { value: "Module" })), Iu = { class: "fu-bell-icon" }, Eu = {
  key: 2,
  class: "fu-badge"
}, Lu = /* @__PURE__ */ te({
  __name: "FuNotification",
  props: {
    unreadCount: { default: 0 },
    bellIcon: { default: void 0 },
    align: { default: "right" },
    bellClass: { default: "" },
    bellStyle: { default: "light" }
  },
  setup(e) {
    const v = e, n = y(!1), s = y(null), l = y(null), i = y({}), u = () => {
      if (n.value = !n.value, n.value && s.value) {
        const _ = s.value.getBoundingClientRect();
        let c = _.left + window.scrollX;
        v.align === "right" ? c = _.right + window.scrollX - 300 : v.align === "center" && (c = _.left + window.scrollX - 160 + _.width / 2), i.value = {
          top: `${_.bottom + window.scrollY + 8}px`,
          left: `${Math.max(c, 8)}px`,
          position: "absolute",
          zIndex: "2000"
        };
      }
    }, d = (_) => {
      const c = _.target;
      n.value && s.value && !s.value.contains(c) && (!l.value || !l.value.contains(c)) && (n.value = !1);
    };
    return he(() => document.addEventListener("click", d)), _e(() => document.removeEventListener("click", d)), (_, c) => (t(), o("div", {
      class: "fu-notification-dropdown",
      ref_key: "dropdown",
      ref: s
    }, [
      a("div", {
        class: "fu-notification__trigger",
        onClick: u
      }, [
        a("div", Iu, [
          e.bellIcon ? (t(), X(ce(e.bellIcon), {
            key: 0,
            "stroke-width": 1.5,
            class: "fu-bell-svg"
          })) : (t(), X(Z(Et), {
            key: 1,
            class: H(["fu-bell-svg", [v.bellStyle, v.bellClass]])
          }, null, 8, ["class"])),
          e.unreadCount > 0 ? (t(), o("span", Eu, S(e.unreadCount), 1)) : z("", !0)
        ])
      ]),
      (t(), X(ge, { to: "body" }, [
        P(Ye, { name: "fade" }, {
          default: ae(() => [
            n.value ? (t(), o("div", {
              key: 0,
              ref_key: "panelEl",
              ref: l,
              class: "fu-notification__panel",
              style: oe(i.value)
            }, [
              J(_.$slots, "default", {}, () => [
                c[0] || (c[0] = a("div", { class: "fu-empty" }, [
                  a("h4", null, "No Content"),
                  a("p", null, "Use the default slot to pass custom dropdown body.")
                ], -1))
              ], !0)
            ], 4)) : z("", !0)
          ]),
          _: 3
        })
      ]))
    ], 512));
  }
}), Au = /* @__PURE__ */ G(Lu, [["__scopeId", "data-v-b710a214"]]), ju = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Au
}, Symbol.toStringTag, { value: "Module" })), Pu = {
  key: 0,
  class: "fu-alert__icon"
}, Wu = { class: "fu-alert__content" }, Ru = {
  key: 1,
  class: "fu-alert__close"
}, Hu = /* @__PURE__ */ te({
  __name: "FuAlert",
  props: {
    variant: {},
    icon: { type: Boolean },
    border: { type: Boolean },
    dismissible: { type: Boolean }
  },
  setup(e) {
    const v = y(!0);
    return (n, s) => v.value ? (t(), o("div", {
      key: 0,
      class: H([
        "fu-alert",
        `fu-alert--${e.variant}`,
        { "fu-alert--bordered": e.border }
      ])
    }, [
      e.icon ? (t(), o("div", Pu, [
        J(n.$slots, "icon", {}, () => [
          P(Z(At))
        ], !0)
      ])) : z("", !0),
      a("div", Wu, [
        J(n.$slots, "default", {}, void 0, !0)
      ]),
      e.dismissible ? (t(), o("div", Ru, [
        P(ke, {
          icon: Z(Ue),
          size: "sm",
          variant: "ghost",
          onClick: s[0] || (s[0] = (l) => v.value = !1)
        }, null, 8, ["icon"])
      ])) : z("", !0)
    ], 2)) : z("", !0);
  }
}), qu = /* @__PURE__ */ G(Hu, [["__scopeId", "data-v-d43b4e98"]]), Nu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: qu
}, Symbol.toStringTag, { value: "Module" })), Uu = { class: "fu-toast__content" }, Xu = { class: "fu-toast__message" }, Ku = /* @__PURE__ */ te({
  __name: "FusionToast",
  props: {
    message: {},
    type: { default: "info" },
    duration: { default: 3500 }
  },
  setup(e) {
    const v = e, n = y(!1), s = {
      success: jt,
      error: Pt,
      info: ut,
      dark: ut
    };
    he(() => {
      n.value = !0, setTimeout(() => n.value = !1, v.duration);
    });
    function l() {
      n.value = !1;
    }
    return (i, u) => (t(), X(ge, { to: "body" }, [
      P(Ye, { name: "fu-toast-fade" }, {
        default: ae(() => [
          n.value ? (t(), o("div", {
            key: 0,
            class: H(["fu-toast", [`fu-toast--${e.type}`]]),
            role: "alert"
          }, [
            a("div", Uu, [
              (t(), X(ce(s[e.type]), { class: "fu-toast__icon" })),
              a("span", Xu, S(e.message), 1),
              a("button", {
                class: "fu-toast__close",
                onClick: l
              }, "×")
            ])
          ], 2)) : z("", !0)
        ]),
        _: 1
      })
    ]));
  }
}), xt = /* @__PURE__ */ G(Ku, [["__scopeId", "data-v-9f423b9f"]]), Ju = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: xt
}, Symbol.toStringTag, { value: "Module" })), Zu = { class: "fu-pagination" }, Gu = { class: "fu-pagination__left" }, Qu = {
  key: 0,
  class: "fu-pagination__info"
}, er = { class: "fu-pagination__controls" }, tr = ["disabled"], nr = ["disabled", "onClick"], or = ["disabled"], ar = /* @__PURE__ */ te({
  __name: "FusionPagination",
  props: {
    page: {},
    pageSize: {},
    total: {},
    variant: { default: "default" },
    showInfo: { type: Boolean, default: !0 },
    siblingCount: { default: 1 },
    showPageSize: { type: Boolean, default: !0 },
    pageSizeOptions: { default: () => [10, 25, 50, 100] }
  },
  emits: ["update:page", "update:pageSize"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = N(
      () => n.pageSizeOptions.map((m) => ({
        label: String(m),
        value: m
      }))
    ), i = N({
      get() {
        return l.value.find((m) => m.value === n.pageSize) || l.value[0];
      },
      set(m) {
        s("update:pageSize", m.value), s("update:page", 1);
      }
    }), u = N(() => Math.max(1, Math.ceil(n.total / n.pageSize))), d = N(
      () => n.total === 0 ? 0 : (n.page - 1) * n.pageSize + 1
    ), _ = N(() => Math.min(n.page * n.pageSize, n.total));
    function c(m) {
      m < 1 || m > u.value || s("update:page", m);
    }
    function w(m) {
      m && c(m);
    }
    const f = N(() => {
      const m = [], g = u.value, k = n.page, r = n.siblingCount, p = Math.max(2, k - r), O = Math.min(g - 1, k + r);
      m.push({ key: "p-1", label: "1", page: 1 }), p > 2 && m.push({ key: "e-left", label: "…", ellipsis: !0 });
      for (let j = p; j <= O; j++)
        m.push({ key: `p-${j}`, label: String(j), page: j });
      return O < g - 1 && m.push({ key: "e-right", label: "…", ellipsis: !0 }), g > 1 && m.push({ key: `p-${g}`, label: String(g), page: g }), m;
    });
    return (m, g) => (t(), o("div", Zu, [
      a("div", Gu, [
        e.showInfo ? (t(), o("div", Qu, S(d.value) + "–" + S(_.value) + " of " + S(e.total), 1)) : z("", !0),
        e.showPageSize ? (t(), X($t, {
          key: 1,
          modelValue: i.value,
          "onUpdate:modelValue": g[0] || (g[0] = (k) => i.value = k),
          options: l.value,
          align: "left",
          size: "sm"
        }, null, 8, ["modelValue", "options"])) : z("", !0)
      ]),
      a("div", er, [
        a("button", {
          class: "fu-pagination__btn",
          disabled: e.page <= 1,
          onClick: g[1] || (g[1] = (k) => c(e.page - 1)),
          "aria-label": "Previous page"
        }, [
          P(Z(Pe), { class: "fu-pagination__icon" })
        ], 8, tr),
        e.variant !== "simple" ? (t(!0), o(U, { key: 0 }, ne(f.value, (k) => (t(), o("button", {
          key: k.key,
          class: H(["fu-pagination__btn", { active: k.page === e.page, ellipsis: k.ellipsis }]),
          disabled: !!k.ellipsis,
          onClick: (r) => w(k.page)
        }, S(k.label), 11, nr))), 128)) : z("", !0),
        a("button", {
          class: "fu-pagination__btn",
          disabled: e.page >= u.value,
          onClick: g[2] || (g[2] = (k) => c(e.page + 1)),
          "aria-label": "Next page"
        }, [
          P(Z(We), { class: "fu-pagination__icon" })
        ], 8, or)
      ])
    ]));
  }
}), lr = /* @__PURE__ */ G(ar, [["__scopeId", "data-v-209f4783"]]), sr = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: lr
}, Symbol.toStringTag, { value: "Module" })), ir = {
  key: 0,
  class: "fu-panel__header px-2"
}, ur = {
  key: 0,
  class: "fu-panel__title"
}, rr = {
  key: 1,
  class: "fu-panel__actions"
}, dr = { class: "fu-panel__body-wrapper" }, cr = { class: "fu-panel__body scrollbar__control customScrollBar px-2" }, fr = /* @__PURE__ */ te({
  __name: "FuPanel",
  props: {
    title: {},
    basis: {}
  },
  setup(e) {
    const v = e, n = N(() => v.basis ? typeof v.basis == "number" ? `${v.basis}px` : v.basis : "300px");
    return (s, l) => (t(), o("div", {
      class: "fu-panel",
      style: oe({ flexBasis: n.value })
    }, [
      e.title || s.$slots.actions ? (t(), o("div", ir, [
        e.title ? (t(), o("h3", ur, S(e.title), 1)) : z("", !0),
        s.$slots.actions ? (t(), o("div", rr, [
          J(s.$slots, "actions")
        ])) : z("", !0)
      ])) : z("", !0),
      a("div", dr, [
        a("div", cr, [
          J(s.$slots, "default")
        ])
      ])
    ], 4));
  }
}), vr = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: fr
}, Symbol.toStringTag, { value: "Module" })), mr = {
  key: 0,
  class: "fu-input-label"
}, pr = {
  key: 0,
  class: "fu-input-required"
}, hr = ["type", "placeholder", "disabled", "required"], _r = {
  key: 1,
  class: "fu-input-error"
}, yr = /* @__PURE__ */ te({
  __name: "FusionPasswordInput",
  props: {
    modelValue: { default: "" },
    label: { default: "" },
    placeholder: { default: "Enter password" },
    size: { default: "sm" },
    variant: { default: "outline" },
    disabled: { type: Boolean, default: !1 },
    error: { default: null },
    required: { type: Boolean, default: !1 },
    formWrapperWidth: { default: "fit-content" }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(n.modelValue), i = y(!1), u = () => {
      i.value = !i.value;
    };
    return fe(l, (d) => s("update:modelValue", d)), fe(() => n.modelValue, (d) => l.value = d), (d, _) => (t(), o("div", {
      class: "fu-input-wrapper",
      style: oe({ width: e.formWrapperWidth })
    }, [
      e.label ? (t(), o("label", mr, [
        ie(S(e.label) + " ", 1),
        e.required ? (t(), o("span", pr, "*")) : z("", !0)
      ])) : z("", !0),
      a("div", {
        class: H(["fu-input-container", [
          `fu-input--${e.size}`,
          `fu-input--${e.variant}`,
          { "fu-input--error": e.error }
        ]])
      }, [
        Te(a("input", tt(d.$attrs, {
          class: "fu-input",
          type: i.value ? "text" : "password",
          placeholder: e.placeholder,
          disabled: e.disabled,
          required: e.required,
          "onUpdate:modelValue": _[0] || (_[0] = (c) => l.value = c)
        }), null, 16, hr), [
          [yt, l.value]
        ]),
        a("button", {
          type: "button",
          class: "fu-password-toggle",
          onClick: u
        }, [
          (t(), X(ce(i.value ? Z(Wt) : Z(Rt)), { class: "fu-password-icon" }))
        ])
      ], 2),
      e.error ? (t(), o("span", _r, S(e.error), 1)) : z("", !0)
    ], 4));
  }
}), gr = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: yr
}, Symbol.toStringTag, { value: "Module" })), br = { class: "fu-progress-stepper" }, wr = { class: "fu-progress-bar" }, kr = {
  key: 0,
  class: "fu-step-labels"
}, $r = ["onClick"], Mr = { class: "circle" }, Cr = { class: "label" }, Sr = /* @__PURE__ */ te({
  __name: "FusionProgressStepper",
  props: {
    currentStep: {},
    totalSteps: {},
    showLabels: { type: Boolean },
    steps: {}
  },
  emits: ["step-click"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = N(
      () => `${(n.currentStep + 1) / n.totalSteps * 100}%`
    ), i = (u) => s("step-click", u);
    return (u, d) => (t(), o("div", br, [
      a("div", wr, [
        a("div", {
          class: "fu-progress-fill",
          style: oe({ width: l.value })
        }, null, 4)
      ]),
      e.showLabels && e.steps ? (t(), o("div", kr, [
        (t(!0), o(U, null, ne(e.steps, (_, c) => (t(), o("div", {
          key: c,
          class: H(["fu-step-label", { active: e.currentStep === c }]),
          onClick: (w) => i(c)
        }, [
          a("div", Mr, S(c + 1), 1),
          a("div", Cr, S(_.title), 1)
        ], 10, $r))), 128))
      ])) : z("", !0)
    ]));
  }
}), xr = /* @__PURE__ */ G(Sr, [["__scopeId", "data-v-6ac0e869"]]), zr = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: xr
}, Symbol.toStringTag, { value: "Module" })), Or = { class: "fu-range-control" }, Dr = {
  key: 0,
  class: "fu-range-label"
}, Yr = { class: "fu-range-track" }, Vr = ["min", "max", "step", "value"], Tr = { class: "fu-range-value" }, Fr = /* @__PURE__ */ te({
  __name: "FusionRangeControl",
  props: {
    modelValue: { default: 0 },
    min: { default: 0 },
    max: { default: 100 },
    step: { default: 1 },
    label: { default: "" },
    unit: { default: "" }
  },
  emits: ["update:modelValue", "change"],
  setup(e, { emit: v }) {
    const n = e, s = v;
    function l(u) {
      const d = u.target, _ = Number(d.value);
      s("update:modelValue", _), s("change", _);
    }
    const i = N(
      () => n.unit ? `${n.modelValue}${n.unit}` : String(n.modelValue)
    );
    return (u, d) => (t(), o("div", Or, [
      e.label ? (t(), o("label", Dr, S(e.label), 1)) : z("", !0),
      a("div", Yr, [
        a("input", {
          type: "range",
          min: e.min,
          max: e.max,
          step: e.step,
          value: e.modelValue,
          onInput: l
        }, null, 40, Vr),
        a("span", Tr, S(i.value), 1)
      ])
    ]));
  }
}), Br = /* @__PURE__ */ G(Fr, [["__scopeId", "data-v-9c1e4c91"]]), Ir = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Br
}, Symbol.toStringTag, { value: "Module" })), Er = {}, Lr = { class: "fu-section-header" }, Ar = { class: "fu-section-header__left" }, jr = { class: "fu-section-header__right" };
function Pr(e, v) {
  return t(), o("div", Lr, [
    a("div", Ar, [
      J(e.$slots, "left", {}, void 0, !0)
    ]),
    a("div", jr, [
      J(e.$slots, "right", {}, void 0, !0)
    ])
  ]);
}
const Wr = /* @__PURE__ */ G(Er, [["render", Pr], ["__scopeId", "data-v-b7f6e763"]]), Rr = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Wr
}, Symbol.toStringTag, { value: "Module" })), Hr = { class: "fu-sidebar__list" }, qr = ["onClick"], Nr = { class: "fu-sidebar__icon-wrapper" }, Ur = {
  key: 0,
  class: "fu-sidebar__badge"
}, Xr = { class: "fu-sidebar__label" }, Kr = /* @__PURE__ */ te({
  __name: "FusionSidebar",
  props: {
    modules: {},
    activeModule: {},
    backgroundColor: {},
    borderRadius: {}
  },
  emits: ["select"],
  setup(e) {
    const v = e, n = N(() => ({
      "--fu-sidebar-bg": v.backgroundColor ?? "var(--fu-brand-background)",
      "--fu-sidebar-radius": v.borderRadius ?? "0px"
    }));
    return (s, l) => (t(), o("nav", {
      class: "fu-sidebar",
      style: oe(n.value)
    }, [
      a("ul", Hr, [
        (t(!0), o(U, null, ne(e.modules, (i) => (t(), o("li", {
          key: i.name,
          class: H({ active: e.activeModule === i.name })
        }, [
          a("div", {
            class: "fu-sidebar__item",
            onClick: (u) => s.$emit("select", i)
          }, [
            a("div", Nr, [
              (t(), X(ce(i.icon), {
                class: "fu-sidebar__icon",
                size: 20
              })),
              i.count ? (t(), o("span", Ur, S(i.count > 99 ? "99+" : i.count), 1)) : z("", !0)
            ]),
            a("span", Xr, S(i.label), 1)
          ], 8, qr)
        ], 2))), 128))
      ])
    ], 4));
  }
}), Jr = /* @__PURE__ */ G(Kr, [["__scopeId", "data-v-dfe88081"]]), Zr = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Jr
}, Symbol.toStringTag, { value: "Module" })), Gr = ["disabled", "aria-expanded"], Qr = ["aria-expanded", "disabled"], ed = ["onClick"], td = ["src"], nd = { class: "fu-split-button__option-label" }, od = /* @__PURE__ */ te({
  __name: "FusionSplitButton",
  props: {
    options: {},
    disabled: { type: Boolean },
    size: {},
    variant: {},
    color: {},
    icon: {},
    align: {},
    buttonWidth: {}
  },
  emits: ["main-action", "select"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(!1), i = y(null), u = y(null), d = N(() => n.align ?? "right"), _ = y({}), c = N(() => `fu-split-button--${n.size ?? "sm"}`), w = N(() => `fu-split-button--${n.variant ?? "solid"}`), f = N(() => ({
      ...n.color ? { "--fu-split-bg": n.color } : {},
      ...n.buttonWidth ? { width: n.buttonWidth } : {}
    }));
    function m() {
      n.disabled || (l.value = !l.value, l.value && ve(() => {
        g(), window.addEventListener("click", k), window.addEventListener("resize", g);
      }));
    }
    function g() {
      if (!u.value || !i.value) return;
      const O = u.value.getBoundingClientRect(), j = i.value.offsetWidth, D = {
        top: `${O.bottom + window.scrollY + 4}px`,
        left: `${O.left + window.scrollX}px`
      };
      d.value === "right" ? D.left = `${O.right - j + window.scrollX}px` : d.value === "center" && (D.left = `${O.left + O.width / 2 - j / 2 + window.scrollX}px`), _.value = {
        position: "absolute",
        ...D,
        zIndex: "1000"
      };
    }
    function k(O) {
      u.value?.contains(O.target) || i.value?.contains(O.target) || (l.value = !1, window.removeEventListener("click", k), window.removeEventListener("resize", g));
    }
    function r() {
      n.disabled || s("main-action");
    }
    function p(O) {
      s("select", O), l.value = !1;
    }
    return he(() => {
      _e(() => {
        window.removeEventListener("click", k), window.removeEventListener("resize", g);
      });
    }), (O, j) => (t(), o("div", {
      class: H(["fu-split-button", [c.value, w.value]]),
      style: oe(f.value),
      ref_key: "splitButtonRef",
      ref: u
    }, [
      a("button", {
        class: "fu-split-button__main",
        disabled: e.disabled,
        onClick: r,
        type: "button",
        "aria-haspopup": "true",
        "aria-expanded": l.value
      }, [
        e.icon ? (t(), X(ce(e.icon), {
          key: 0,
          class: "fu-split-button__icon"
        })) : z("", !0),
        a("span", null, [
          J(O.$slots, "default", {}, void 0, !0)
        ])
      ], 8, Gr),
      a("button", {
        class: "fu-split-button__toggle",
        onClick: m,
        "aria-expanded": l.value,
        disabled: e.disabled,
        type: "button",
        "aria-label": "Toggle dropdown"
      }, [
        P(Z($e))
      ], 8, Qr),
      (t(), X(ge, { to: "body" }, [
        P(Ye, { name: "fade" }, {
          default: ae(() => [
            l.value ? (t(), o("div", {
              key: 0,
              class: H(["fu-split-button__dropdown", [`fu-split-button__dropdown--${d.value}`]]),
              ref_key: "dropdownRef",
              ref: i,
              style: oe(_.value)
            }, [
              (t(!0), o(U, null, ne(e.options, (D) => (t(), o("div", {
                key: D.value,
                class: "fu-split-button__option",
                onClick: (V) => p(D)
              }, [
                D.type === "icon" ? (t(), X(ce(D.icon), {
                  key: 0,
                  class: "fu-split-button__option-icon"
                })) : D.type === "image" ? (t(), o("img", {
                  key: 1,
                  src: D.imageUrl,
                  class: "fu-split-button__option-image",
                  alt: ""
                }, null, 8, td)) : z("", !0),
                a("span", nd, S(D.label), 1)
              ], 8, ed))), 128))
            ], 6)) : z("", !0)
          ]),
          _: 1
        })
      ]))
    ], 6));
  }
}), ad = /* @__PURE__ */ G(od, [["__scopeId", "data-v-c4af5250"]]), ld = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: ad
}, Symbol.toStringTag, { value: "Module" })), sd = ["aria-checked", "disabled"], id = /* @__PURE__ */ te({
  __name: "FusionSwitch",
  props: {
    modelValue: { type: Boolean },
    disabled: { type: Boolean },
    size: {}
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v;
    function l() {
      n.disabled || s("update:modelValue", !n.modelValue);
    }
    const i = N(() => {
      switch (n.size) {
        case "sm":
          return "fu-switch--sm";
        case "lg":
          return "fu-switch--lg";
        default:
          return "fu-switch--md";
      }
    });
    return (u, d) => (t(), o("button", {
      type: "button",
      class: H(["fu-switch", [{ "fu-switch--checked": e.modelValue, disabled: e.disabled }, i.value]]),
      role: "switch",
      "aria-checked": e.modelValue,
      disabled: e.disabled,
      onClick: l
    }, [...d[0] || (d[0] = [
      a("span", { class: "fu-switch__thumb" }, null, -1)
    ])], 10, sd));
  }
}), ud = /* @__PURE__ */ G(id, [["__scopeId", "data-v-c8285d1a"]]), rd = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: ud
}, Symbol.toStringTag, { value: "Module" })), dd = { class: "fu-tabs" }, cd = { class: "fu-tabs__header-wrapper" }, fd = { class: "fu-tabs-buttons scrollbar__control customScrollBar" }, vd = ["onClick", "disabled"], md = {
  key: 0,
  class: "fu-tab__avatar"
}, pd = ["src"], hd = {
  key: 1,
  class: "fu-tab__avatar-fallback"
}, _d = {
  key: 2,
  class: "fu-tab__title"
}, yd = {
  key: 3,
  class: "fu-tab__count"
}, gd = { class: "fu-tabs__content-wrapper" }, bd = {
  key: 0,
  class: "fu-tabs__footer"
}, wd = {
  __name: "FusionTab",
  props: {
    tabs: Array,
    defaultActiveDesktop: String,
    defaultActiveMobile: String
  },
  emits: ["tab-change"],
  setup(e, { expose: v, emit: n }) {
    const s = e, l = n, i = y(window.innerWidth <= 768), u = y(""), d = y(null), _ = N(
      () => s.tabs.filter((g) => !g.mobileOnly || i.value)
    );
    function c() {
      const g = d.value;
      g && (g.style.overflowY = "hidden", requestAnimationFrame(() => {
        g.style.overflowY = "auto";
      }));
    }
    function w(g) {
      const k = s.tabs.find((r) => r.key === g);
      !k || k.disabled || (u.value = g, l("tab-change", g), ve(() => {
        const r = d.value;
        r && (r.scrollTop = 0, c());
      }));
    }
    function f() {
      const g = s.tabs[0]?.key, k = i.value ? s.defaultActiveMobile || g : s.defaultActiveDesktop || g;
      u.value = k;
    }
    function m() {
      const g = i.value;
      i.value = window.innerWidth <= 768, g !== i.value && ve(f);
    }
    return he(() => {
      m(), f(), c(), window.addEventListener("resize", m);
    }), _e(() => {
      window.removeEventListener("resize", m);
    }), v({ setActive: w }), (g, k) => (t(), o("div", dd, [
      a("div", cd, [
        a("div", fd, [
          (t(!0), o(U, null, ne(_.value, (r) => (t(), o("button", {
            key: r.key,
            onClick: (p) => w(r.key),
            class: H(["fu-tab", { "fu-tab--active": u.value === r.key }]),
            disabled: r.disabled
          }, [
            r.avatarSrc || r.avatarText ? (t(), o("div", md, [
              r.avatarSrc ? (t(), o("img", {
                key: 0,
                src: r.avatarSrc,
                class: "fu-tab__avatar-img",
                alt: "avatar"
              }, null, 8, pd)) : (t(), o("div", hd, S(r.avatarText?.charAt(0)?.toUpperCase()), 1))
            ])) : r.icon ? (t(), X(ce(r.icon), {
              key: 1,
              size: 16,
              class: "fu-tab__icon"
            })) : z("", !0),
            !r.avatarSrc && !r.avatarText && r.title ? (t(), o("span", _d, S(r.title), 1)) : z("", !0),
            typeof r.count == "number" ? (t(), o("span", yd, S(r.count), 1)) : z("", !0)
          ], 10, vd))), 128))
        ])
      ]),
      a("div", gd, [
        a("div", {
          class: "fu-tabs__body scrollbar__control customScrollBar",
          ref_key: "tabBody",
          ref: d
        }, [
          (t(), X(Dt, null, [
            (t(!0), o(U, null, ne(_.value, (r) => Te((t(), o("div", {
              key: r.key,
              class: "fu-tab-panel"
            }, [
              J(g.$slots, r.key, {}, void 0, !0)
            ])), [
              [nt, u.value === r.key]
            ])), 128))
          ], 1024))
        ], 512),
        g.$slots.footer ? (t(), o("div", bd, [
          J(g.$slots, "footer", {}, void 0, !0)
        ])) : z("", !0)
      ])
    ]));
  }
}, kd = /* @__PURE__ */ G(wd, [["__scopeId", "data-v-b6516c96"]]), $d = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: kd
}, Symbol.toStringTag, { value: "Module" })), Md = { class: "fu-textarea-wrapper" }, Cd = {
  key: 0,
  class: "fu-textarea-label"
}, Sd = {
  key: 0,
  class: "fu-textarea-required"
}, xd = ["placeholder", "disabled", "rows", "required"], zd = {
  key: 1,
  class: "fu-textarea-error"
}, Od = /* @__PURE__ */ te({
  __name: "FusionTextArea",
  props: {
    modelValue: { default: "" },
    label: { default: "" },
    variant: { default: "subtle" },
    placeholder: { default: "" },
    size: { default: "sm" },
    disabled: { type: Boolean, default: !1 },
    error: { default: null },
    rows: { default: 3 },
    required: { type: Boolean, default: !1 }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = e, s = v, l = y(n.modelValue);
    return fe(l, (i) => s("update:modelValue", i)), fe(
      () => n.modelValue,
      (i) => l.value = i
    ), (i, u) => (t(), o("div", Md, [
      e.label ? (t(), o("label", Cd, [
        ie(S(e.label) + " ", 1),
        e.required ? (t(), o("span", Sd, "*")) : z("", !0)
      ])) : z("", !0),
      Te(a("textarea", tt(i.$attrs, {
        class: ["fu-textarea fu-form-control", [
          `fu-textarea--${e.size}`,
          { "fu-textarea--error": e.error },
          e.variant ? `fu-textarea--${e.variant}` : null
        ]],
        placeholder: e.placeholder,
        disabled: e.disabled,
        rows: e.rows,
        required: e.required,
        "onUpdate:modelValue": u[0] || (u[0] = (d) => l.value = d)
      }), null, 16, xd), [
        [ot, l.value]
      ]),
      e.error ? (t(), o("span", zd, S(e.error), 1)) : z("", !0)
    ]));
  }
}), Dd = /* @__PURE__ */ G(Od, [["__scopeId", "data-v-3beafd95"]]), Yd = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Dd
}, Symbol.toStringTag, { value: "Module" })), Vd = { class: "fu-theme-selector" }, Td = { class: "fu-theme-grid" }, Fd = ["onClick"], Bd = {
  key: 0,
  class: "fu-theme-check"
}, Id = { class: "fu-theme-label" }, Ed = /* @__PURE__ */ te({
  __name: "FuThemeSelector",
  props: {
    modelValue: { default: "auto" },
    themes: { default: () => [
      { key: "light", name: "Day Light" },
      { key: "dark", name: "Timeless Night" },
      { key: "auto", name: "Automatic" }
    ] }
  },
  emits: ["update:modelValue"],
  setup(e, { emit: v }) {
    const n = v;
    function s(l) {
      n("update:modelValue", l);
    }
    return (l, i) => (t(), o("div", Vd, [
      a("div", Td, [
        (t(!0), o(U, null, ne(e.themes, (u) => (t(), o("div", {
          key: u.key,
          class: H(["fu-theme-card", { "is-active": e.modelValue === u.key }]),
          onClick: (d) => s(u.key),
          tabindex: "0",
          role: "button"
        }, [
          a("div", {
            class: H(["fu-theme-preview", `fu-theme-preview--${u.key}`])
          }, [
            i[0] || (i[0] = a("div", { class: "fu-theme-header" }, null, -1)),
            i[1] || (i[1] = a("div", { class: "fu-theme-body" }, null, -1)),
            i[2] || (i[2] = a("div", { class: "fu-theme-footer" }, null, -1)),
            e.modelValue === u.key ? (t(), o("div", Bd, [
              P(Z(bt), { class: "fu-check-icon" })
            ])) : z("", !0)
          ], 2),
          a("span", Id, S(u.name), 1)
        ], 10, Fd))), 128))
      ])
    ]));
  }
}), Ld = /* @__PURE__ */ G(Ed, [["__scopeId", "data-v-43c6ff7c"]]), Ad = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Ld
}, Symbol.toStringTag, { value: "Module" })), jd = { class: "fu-toolbar__left" }, Pd = { class: "fu-toolbar__actions" }, Wd = /* @__PURE__ */ te({
  __name: "FusionToolbar",
  props: {
    wrap: { type: Boolean, default: !0 },
    gap: { type: String, default: "md" },
    align: { type: String, default: "center" }
  },
  setup(e) {
    const v = e, n = N(() => ({
      "flex--wrap": v.wrap,
      [`flex--gap-${v.gap}`]: !!v.gap,
      [`align--${v.align}`]: !!v.align
    }));
    return (s, l) => (t(), o("div", {
      class: H(["fu-toolbar", n.value])
    }, [
      a("div", jd, [
        J(s.$slots, "left", {}, void 0, !0)
      ]),
      a("div", Pd, [
        J(s.$slots, "right", {}, void 0, !0)
      ])
    ], 2));
  }
}), Rd = /* @__PURE__ */ G(Wd, [["__scopeId", "data-v-aa44a495"]]), Hd = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Rd
}, Symbol.toStringTag, { value: "Module" }));
function pt() {
  return ({ message: e, type: v = "info", duration: n = 3500 }) => {
    const s = document.createElement("div");
    document.body.appendChild(s);
    const l = Yt(xt, { message: e, type: v, duration: n });
    l.mount(s), setTimeout(() => {
      l.unmount(), document.body.removeChild(s);
    }, n + 500);
  };
}
const zt = localStorage.getItem("theme") || "auto", Fe = y(zt);
function qd() {
  return window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
}
function je(e, v = !1) {
  const n = e === "auto" ? qd() : e;
  if (v) {
    document.documentElement.setAttribute("data-theme", n);
    return;
  }
  document.documentElement.setAttribute("data-theme", n), localStorage.setItem("theme", e);
}
je(zt, !0);
const Nd = window.matchMedia("(prefers-color-scheme: dark)");
Nd.addEventListener("change", () => {
  Fe.value === "auto" && je("auto");
});
Vt(() => {
  je(Fe.value);
});
function Kd() {
  return {
    theme: Fe,
    setTheme: (e) => {
      Fe.value = e, je(e);
    },
    toggleTheme: () => {
      Fe.value = Fe.value === "dark" ? "light" : "dark", je(Fe.value);
    }
  };
}
const ht = /* @__PURE__ */ Object.assign({ "./components/StatusDropdown/FusionStatusDropdown.vue": en, "./components/TextInput/EditableDisplayField.vue": fn, "./components/TextInput/FusionTextInput.vue": bn, "./components/accordion/FusionAccordion.vue": Sn, "./components/actionButton/FusionActionButton.vue": ln, "./components/autocomplete/FusionAutocomplete.vue": Hn, "./components/avatar/FuAvatar.vue": Kn, "./components/avatar/FuAvatarGroup.vue": Gn, "./components/badge/FusionBadge.vue": to, "./components/button/FusionButton.vue": io, "./components/cards/FusionStatCard.vue": go, "./components/checkbox/FusionCheckbox.vue": Mo, "./components/codeInput/FusionCodeInput.vue": Yo, "./components/colourPallet/FuColorPopover.vue": Lo, "./components/combobox/FuCombobox.vue": Ko, "./components/datePicker/dateField/FusionDatePicker.vue": wa, "./components/datePicker/datePickerBackup.vue": Pa, "./components/datePicker/monthPicker/FusionMonthPicker.vue": Za, "./components/datePicker/plainDate/FusionPlainDatePicker.vue": vl, "./components/datePicker/time/FusionTimePicker.vue": _l, "./components/drawer/FusionDrawer.vue": Cl, "./components/dropdown/FusionDropdownButton.vue": Ol, "./components/dropdownInline/FusionDropdownInline.vue": Bl, "./components/dropdownMenu/DropdownMenu.vue": Al, "./components/editWrapper/EditableFieldWrapper.vue": Ul, "./components/fileUploader/FusionUpload.vue": os, "./components/filterDropdown/FusionFilterDropdown.vue": ds, "./components/floatingHeader/FusionSmartHeader.vue": ps, "./components/icons/FusionTrashIcon.vue": gs, "./components/items/activity/FusionActivityItem.vue": Ts, "./components/items/attachments/FusionAttachment.vue": qs, "./components/items/notes/FusionNoteCard.vue": ti, "./components/items/task/FusionTaskItem.vue": vi, "./components/kanban/Kanban.vue": Ti, "./components/layout/AppShell.vue": eu, "./components/list/FusionListView.vue": cu, "./components/modal/FusionConfirmDialog.vue": gu, "./components/modal/FusionModal.vue": Ai, "./components/modal/FusionPreviewModal.vue": Ou, "./components/modulemenu/FusionModuleMenu.vue": Bu, "./components/notification/FuNotification.vue": ju, "./components/notifications/FuAlert.vue": Nu, "./components/notifications/FusionToast.vue": Ju, "./components/pagination/FusionPagination.vue": sr, "./components/panel/FuPanel.vue": vr, "./components/password/FusionPasswordInput.vue": gr, "./components/progress/FusionProgressStepper.vue": zr, "./components/rangeControl/FusionRangeControl.vue": Ir, "./components/section/FuSectionHeader.vue": Rr, "./components/sidebarmenu/FusionSidebar.vue": Zr, "./components/splitButton/FusionSplitButton.vue": ld, "./components/switch/FusionSwitch.vue": rd, "./components/tabs/FusionTab.vue": $d, "./components/textArea/FusionTextArea.vue": Yd, "./components/theme/FuThemeSelector.vue": Ad, "./components/toolbar/FusionToolbar.vue": Hd }), Jd = {
  install(e) {
    for (const v in ht) {
      const n = ht[v].default, s = n.name || v.split("/").pop()?.replace(".vue", "");
      e.component(s, n);
    }
    e.config.globalProperties.FusionToast = (v) => {
      pt()(v);
    }, typeof window < "u" && (window.FusionToast = (v) => {
      pt()(v);
    });
  }
};
export {
  Le as FuAvatar,
  Ee as UserStatus,
  Jd as default,
  Kd as useTheme,
  pt as useToast
};
//# sourceMappingURL=fusion-binary-ui.es.js.map
