import{d as k,b as m,F as I,e as S,o as u,f as z,r as c}from"./iframe-DQPhCJt3.js";import{_ as T}from"./PageRenderer-DyD9qZSS.js";import"./preload-helper-Ct5FWWRu.js";import"./BlockRenderer-DInZEBfU.js";import"./_plugin-vue_export-helper-DlAUqK2U.js";const W={class:"document-root"},g=k({__name:"FuDocumentRenderer",props:{document:{}},emits:["action"],setup(a){return(s,o)=>(u(),m("div",W,[(u(!0),m(I,null,S(a.document.pages,i=>(u(),z(T,{key:i.id,page:i,theme:a.document.meta?.theme,onAction:o[0]||(o[0]=t=>s.$emit("action",t))},null,8,["page","theme"]))),128))]))}});g.__docgenInfo={exportName:"default",displayName:"FuDocumentRenderer",description:"",tags:{},props:[{name:"document",required:!0,type:{name:"any"}}],events:[{name:"action"}],sourceFiles:["/Users/xavier/Documents/Websites/Skkido/Fusion-binary-ui/src/components/editor/documentRender/FuDocumentRenderer.vue"]};const R={title:"Fusion/Renderers/DocumentRenderer",component:g,tags:["autodocs"],parameters:{layout:"fullscreen",docs:{description:{component:`
...
Watch the event log panel update in real time.
        `}}},render:()=>({components:{DocumentRenderer:g},setup(){const a=c([]),s=c(JSON.parse(JSON.stringify(b)));function o(t,n,e){for(const f of s.value.pages)for(const w of f.blocks)for(const v of w.columns){const y=v.widgets.find(x=>x.id===t);if(y){y.props[n]=e;return}}}function i({widgetId:t,type:n,payload:e}){if(a.value.unshift({widgetId:t,type:n,payload:e,time:new Date().toLocaleTimeString("en-GB")}),n==="month-change"){const f=h(e.year,e.month);o(t,"availableDates",f)}n==="date-select"&&(o(t,"slots",null),o(t,"slotsLoading",!0),setTimeout(()=>{o(t,"slots",[{id:`${e.date}-s1`,start:`${e.date}T09:00:00.000Z`,end:`${e.date}T09:30:00.000Z`},{id:`${e.date}-s2`,start:`${e.date}T10:00:00.000Z`,end:`${e.date}T10:30:00.000Z`},{id:`${e.date}-s3`,start:`${e.date}T11:30:00.000Z`,end:`${e.date}T12:00:00.000Z`},{id:`${e.date}-s4`,start:`${e.date}T14:00:00.000Z`,end:`${e.date}T14:30:00.000Z`},{id:`${e.date}-s5`,start:`${e.date}T15:00:00.000Z`,end:`${e.date}T15:30:00.000Z`}]),o(t,"slotsLoading",!1)},800))}return{document:s,eventLog:a,handleAction:i}},template:`
    ...
    `})},l={name:"Live (upload question + @action relay)",parameters:{docs:{description:{story:`
### What this story demonstrates

Shows a \`question\` widget with \`questionType: "upload"\` flowing through the exact same
\`@action\` relay as every other widget — no special-casing needed anywhere above \`BlockRenderer\`.
...
\`\`\`
FusionUpload
  emits: filesSelected (File[])
    ↓
FuQuestionRenderer.onUploadChange(files)
  localValue = files
  emit("update", { value: files })
    ↓
BlockRenderer.widgetListeners()
  normalises "update" → { widgetId, type: "update", payload: { value: files } }
  emits: @action
    ↓
PageRenderer
  forwards: @action="$emit('action', $event)"
...
DocumentRenderer
  forwards: @action="$emit('action', $event)"
    ↓
YOUR handleAction({ widgetId, type: 'update', payload })
\`\`\`
...
\`\`\`ts
function handleAction({ widgetId, type, payload }) {
  if (type === 'update' && Array.isArray(payload.value) && payload.value[0] instanceof File) {
    // payload.value is the raw File[] selected by FusionUpload
    const formData = new FormData()
    payload.value.forEach(f => formData.append('files', f))
...
**Try it:** select or drop a file into the upload box below and watch the event log —
this is the exact payload shape your production \`@action\` handler will receive.
        `}}},render:()=>({components:{DocumentRenderer:g},setup(){const a=c([]),s=c({id:"upload-demo-doc",pages:[{id:"page-upload-demo",name:"Upload demo",styles:{backgroundColor:"#fffdf0",backgroundImage:"",backgroundOpacity:1},blocks:[{id:"block-upload-header",columns:[{width:100,widgets:[{id:"widget-upload-title",type:"text",props:{content:'<p><span style="font-family: Georgia, serif; font-size: 32px; color: #1a3d2b;">Upload your files</span></p>'}},{id:"widget-upload-sub",type:"text",props:{content:'<p><span style="font-family: Inter, sans-serif; font-size: 14px; color: #6b7280;">Attach any supporting documents below — selecting a file fires the @action event on the right.</span></p>'}}]}],backgroundColor:"#fffdf0",backgroundOpacity:100,backgroundImage:"",contentWidth:"md",paddingTop:60,paddingBottom:20},{id:"block-upload-question",columns:[{width:100,widgets:[{id:"widget-upload-q1",type:"question",props:{questionType:"upload",label:"Upload supporting documents",required:!1,isVisible:!0,value:null}}]}],backgroundColor:"#fffdf0",backgroundOpacity:100,backgroundImage:"",contentWidth:"md",paddingTop:0,paddingBottom:80}]}]});function o({widgetId:t,type:n,payload:e}){a.value.unshift({widgetId:t,type:n,payload:e,time:new Date().toLocaleTimeString("en-GB")})}function i(t){const n=t?.value;return Array.isArray(n)&&n.length&&n[0]instanceof File?n.map(e=>`${e.name}  (${(e.size/1024).toFixed(1)} KB, ${e.type||"unknown"})`).join(`
`):JSON.stringify(t,null,2)}return{document:s,eventLog:a,handleAction:o,formatPayload:i}},template:`
      <div style="display: flex; min-height: 100vh;">
        ...
        ">
          ...
          <p style="margin: 0 0 16px; font-size: 11px; color: #475569; line-height: 1.6;">
            Upload a file to see<br/>
            <code style="color:#7dd3fc">{ widgetId, type: 'update', payload }</code>
          </p>
          ...
          <div v-if="!eventLog.length" style="
            ...
            font-family: system-ui;
          ">
            Select or drop a file in the upload box to see the event appear here.
          </div>
          ...
          >
            <!-- Header row -->
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
              <span style="font-weight: 700; font-size: 12px; color: #4ade80;">{{ entry.type }}</span>
              <span style="color: #475569; font-size: 10px;">{{ entry.time }}</span>
            </div>
            ...
            <!-- Payload -->
            <pre style="margin: 0; white-space: pre-wrap; color: #94a3b8; font-size: 11px; line-height: 1.6;">{{ formatPayload(entry.payload) }}</pre>
          </div>
...
Watch the event log panel update in real time.
  ...
  name: "Live (upload question + @action relay)",
  parameters: {
    docs: {
      description: {
        story: \`
    ...
    setup() {
      ...
      const document = ref({
        ...
        pages: [{
          ...
          blocks: [{
            id: "block-upload-header",
            columns: [{
              width: 100,
              widgets: [{
                id: "widget-upload-title",
                type: "text",
                props: {
                  content: '<p><span style="font-family: Georgia, serif; font-size: 32px; color: #1a3d2b;">Upload your files</span></p>'
                }
              }, {
                id: "widget-upload-sub",
                type: "text",
                props: {
                  content: '<p><span style="font-family: Inter, sans-serif; font-size: 14px; color: #6b7280;">Attach any supporting documents below \\u2014 selecting a file fires the @action event on the right.</span></p>'
                }
              }]
            }],
            backgroundColor: "#fffdf0",
      ...
      // File objects don't JSON.stringify usefully (they serialise to "{}"),
      // so pull out the bits worth showing in the log instead.
      function formatPayload(payload: any) {
        const value = payload?.value;
        if (Array.isArray(value) && value.length && value[0] instanceof File) {
          return value.map((f: File) => \`\${f.name}  (\${(f.size / 1024).toFixed(1)} KB, \${f.type || "unknown"})\`).join("\\n");
        }
...
}`,...l.parameters?.docs?.source}}};const C=["Default","Empty","LiveWithEventLog","LiveWithUploadEventLog"];export{r as Default,d as Empty,p as LiveWithEventLog,l as LiveWithUploadEventLog,C as __namedExportsOrder,R as default};
